...
_Goal: grow organic search traffic. Last updated: 5 July 2026 (Phase 1 complete; Phase 2 complete — 58 tools live, 0 stubs remaining; Phase 3 advancing — 32 guides live; full SEO audit run 5 Jul — see below)._
...
**58 tools now built and live in code, across 6 categories** (up from 9 at launch, 23 on 10 Jun, 43 on 25 Jun). Each has an h1, meta description, FAQ block (feeds FAQ structured data), and is in the sitemap automatically. **32 guides are also live** under `/guides`. **No `coming soon` stubs remain — the catalog is fully live.**
...
- **PDF to Word / PDF to Excel** (added 5 Jul) reuse `pdfjs-dist` and add **zero new npm dependencies**: real `.docx`/`.xlsx` (incl. multi-sheet) are written by a tiny hand-rolled ZIP+OOXML builder (`components/tools/officeExport.js`) instead of a heavy library like `docx` or SheetJS. **PDF to Excel is a multi-table extractor** (`pdfTable.js` + `pdfLines.js`): it parses the pdf.js operator list for drawn ruling lines (via `getOperatorList`, op codes read from `pdfjs.OPS` at runtime) to get exact cell edges — the only reliable way to handle right-aligned numbers, tight columns and wrapped cells together — and falls back to whitespace-band detection for borderless tables. It detects each table on a page separately, strips repeated headers/footers, stitches tables across page breaks, and exports one sheet per table. All validated against synthetic pdf-lib fixtures; keeps the light-bundle story intact.
