...
// MD5 (self-contained). Browsers do not expose MD5 via SubtleCrypto, so this
// is a compact pure-JS implementation. SHA-1/256/512 use the native crypto API.
function md5(input) {
  function toUtf8(str) {
    const out = [];
  ...
  for (let i = 0; i < words.length; i++) x[i] = words[i];
...
export default function HashGenerator() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const out = { "MD5": md5(input) };
      for (const a of ["SHA-1", "SHA-256", "SHA-512"]) out[a] = await shaHex(a, input);
  ...
  return (
    <div>
      <label className="field">
        <span className="field-label">Text to hash</span>
        <textarea
          className="inp mono"
          rows={4}
          value={input}
        ...
        <div className="result-list">
          {ALGOS.map((a) => (
            <div className="result-row" key={a}>
              ...
              <code className="result-val">{hashes[a] || "…"}</code>
            </div>
          ))}
        </div>
      ) : (
        <p className="muted">Enter text to see its MD5, SHA-1, SHA-256 and SHA-512 hashes.</p>
      )}
    </div>
  );
}
