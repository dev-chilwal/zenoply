...
export const SITE = {
  ...
  description:
    "Free online calculators, converters, PDF, text and developer tools. Everything runs in your browser — nothing uploaded, no signup, no clutter.",
};
...
export const TOOLS = [
  ...
  {
    slug: "sort-lines", category: "text", live: true,
    guide: { slug: "how-to-sort-a-list-alphabetically", title: "How to Sort a List Alphabetically" },
    title: "Sort Lines", h1: "Sort Lines Alphabetically",
    desc: "Sort lines alphabetically online free. Real A-Z dictionary order, natural number order (item9 before item10), sort by length or number, remove duplicates \u2014 in your browser.",
    faqs: [
      { q: "Why do other sorters put Banana before apple?", a: "Because they call JavaScript's built-in sort with no comparator, which compares characters by their numeric code. Every ASCII capital letter has a lower code than every lowercase letter, so all the capitalised words end up in a block before all the lowercase ones, and an accented letter like \u00c4 lands past Z. That is not alphabetical order in any language. This tool sorts with Intl.Collator instead, the browser's implementation of real dictionary order, so apple comes before Banana and \u00c4rger files with A." },
      { q: "How do I get item9 to sort before item10?", a: "That is on by default \u2014 it is the 'Read digits as numbers' option. Compared character by character, the '1' in item10 beats the '9' in item9, so a plain sort files item10 first. With the option on, a run of digits is compared as a single number, which puts item1, item9, item10 in the order you expect. Turn it off if you want strict character order, for example when you need to match what a database or a shell script produced." },
      { q: "Can it sort a comma-separated list?", a: "Yes. Set 'Items are separated by' to Commas and paste something like banana, apple, cherry. Each item is trimmed, sorted and joined back with a comma and a space, so you get a tidy list you can drop straight back into a sentence or a spreadsheet cell." },
      { q: "What happens to blank lines and duplicates?", a: "Blank lines are always dropped, which is what stops the trailing newline on a pasted list from becoming an empty first line in the result. Duplicates are kept unless you tick 'Remove duplicates', and when you do, they are matched using the same case rule as the sort \u2014 so a case-insensitive sort treats Apple and apple as one item. The count of what was dropped is shown under the options." },
      { q: "Does sorting change the lines themselves?", a: "No. Options like ignoring case, ignoring a leading 'The' or reading a number out of a line only build the key used for comparing; the line is copied to the output exactly as you typed it, minus leading and trailing spaces. The only settings that change content are the ones that say so \u2014 dropping blanks and removing duplicates." },
      { q: "Is my list uploaded anywhere?", a: "No. Sorting runs entirely in your browser and nothing is sent to a server, so a list of names, emails or customer records never leaves your device." },
    ],
  },
  {
    slug: "find-and-replace", category: "text", live: true,
    title: "Find and Replace", h1: "Find and Replace Text Online",
    desc: "Find and replace text online for free. Match case, whole words only, see how many matches were replaced — instant, and nothing leaves your browser.",
    faqs: [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Is the text generated on my device?", a: "Yes. Generation happens in your browser — nothing is fetched from a server, and there is no signup." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is my title sent anywhere?", a: "No. Slugs are generated locally in your browser, so the text you paste never leaves your device." },
    ],
  ...
  },
  {
    ...
    desc: "Decode JWT tokens online for free. Inspect the header, payload, issued and expiry times of any JSON Web Token — in your browser, nothing uploaded.",
    faqs: [
  ...
  {
    slug: "html-beautifier", category: "dev", live: true,
    guide: { slug: "how-to-format-html", title: "How to Format HTML Without Changing How It Renders" },
    title: "HTML Beautifier", h1: "HTML Beautifier & Formatter",
    desc: "Free HTML beautifier. Indent messy or minified HTML without changing how the page renders — tags, attributes and text are copied exactly, in your browser.",
    faqs: [
      { q: "Can beautifying HTML change how my page looks?", a: "It can, and that is the thing to watch for. In HTML the whitespace between two inline tags is a rendered space, so a formatter that puts <span>a</span><span>b</span> onto two lines has just added a word gap that was not there. This tool only ever breaks a line next to a block-level boundary, where the browser throws the whitespace away. Anything holding text or inline tags stays on one line with its text copied through byte for byte." },
      { q: "Will it rewrite my tags or attributes?", a: "No. The only thing that ever changes is whitespace. Attribute order, quoting (double, single or none), bare boolean attributes like disabled, and the letter case of tag names are all copied straight out of your source, and no tag is ever added or removed. That matters if you are formatting a template: lower-casing <MyComponent> would break a Vue or JSX-style component, and adding a </li> would change what a framework parses." },
      { q: "What happens to pre, textarea, script and style?", a: "The contents of pre and textarea are left exactly as written, because every space in them is on the page — and no newline is inserted after <pre>, since a browser swallows it. Script and style blocks are shifted to the new indentation but never rewritten, and even that shift is skipped when the block contains a template literal or a line continued with a backslash, where a line's leading spaces can be part of a string. An inline style=\"white-space: pre\" is honoured too." },
      { q: "Why did my custom element stay on one line?", a: "Because a custom element with no CSS is display: inline, so a line break beside it would add a rendered space, exactly as it would beside a <span>. If you know your components are block-level, tick \"Treat custom elements as block-level\" and they will be indented like a div. It is off by default because it is the one option that can change how the page renders." },
      { q: "Does it handle HTML with missing closing tags?", a: "Yes. A bare <li>, <p>, <tr> or <td> is legal HTML, and the tool applies the same optional-end-tag rules a browser does, so those elements are indented at the right depth. It tells you how many it found, and it does not add the missing tags — the output stays byte-identical to your input apart from whitespace. A stray closing tag with no opening tag is reported and left exactly where it is." },
      { q: "Is my HTML uploaded anywhere?", a: "No. The markup is parsed and formatted entirely in your browser and nothing is sent to a server, so a page template holding internal URLs, keys or customer data never leaves your device." },
    ],
  },
  {
    slug: "xml-formatter", category: "dev", live: true,
    guide: { slug: "how-to-format-xml", title: "How to Format XML" },
    title: "XML Formatter", h1: "XML Formatter & Validator",
    desc: "Format XML online free — pretty-print or minify, with well-formedness checking. Keeps mixed content, entities and CDATA exactly as written, all in your browser.",
    faqs: [
      { q: "Does formatting change what my XML means?", a: "No, and that is the part most formatters get wrong. The only whitespace ever removed is whitespace-only text sitting between elements, which is what indentation replaces. An element that holds any real text — including one that mixes text with tags, like a paragraph with a bold word inside it — is written on a single line with its text untouched, because pushing its children onto separate lines would insert line breaks into the text itself. Entities such as &amp;#233; are passed through as written rather than decoded, CDATA sections are copied verbatim, and anything marked xml:space=\"preserve\" is left completely alone." },
      { q: "What does the well-formedness check look for?", a: "Everything that stops a real XML parser: a closing tag that does not match its opening tag, a tag that is never closed, a bare & that should be &amp;amp;, a < inside an attribute value, an attribute with no value or with an unquoted one, the same attribute twice on one element, text outside the root element, an unterminated comment or CDATA section, a comment containing --, and an XML declaration that is not the first thing in the file. Each error names the problem and gives you the line and column, so you can go straight to it." },
      { q: "Why is my text value indented differently from the tags around it?", a: "Because a text value that runs over several lines is copied exactly as you wrote it. The tool re-indents markup, never character data, so the interior lines of a long text value keep their original spacing. The 'Trim spaces and line breaks around text values' option cleans up the whitespace at the two ends of a value — turning a name spread over three lines into a single tidy line — and you can switch it off to get a byte-exact result, which matters for signed or canonicalised documents." },
      { q: "What is the difference between beautify and minify?", a: "Beautify adds indentation and line breaks so the structure is readable; minify removes them, putting the whole document on one line to save bytes over the wire. Both make exactly the same guarantees about your content, so a document can be minified for transport and beautified again later without drifting. Minify also reports how many characters it saved." },
      { q: "Is my XML uploaded anywhere?", a: "No. The XML is parsed and formatted entirely in your browser and nothing is sent to a server, so a SOAP request, an API response or a config file holding hostnames and keys never leaves your device." },
    ],
  },
  {
    slug: "qr-code-generator", category: "dev", live: true,
    title: "QR Code Generator", h1: "QR Code Generator",
    desc: "Free QR code generator with no expiry and no scan limits. Codes for a link, Wi-Fi, UPI, email or phone, made in your browser and saved as PNG or SVG.",
  ...
  },
  {
    ...
    desc: "Free LLM token counter. Count GPT-5, GPT-4o and GPT-4 tokens exactly with OpenAI's tiktoken tables — in your browser, nothing uploaded.",
  ...
  {
    slug: "string-escaper", category: "dev", live: true,
    guide: { slug: "how-to-escape-a-string", title: "How to Escape a String" },
    title: "String Escaper", h1: "String Escaper & Unescaper",
    desc: "Free string escaper and unescaper for JSON, JavaScript, HTML, XML, CSV, SQL, regex and shell. Escapes to the exact rules of each format, in your browser.",
    faqs: [
      { q: "Which format should I pick?", a: "The one the text is going into, not the one it came from. There is no such thing as a generally escaped string: the same apostrophe is nothing at all in JSON, &#39; in an HTML attribute, &apos; in XML, two apostrophes in SQL and a four-character dance in a shell command. Pick JSON for an API payload or config file, JavaScript for a string literal you are pasting into code, HTML for anything a browser will render, XML for a SOAP body or feed, CSV for a spreadsheet column, SQL for a literal in a hand-written query, Regular expression to match text literally, and Shell for one argument on a command line." },
      { q: "Why does the JSON output escape characters that were not special?", a: "Only if you asked it to. By default the tool escapes exactly what RFC 8259 requires — the double quote, the backslash and every control character — plus any unpaired surrogate, which has no UTF-8 encoding and so cannot legally be written literally. That output is byte for byte what JavaScript's own JSON.stringify produces. The three switches add more: pure-ASCII output for a pipeline that mangles UTF-8, an escaped forward slash, and hiding <, > and & so the JSON can sit inside an HTML page without the parser finding a stray </script> in it." },
      { q: "Is escaping enough to stop SQL injection?", a: "No, and it is worth being blunt about that. Escaping only protects a value that is already inside quotes, so it does nothing for a number, a table or column name, or an ORDER BY direction pasted into a query. It also has to match the exact dialect and server settings — MySQL reads a backslash as an escape unless NO_BACKSLASH_ESCAPES is on, and standard SQL does not. A parameterised query sends the value to the server separately from the statement, so there is no parsing step to subvert; use one wherever you can. This tool is for the cases where you are genuinely writing a literal by hand, such as a migration or a one-off report." },
      { q: "Why can it not unescape a regular expression or a shell command?", a: "Because the answer would be a guess. In a regex, a backslash before a letter usually does not mean that letter: \\d is any digit, \\b is a word boundary and \\w is a word character, so there is no way to tell an escaped literal from a metasequence without knowing what the author meant. Shell quoting has the opposite problem — the same argument can be written a dozen equally valid ways, and undoing it properly means running the shell's own word-splitting and expansion rules. Every other format here reverses cleanly, so those six offer both directions." },
      { q: "What happens to characters XML cannot hold?", a: "They are reported rather than quietly emitted. XML 1.0 allows only three control characters — tab, line feed and carriage return — and there is no escape for the rest: &#1; is itself a parse error, not a way of writing character one. A tool that just swaps the five entities hands you a document that will not parse and does not say so. This one names the code points it found and offers to remove them, which is the only way to get a parseable XML 1.0 document out of that text. It also writes a carriage return as &#13; even in ordinary text, because a parser normalises a literal CR to a line feed before your program ever sees it." },
      { q: "Is my text uploaded anywhere?", a: "No. Every rule runs in JavaScript inside your browser tab, so a connection string, an API key, a customer record or a password you are quoting for a command line never leaves your device. The page keeps working with the network switched off." },
    ],
  },

  {
    slug: "cron-expression-generator", category: "dev", live: true,
    guide: { slug: "how-to-read-a-cron-expression", title: "How to Read a Cron Expression" },
    title: "Cron Expression Generator", h1: "Cron Expression Generator & Explainer",
    desc: "Free cron expression generator and explainer. Translate any crontab, Quartz or 6-field schedule into plain English, see the next run times in any time zone, and build one from a dropdown.",
    faqs: [
      { q: "What do the five fields of a cron expression mean?", a: "In order: minute (0-59), hour (0-23), day of the month (1-31), month (1-12 or JAN-DEC) and day of the week (0-7 or SUN-SAT, where both 0 and 7 mean Sunday). A * means every value, a-b is a range, a,b,c is a list and */n is a step, so 0 9 * * 1-5 reads as minute 0, hour 9, any day of the month, any month, Monday to Friday — nine in the morning on weekdays. Schedulers that add a seconds field put it in front, making six fields in total." },
      { q: "Why does my job run on days I did not expect?", a: "Almost always the day-of-month and day-of-week rule. If both of those fields are restricted — neither is a * — cron runs the job when EITHER matches, not when both do. So 0 0 1 * 1 is not \"the 1st, if it is a Monday\"; it is \"the 1st of every month, and also every Monday\", which fires around five times a month instead of once or twice a year. The tool flags this whenever it applies. Vixie cron decides \"restricted\" by looking at whether the field starts with a *, so */2 counts as unrestricted and 1-31 does not, even though they cover the same days." },
      { q: "Which cron flavour should I pick?", a: "Standard crontab for a Linux or macOS crontab, a Docker image or anything that copies Vixie cron. 6-field with seconds for node-cron, Spring's @Scheduled and the many libraries that prepend a seconds field. Quartz for Java Quartz, and for AWS EventBridge, which uses the same shape: those need a ? in exactly one of the two day fields and, importantly, number the days of the week from 1 = Sunday, so the same digit means a different day than it does in a crontab." },
      { q: "What are L, W and # in a cron expression?", a: "Quartz extensions, and the real crontab on Linux does not understand them. L in day-of-month is the last day of the month and L-3 is three days before it; LW is the last weekday; 15W is the weekday nearest the 15th, without crossing into another month. In day-of-week, 6L is the last Friday and 6#3 is the third Friday. They are genuinely useful for month-end jobs, which plain cron cannot express at all — the usual workaround is to run daily and check the date in the script." },
      { q: "What happens to my cron job when the clocks change?", a: "It depends on which way they move. The crontab(5) manual says a job skipped by a forward change is run soon after the change, so a 02:30 job on the spring-forward night runs at 03:00. Going the other way, a job set for a particular time is not re-run in the repeated hour. Jobs with a wildcard in the hour or minute simply follow the new wall clock. This tool shows next runs in whichever zone you choose and flags any run that lands in a skipped or repeated hour, because the answer differs between schedulers." },
      { q: "Is my expression sent anywhere?", a: "No. The parsing, the description and the next-run search all run in JavaScript inside your browser tab, so an internal hostname or job name in a comment never leaves your device. The page keeps working with the network switched off." },
    ],
  },
  ...
  // ---- CONVERT ----
  {
    ...
    faqs: [
      ...
      { q: "Is the conversion done in my browser?", a: "Yes. Colors are converted locally on your device — nothing is uploaded and there is no signup." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Does this need an internet connection?", a: "Only to load the page. The conversion itself runs in your browser, so nothing you enter is sent anywhere." },
    ],
  },
  {
  ...
  },
  {
    ...
    desc: "Convert Unix epoch timestamps to readable dates and back, free. Seconds and milliseconds, with local, UTC and ISO output — all in your browser.",
    faqs: [
      ...
      { q: "Are the timestamps I paste sent to a server?", a: "No. Conversion happens entirely in your browser and nothing you paste is uploaded." },
    ],
  },
  {
    ...
    desc: "Convert a JSON array to CSV online for free. Turn objects into a spreadsheet-ready CSV with headers, pick your delimiter — all in your browser.",
    faqs: [
  ...
  },
  {
    ...
    desc: "Convert CSV to JSON online for free. Parse comma, tab, semicolon or pipe-separated data into a clean JSON array — all in your browser, no upload.",
    faqs: [
      ...
      { q: "Is my CSV data uploaded?", a: "No. Parsing runs in your browser, so even large or confidential exports never leave your device." },
    ],
  },
  {
    ...
    desc: "Convert YAML to JSON online for free. Paste config and get clean, pretty-printed JSON — handles nested maps and lists, fully in your browser.",
    faqs: [
  ...
  {
    slug: "json-to-yaml", category: "convert", live: true,
    guide: { slug: "how-to-convert-json-to-yaml", title: "How to Convert JSON to YAML" },
    title: "JSON to YAML", h1: "JSON to YAML Converter",
    desc: "Convert JSON to YAML online for free. Keeps long IDs and version numbers exact, quotes values YAML would misread, and runs entirely in your browser.",
    faqs: [
      { q: "Why are some of my values wrapped in quotes?", a: "Because unquoted YAML is typed by shape, and a handful of ordinary strings would come back as something else. The country code NO and the words yes, no, on and off are read as booleans by YAML 1.1 loaders such as PyYAML and Ruby's Psych - the famous 'Norway problem'. Values like 017, 0x1F, 1_000, 1:30 and 2026-08-25 are read as an octal number, a hex number, a thousand, ninety seconds and a date. Anything that would be re-typed like this is quoted so it stays the exact string your JSON had." },
      { q: "Will a long ID or a version number like 1.50 change?", a: "No. The converter reads each number straight out of your JSON text and writes those same digits back, so a 20-digit identifier keeps every digit and 1.50 stays 1.50. That is worth knowing because the obvious approach does not: JavaScript's own JSON.parse rounds 12345678901234567890 to 12345678901234567000 and rewrites 1.50 as 1.5. The one change made on purpose is to exponent numbers - 1e5 is written as 1.0e+5, because the YAML 1.1 resolver reads a bare 1e5 as a string rather than a number." },
      { q: "How are multi-line strings converted?", a: "As a literal block scalar - the key, then a | marker, then the text indented beneath it - which is how multi-line values are meant to look in YAML and stays readable in a diff. A block scalar cannot represent every string exactly: if a line ends in a space, or the text begins with one, the converter falls back to a double-quoted string, which is always exact because YAML's double-quoted style is a superset of a JSON string." },
      { q: "What happens if my JSON has duplicate keys?", a: "The last value for a repeated key is kept, matching what JSON.parse and most parsers do, and the tool tells you how many duplicates it found so the collision is not silent. YAML has no way to write the same key twice in one mapping, so keeping both is not an option." },
      { q: "Is my JSON uploaded anywhere?", a: "No. The conversion runs entirely in your browser and nothing is sent to a server, so you can convert a config file holding hostnames, tokens or connection strings without it leaving your device." },
    ],
  },
  {
    slug: "xml-to-json", category: "convert", live: true,
    guide: { slug: "how-to-convert-xml-to-json", title: "How to Convert XML to JSON" },
    title: "XML to JSON", h1: "XML to JSON Converter",
    desc: "Convert XML to JSON online free. Decodes entities properly, keeps IDs and versions exact, strips namespace prefixes, and runs entirely in your browser.",
    faqs: [
      { q: "Why did my list come back as an object instead of an array?", a: "Because XML has no way to say 'this is a list'. A converter can only look at how many times an element name appears, so two <item> tags become a JSON array and one <item> tag becomes a plain object. That means the shape of your output depends on the data rather than on your schema, and code written to loop over a list breaks the day the list has a single entry. Switch on 'Always arrays' to get an array for every child element regardless of count - it is the option to use whenever something will read the JSON programmatically. The tool also warns you whenever this ambiguity is present in what you pasted." },
      { q: "How are attributes represented?", a: "As ordinary keys with a marker in front, so <price currency=\"USD\">44.95</price> becomes { \"@currency\": \"USD\", \"#text\": \"44.95\" }. The prefix exists because XML lets an attribute and a child element share a name - <book id=\"1\"><id>2</id></book> is perfectly legal - and without a prefix one silently overwrites the other. You can choose @, _, $ or no prefix at all; if you pick no prefix and a collision actually happens, the tool tells you which key was affected instead of losing a value quietly." },
      { q: "Will my numbers, IDs and version strings survive?", a: "Yes, because every value stays a string unless you turn on type conversion, and even then a number is only written when its digits come back identical. That keeps a leading zero (007 stays \"007\"), a trailing zero (a version of 1.50 stays \"1.50\") and any integer past 2^53 - a 20-digit ID would otherwise be rounded to a double and come back with the last few digits replaced by zeros, which is what a converter built on parseFloat does." },
      { q: "Can it remove the soap: and ns: prefixes?", a: "Yes. Turn on 'Remove namespace prefixes' and soap:Envelope becomes Envelope, which makes a SOAP response far easier to navigate in code. The xmlns declarations are dropped at the same time, since the prefixes they define no longer appear anywhere. If removing prefixes would make two different names collide - a:id and b:id both becoming id - the tool says so rather than silently keeping one." },
      { q: "What gets lost in the conversion?", a: "Four things, and each one is reported rather than dropped quietly. Comments, processing instructions and the XML declaration have nowhere to live in JSON. Mixed content - an element holding text and tags together, like a paragraph with a bold word in it - loses the order between the text and the tags, because JSON object keys carry no position. Undeclared entities such as &nbsp; are left exactly as written, since only &lt; &gt; &amp; &apos; and &quot; exist in XML without a DTD. And the array ambiguity described above." },
  ...
  {
    slug: "json-to-xml", category: "convert", live: true,
    guide: { slug: "how-to-convert-json-to-xml", title: "How to Convert JSON to XML" },
    title: "JSON to XML", h1: "JSON to XML Converter",
    desc: "Convert JSON to XML online free. Keeps long IDs and trailing zeros exact, escapes correctly, renames keys XML cannot use, and runs entirely in your browser.",
    faqs: [
      { q: "How do I control which keys become attributes?", a: "Any key starting with @ becomes an attribute on its element, so { \"@id\": \"bk101\", \"title\": \"...\" } gives <book id=\"bk101\"><title>...</title></book>. You can switch the marker to _ or $, or turn attributes off entirely so that every key becomes an element. The key called #text becomes the element's own text, which is what lets an element carry both attributes and a value. This is the exact mirror of the XML to JSON converter, so output from one feeds straight back into the other." },
      { q: "What happens to my arrays?", a: "An array becomes repeated sibling elements, which is the only thing XML has that behaves like a list: { \"item\": [\"a\", \"b\"] } becomes <item>a</item><item>b</item>. Two consequences follow and both are reported. An empty array produces nothing at all, so an empty list and a missing key look identical afterwards. And an array sitting directly inside another array has no key to take a name from, so its entries are written as <item> - a name this tool invents rather than one from your data. You can change that name." },
      { q: "Why did my key get renamed?", a: "Because an XML element name is far narrower than a JSON key. A name cannot start with a digit and cannot contain a space or most punctuation, so \"first name\" has to be written <first_name> and \"2024\" has to become <_2024>. Every rename is listed. The case worth watching for is two sibling keys that end up with the same name - \"first name\" and \"first_name\" both give <first_name> - because read back that is a two-item array rather than two fields, so it is called out separately." },
      { q: "Will a long ID or a price like 1.50 survive?", a: "Yes. The JSON is parsed with every number kept as the text you wrote, and that text is what gets written out, so 1.50 stays 1.50 and a 20-digit order ID keeps all twenty digits. A converter that runs your JSON through JSON.parse first cannot do this: it rounds anything past 2^53 to the nearest double and rewrites 1.50 as 1.5, which quietly damages IDs, version pins and money." },
      { q: "How is null handled?", a: "XML has no null, so you choose. An empty tag is the default and the honest caveat is that it reads back as an empty string. xsi:nil=\"true\" marks the element explicitly and adds the one namespace declaration it needs to the root element, which is the right choice if a schema or a typed reader is at the other end. Leave out removes those elements entirely, so the key is simply absent." },
      { q: "Is the output valid XML?", a: "Yes, and that is not automatic. Ampersands and angle brackets are escaped, including every > - a literal one closing the sequence ]]> is a parse error, not a stylistic choice. Carriage returns are written as &#13; and tabs and newlines inside attribute values as &#9; and &#10;, because a parser turns the literal characters into line feeds and spaces before your code ever sees them. Characters XML 1.0 cannot hold at all, such as most control codes, are reported rather than written into a document that would not parse." },
      { q: "Is my JSON uploaded anywhere?", a: "No. The JSON is parsed and converted entirely in your browser and nothing is sent to a server, so an API payload or a config file holding hostnames and keys never leaves your device." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is anything I type stored?", a: "No. The number is converted in your browser and nothing is uploaded or kept." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is my cheque amount private?", a: "Yes. The amount is converted in your browser and never uploaded — nothing you type is stored." },
    ],
  },
  {
    ...
    desc: "Convert CGPA to percentage free with the CBSE 9.5, 10-point or VTU formula — and back again. Your grades stay in your browser, with no signup.",
    faqs: [
      ...
      { q: "Are my grades uploaded anywhere?", a: "No. The conversion runs in your browser; your CGPA and credits stay on your own device." },
    ],
  },

  {
    slug: "base-converter", category: "convert", live: true,
    guide: { slug: "how-to-convert-binary-to-decimal", title: "How to Convert Binary to Decimal" },
    title: "Base Converter", h1: "Number Base Converter",
    desc: "Convert binary, octal, decimal and hex online free \u2014 any base from 2 to 36, exact for numbers of any size, with fractions and two's complement. Runs in your browser.",
    faqs: [
      { q: "How do I convert binary to decimal?", a: "Set the input base to 2, paste or type the binary digits, and read the Decimal row. Spaces and underscores are ignored, so a grouped value like 1010 1010 pastes straight in and converts to 170. The same page gives you octal and hexadecimal at the same time, and every row has its own Copy button." },
      { q: "Why do other converters get large numbers wrong?", a: "Because they are built on JavaScript's parseInt and toString, which run every value through a 64-bit floating point number. That holds whole numbers exactly only up to about 9 quadrillion (2 to the power 53), and silently rounds everything above it. Ask one of those converters for 0xFFFFFFFFFFFFFFFF in binary and it answers with a 1 followed by 64 zeros, when the real answer is 64 ones. This tool uses arbitrary-precision integers instead, so a 64-bit hash, a snowflake ID or a 128-bit UUID converts exactly however long it is." },
      { q: "Can it convert numbers with a decimal point?", a: "Yes, and it does the arithmetic exactly rather than through a float. Type 10.625 in base 10 and you get 1010.101 in binary. Where an expansion never ends it is shown with the repeating run in brackets \u2014 0.1 in decimal is 0.0(0011) in binary, because one tenth has no exact binary form. That is the honest answer; a converter that prints a tidy 55-digit number there is showing you the binary of the nearest floating point number, not of a tenth." },
      { q: "How do I get the binary of a negative number?", a: "Type it with a minus sign. The main rows show the sign and the magnitude, which is how a number is written on paper, and a Two's complement panel appears underneath with the actual stored bit pattern at 8, 16, 32 and 64 bits \u2014 the form a computer uses. A value that will not fit in a width says so instead of quietly wrapping around, so -200 is reported as outside the 8-bit range rather than shown as 0x38." },
      { q: "What bases does it support?", a: "Every base from 2 to 36, using the digits 0-9 then a-z. Binary, octal, decimal and hexadecimal are always shown, and the 'Also show base' box adds any other one \u2014 base 36 for short IDs, base 32, base 12, or whatever your assignment asks for." },
      { q: "Is anything I type uploaded?", a: "No. The conversion runs entirely in your browser, so a key, a hash or an internal ID never leaves your device. There is no signup and nothing is stored." },
    ],
  },
  ...
  // ---- FINANCE ----
  {
    ...
    faqs: [
      ...
      { q: "Are the amounts I enter stored anywhere?", a: "No. The calculation runs in your browser and nothing is sent to a server — there is no account and no record of what you type." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is anything I type here saved?", a: "No. Every figure stays on your device — the maths runs locally in your browser and nothing is uploaded." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Do you store my loan details?", a: "No. The amount, rate and tenure you enter never leave your browser — the EMI is worked out on your own device, with no signup." },
    ],
  },
  {
    ...
    desc: "Add or remove GST or VAT and see the tax breakup instantly. Follows your country's rates — India, the UK, Europe, Australia, Canada and Singapore.",
    faqs: [
      ...
      { q: "Are my invoice amounts sent to a server?", a: "No. The tax breakup is calculated in your browser, so the amounts you enter stay on your device." },
    ],
  },
  {
    slug: "income-tax-calculator", category: "finance", live: true,
    title: "Income Tax Calculator", h1: "Income Tax Calculator",
    desc: "Free income tax calculator. Compare India's old and new regime for FY 2025-26, or Australian tax and Medicare levy — private, in your browser.",
    faqs: [
      ...
      { q: "Is my income information sent anywhere?", a: "No. Your salary and deduction figures are processed entirely in your browser and never uploaded. There is no account to create." },
    ],
  ...
  },
  {
    ...
    desc: "Work out your monthly take-home pay after tax across 10 countries, each on its own national rules. Free — and your salary never leaves your browser.",
    faqs: [
      ...
      { q: "Is my salary data private?", a: "Yes. Your CTC and deductions are never uploaded — the whole calculation runs in your browser, and nothing is stored or logged." },
    ],
  },
  {
    ...
    desc: "Calculate fixed deposit maturity and interest earned. Free FD calculator using your country's compounding convention — quarterly for Indian banks.",
    faqs: [
      ...
      { q: "Do you keep a record of my deposit amounts?", a: "No. Nothing is uploaded or saved — the maturity figure is worked out locally in your browser." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is anything I enter uploaded?", a: "No. The calculation happens in your browser, so your figures stay on your own device." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Are my mortgage details stored?", a: "No. The loan amount, rate and term stay in your browser — nothing is sent to a server, and there is no signup." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Does this run in my browser?", a: "Yes. Every calculation happens locally on your device, with nothing uploaded and no account needed." },
    ],
  },
  {
    ...
    faqs: [
      ...
      { q: "Is my deposit information sent anywhere?", a: "No. The projection is worked out in your browser and nothing you type is uploaded or stored." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Do you save what I enter?", a: "No. The maturity is calculated locally in your browser — your figures never leave your device." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Is anything uploaded when I calculate?", a: "No. The maths runs entirely in your browser; no amount you enter is sent to a server." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Are my corpus and withdrawal figures private?", a: "Yes. They are processed in your browser only, never uploaded, and nothing is retained once you close the page." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Is my investment data stored?", a: "No. The CAGR is computed in your browser and the values you enter are never sent anywhere." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Does anything I enter leave my device?", a: "No. The calculation runs locally in your browser, with no upload and no account." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Is this calculator private?", a: "Yes. It runs entirely in your browser — nothing you enter is uploaded, stored or tracked." },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Do you record the amounts I enter?", a: "No. Everything is calculated in your browser and nothing is sent to a server." },
    ],
  ...
  },
  {
    ...
    desc: "Free NPS calculator to estimate your National Pension System corpus, tax-free lump sum, annuity and monthly pension — worked out in your browser.",
    faqs: [
      ...
      { q: "Is my retirement and salary data uploaded?", a: "No. Your contributions and expected returns are processed in your browser only and never leave your device." },
    ],
  ...
  },
  {
    ...
    desc: "Free gratuity calculator for India under the Payment of Gratuity Act. Estimate gratuity from Basic+DA and service years — private, in your browser.",
    faqs: [
      ...
      { q: "Is my salary information kept anywhere?", a: "No. Your Basic + DA and service length are used only in your browser to work out the figure — nothing is uploaded or saved." },
    ],
  ...
  },
  {
    ...
    desc: "Free HRA exemption calculator under Section 10(13A). Find your exempt and taxable House Rent Allowance — your rent figures stay in your browser.",
    faqs: [
      ...
      { q: "Are my rent and salary details private?", a: "Yes. The exemption is calculated in your browser — your rent, Basic + DA and HRA figures are never uploaded." },
    ],
  ...
  },
  {
    ...
    desc: "Compress PDF files online for free to reduce file size. Shrink large, scanned or image-heavy PDFs in your browser — nothing uploaded.",
    faqs: [
  ...
  {
    slug: "compress-pdf-to-size", category: "pdf", live: true,
    guide: { slug: "how-to-compress-a-pdf-to-a-specific-size", title: "How to Compress a PDF to 100KB (or Any Size Limit)" },
    title: "Compress PDF to Size (KB)", h1: "Compress a PDF to a Target Size",
    desc: "Compress a PDF to an exact size limit — 100KB, 200KB, 500KB or any cap a form demands. The quality is found for you, in your browser, no upload.",
    faqs: [
      { q: "How does it hit an exact KB target?", a: "It renders your pages once, then searches for the highest JPEG quality whose rebuilt PDF still fits your limit, re-encoding a few times and keeping the best result. If even the lowest quality is too big, it also lowers the page resolution and searches again, so the cap is met without you guessing at a quality slider. The document's own structure is measured from a real build rather than estimated, so the final file genuinely fits rather than landing just over." },
      { q: "Is 1 KB counted as 1024 bytes?", a: "Yes. A 100 KB target means 102,400 bytes, which is how Windows and most upload forms count, and the exact byte figure is shown next to the result. A few strict portals treat 100 KB as 100,000 bytes instead, so if one rejects a file that looks like it fits, set the target slightly lower and compress again." },
      { q: "Will my PDF still have selectable text?", a: "No. Reaching a fixed byte target means re-rendering each page as a compressed image, so text becomes part of the picture and can no longer be selected or searched. That is the trade every size-target compressor makes, and it is why the tool works best on scans, certificates and image-heavy documents. If your file is already under the limit it is handed back untouched, with its text intact." },
      { q: "Why can't it reach a very small target?", a: "Two reasons. Every PDF spends some bytes on its own structure — roughly a third of a kilobyte per page — so a 50-page document cannot fit in 10 KB no matter what the pages look like. And past a point there are simply too many pixels sharing too few bytes to stay readable. When the target is unreachable the tool says so and still offers the smallest result it managed, rather than pretending it succeeded." },
      { q: "Is my PDF uploaded to a server?", a: "No. The file is read from your own device, rendered and re-encoded by your browser, and the download comes from memory. Nothing is sent anywhere — which matters for the ID documents and certificates these size limits are usually attached to." },
    ],
  },
  {
  ...
  },
  {
    ...
    desc: "Remove a PDF password online for free. Strip printing, copying and editing limits, or enter the open password once — all in your browser.",
    faqs: [
  ...
  {
    slug: "protect-pdf", category: "pdf", live: true,
    guide: { slug: "how-to-password-protect-a-pdf", title: "How to Password Protect a PDF" },
    title: "Protect PDF – Add a Password", h1: "Password Protect a PDF",
    desc: "Password protect a PDF free. Add an AES-256 open password so the file can't be opened without it — all in your browser, nothing uploaded.",
    faqs: [
      { q: "How strong is the encryption?", a: "AES-256, written as revision 6 of the PDF encryption handler — the strongest the PDF format defines, and the same class of encryption used for disks and network traffic. Streams, strings and the whole file are encrypted with it. Older 40-bit and RC4 modes are deliberately not offered: they are broken, and a file using them is protected in appearance only." },
      { q: "What happens if I forget the password?", a: "The file cannot be opened again — by this tool or any other. Your password is what derives the encryption key, and no copy of that key is stored anywhere in the document, so without the password the pages are genuinely unreadable rather than merely hidden. Nothing is kept on our side either, because nothing is sent to us. Save the password in a password manager before you download." },
      { q: "Is my PDF uploaded to a server?", a: "No. The file is encrypted in your browser, and neither the document nor the password ever leaves your device — which is the point, since a tool that asks you to upload a file and a password to protect it has already defeated the purpose." },
    ],
  },
  {
    slug: "pdf-metadata", category: "pdf", live: true,
    guide: { slug: "how-to-remove-metadata-from-a-pdf", title: "How to Remove Metadata from a PDF" },
    title: "PDF Metadata Viewer & Editor", h1: "PDF Metadata Viewer and Remover",
    desc: "View, edit or remove PDF metadata free. See the author, creator and dates hidden in a PDF, then strip them — in your browser, nothing uploaded.",
    faqs: [
      { q: "What metadata does a PDF actually store?", a: "Every PDF has an information dictionary holding a title, author, subject, keywords, the creating application, the PDF writer that produced it, and the creation and modification dates. Most of it is filled in automatically: the author is usually whatever name is registered to the copy of Word or the operating system the file came from, so a document you never signed can still carry your full name." },
      { q: "Does removing metadata change the pages?", a: "No. The title, author, dates and hidden packets live in separate structures alongside the page content, so clearing them leaves the text, images, fonts and page size exactly as they were. The page count and layout are identical — only the descriptive entries about the file are gone." },
      { q: "Is it enough to clear the author box in Word?", a: "Often not. Alongside the visible fields a PDF can carry an XMP packet — an XML block that repeats the title and author in a different format — plus per-page packets and application-private blocks, and no PDF reader displays those. This tool clears the XMP packet as well, and overwrites the object rather than merely unlinking it, so the text is not left recoverable in the saved bytes." },
    ],
  },
  {
  ...
  },
  {
    ...
    desc: "Convert PDF to Word (DOCX) online for free. Extract the text into an editable document for Word, Google Docs or Pages — nothing uploaded.",
    faqs: [
  ...
  {
    slug: "pdf-to-text", category: "pdf", live: true,
    guide: { slug: "how-to-extract-text-from-a-pdf", title: "How to Extract Text from a PDF" },
    title: "PDF to Text", h1: "PDF to Text Converter",
    desc: "Convert PDF to text online free. Extract the text from a PDF, read it on the page, copy it or download a .txt file — nothing is uploaded.",
    faqs: [
      { q: "Can I copy the text without downloading a file?", a: "Yes. The extracted text appears in an editable box on the page, so you can read it, tidy it up and hit Copy to put it straight on your clipboard. Download .txt is there when you want the file, but it is optional." },
      { q: "What is the difference between flowing paragraphs and keeping line breaks?", a: "A PDF stores each visual line separately, so a paragraph that wrapped across four lines arrives as four pieces. Flowing paragraphs stitches those back together, which is what you want for articles and letters. Keep line breaks leaves every line where it was, which is what you want for addresses, tables, code and poetry." },
      { q: "Why is nothing extracted from my scanned PDF?", a: "A scan is a photograph of a page, so there are no characters in the file to extract - only pixels. Run it through the OCR PDF tool first to recognise the words and add a real text layer, then extract the text from the result." },
      { q: "Is my PDF uploaded to a server?", a: "No. The file is opened and read entirely in your browser, so a contract, statement or private report never leaves your device." },
    ],
  },
  {
    slug: "resize-pdf-pages", category: "pdf", live: true,
    guide: { slug: "how-to-change-pdf-page-size", title: "How to Change PDF Page Size (A4, Letter, Legal)" },
    title: "Resize PDF Pages", h1: "Resize PDF Pages — Change Page Size",
    desc: "Change PDF page size online free. Convert a PDF to A4, Letter, Legal or A3, scale pages up or down, and fix mixed page sizes — nothing is uploaded.",
    faqs: [
      { q: "Does this make the file smaller?", a: "No - this changes the paper size of the pages, not the number of bytes. A page redrawn from Letter to A4 holds the same text and images, so the file size barely moves. If you want a smaller file, use Compress PDF or Compress PDF to Size instead." },
      { q: "Will my links and form fields survive?", a: "Yes. Pages are resized in place rather than redrawn into a new document, so links, bookmarks, form fields and comments come across and are moved and scaled to match the content. That is the main difference from converters that flatten each page into a picture." },
      { q: "What happens to landscape pages in a portrait document?", a: "With Orientation left on \"Keep each page's\", a landscape page becomes landscape A4 and a portrait page becomes portrait A4, so nothing is turned on its side. Choose Portrait or Landscape to force every page the same way round instead." },
      { q: "Why is there a white border around my page?", a: "Because A4 and Letter are not the same shape. Fitting Letter content onto A4 scales it until the first edge touches, which leaves a thin band on the other axis. Set a margin of 0 and leave Content on Fit to page to keep that band as small as the two paper shapes allow." },
      { q: "Is my PDF uploaded to a server?", a: "No. The file is opened, resized and saved entirely inside your browser tab, so a contract, invoice or private report never leaves your device." },
    ],
  },
  {
    slug: "crop-pdf", category: "pdf", live: true,
    guide: { slug: "how-to-crop-a-pdf", title: "How to Crop a PDF (Trim Margins Without Losing Text)" },
    title: "Crop PDF", h1: "Crop PDF — Trim Page Margins",
    desc: "Crop a PDF online free. Drag a box over the part you want to keep, or trim white margins automatically, then download — nothing is uploaded.",
    faqs: [
      { q: "How do I choose what to keep?", a: "Drag the box on the page preview over the part you want to keep, or drag a corner or edge to resize it. You can also type an exact trim in millimetres for each edge, and flip through the pages to check the box against the rest of the document before you download." },
      { q: "What does \"Trim white margins\" do?", a: "It renders the pages, finds where the ink actually stops on each one, and sets the box to the smallest crop that keeps every page whole. Isolated specks of scanner dust are ignored, and blank pages are skipped so they cannot drag the crop back out to the full sheet. On a long document it measures an even sample of pages and tells you how many it checked." },
      { q: "Does cropping shrink the file size?", a: "Barely. Cropping changes the page boundary, not the contents — the text and images are all still there, just outside the visible area. If you need a smaller file, use Compress PDF or Compress PDF to Size afterwards." },
      { q: "Can someone recover the part I cropped off?", a: "Yes, so never use a crop to hide anything confidential. A crop moves the page boundary; it does not delete anything. Some readers, including text extractors built on pdf.js, will not show the hidden text — which is exactly what makes this dangerous — but anyone who sets the page box back gets every word returned in full. To genuinely remove content, delete it in an editor before exporting the PDF." },
      { q: "Will my links and form fields survive?", a: "Yes. Pages are cropped in place rather than redrawn, so text stays selectable and links, bookmarks, form fields and comments come across untouched and stay exactly where they were. Rotated pages keep their rotation, and \"top\" always means the top edge as you see it on screen." },
      { q: "Is my PDF uploaded to a server?", a: "No. The page preview is rendered and the crop applied entirely inside your browser tab, so a contract, invoice or private report never leaves your device." },
    ],
  },
  {
    slug: "extract-images-from-pdf", category: "pdf", live: true,
    guide: { slug: "how-to-extract-images-from-a-pdf", title: "How to Extract Images from a PDF (Without Losing Quality)" },
    title: "Extract Images from PDF", h1: "Extract Images from a PDF",
    desc: "Extract images from a PDF online free. Pulls out every photo at full stored resolution, keeps original JPEGs byte for byte, downloads as a zip — no upload.",
    faqs: [
      { q: "How is this different from converting the PDF to images?", a: "PDF to JPG photographs each whole page — text, background and all — at whatever resolution you pick. This reaches inside the page and takes out the pictures themselves, at the size they were stored rather than the size they are printed. A photo placed as a small thumbnail in a report is very often a full-resolution original underneath, and this is what gets it back." },
      { q: "Are the images the same quality as the originals?", a: "Where the PDF stored a plain JPEG, \"Original\" hands you that exact file back, byte for byte — nothing is decoded and re-encoded, so nothing is lost. Everything else is rebuilt from the decoded pixels. PNG does that without any lossy compression: on an opaque image every pixel comes back exactly, and transparency is kept. JPG re-encodes and flattens transparency onto white. Pictures the PDF stores in a form a standalone JPEG can't express — CMYK, palette colour, an inverted or soft-masked image — always take the rebuilt route, because handing over the raw bytes there would give you a file with the wrong colours." },
      { q: "Is PNG exactly lossless?", a: "On an opaque image, yes — every pixel is bit-for-bit what the PDF held. Where a pixel is partly transparent there is one caveat worth knowing: browsers store canvas colour already multiplied by transparency, so the colour hidden underneath a faint pixel can come back a step or two off, and a fully invisible pixel loses the colour it was never showing. The transparency itself is always exact, and so is the colour you actually see, because it is that multiplied value the browser keeps precisely. If you need the untouched bytes, use \"Original\" — it never decodes the image at all." },
      { q: "Why did it find no images, or fewer than I can see?", a: "Not everything that looks like a picture is one. Charts, logos, diagrams and borders from a word processor or design tool are usually vector artwork drawn with lines and curves, and one-colour stencils are shapes rather than images — neither has a picture inside to take out. The size limit also hides the tiny gradient strips and rule artwork most documents are full of; drop it to 0 to see everything." },
      { q: "Why does the same photo only appear once?", a: "Because it is one image. A logo on every page, or a photo placed twice on the same page, is stored once in the file and drawn repeatedly, so it is saved once rather than handed back forty times under forty names." },
      { q: "What does the zip contain?", a: "One file per image, named after the PDF with the page it was found on and its position on that page, so doc-p3-2.jpg is the second image on page 3. The archive stores the images as they are rather than compressing them again — JPEG and PNG are already compressed, so zipping them harder would only cost time." },
      { q: "Is my PDF uploaded to a server?", a: "No. The file is opened and its images taken out entirely inside your browser tab, so a private report, portfolio or scanned document never leaves your device." },
    ],
  },
  {
    slug: "fill-pdf-form", category: "pdf", live: true,
    guide: { slug: "how-to-fill-in-a-pdf-form-and-flatten-it", title: "How to Fill In a PDF Form and Flatten It" },
    title: "Fill PDF Form", h1: "Fill In a PDF Form — and Flatten It",
    desc: "Fill in a PDF form online free. Type into the fields, tick the boxes, then flatten so the answers can't be edited — nothing is uploaded.",
    faqs: [
      { q: "What does flattening actually do?", a: "It paints each answer onto the page and then deletes the interactive field, so what was a typed-in value becomes part of the document itself. The text still selects, copies and prints exactly as before, but there is no longer a box to click into, so nobody can type over your answers or accidentally clear them. That is what an office means when it asks you not to send an editable form. Leave it switched off and the file stays a working form you can come back and change." },
      { q: "Why does it say my PDF has no form fields?", a: "Because most documents that look like forms are flat: the boxes and lines are printed artwork, not real fields, and there is nothing in the file to fill. A PDF only has fields if whoever made it saved it as a fillable form. This tool fills the fields a form already has — it cannot add fields that were never there, so a scanned or printed-style form has to be filled by hand or obtained in its fillable version." },
      { q: "Can it sign the PDF?", a: "No, and it deliberately does not pretend to. A digital signature is a cryptographic operation performed with a certificate that only you hold, so no in-browser tool can produce a valid one for you. Signature fields in your form are listed but left alone; if you flatten, the empty signature box is removed rather than replaced with something that merely looks like a signature." },
      { q: "Why can't it draw the rupee sign or Hindi text?", a: "Field appearances are redrawn with a built-in Helvetica, which covers Latin, accents and most European symbols but not the rupee sign, Devanagari, CJK or emoji. When a value uses one of those, the answer is still written into the file and your PDF reader draws it with its own fonts — so it appears correctly in Acrobat and in most browsers — but that field cannot be flattened, because flattening bakes in the drawing and there would be nothing to bake." },
      { q: "Will the rest of the document survive?", a: "Yes. The pages, text, images and page sizes are untouched — only the field values, and on flattening the field objects themselves, are changed. Links, bookmarks and comments come across unmoved, and the file's title, author and dates are left exactly as they were rather than being re-stamped." },
      { q: "Is my PDF uploaded to a server?", a: "No. The form is read, filled and saved entirely inside your browser tab, so an application, claim or tax form — and everything you type into it — never leaves your device." },
    ],
  },
  {
    ...
    desc: "Extract tables from a PDF to Excel (XLSX) or CSV free. Detects rows and columns, previews the table, downloads a spreadsheet — no upload.",
    faqs: [
      ...
      { q: "Is my PDF uploaded to a server?", a: "No. The file is read and its tables extracted entirely in your browser, so a bank statement or invoice never leaves your device." },
    ],
  },

  {
    ...
    desc: "Add page numbers to a PDF online for free. Choose the position, format and starting number, skip the cover page — all in your browser.",
    faqs: [
  ...
  },
  {
    ...
    desc: "Find and remove blank pages from a scanned PDF free. Detects empty pages and shows you each one before deleting — in your browser, no upload.",
    faqs: [
  ...
  },
  {
    ...
    desc: "Make a scanned PDF searchable free. OCR adds an invisible text layer so you can select and search the text — English and Hindi, in your browser.",
    faqs: [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Are my images uploaded?", a: "No. Conversion runs in your browser using the canvas API, so your images never leave your device." },
    ],
  },
  {
  ...
  },
  {
    ...
    desc: "Compress a JPG or PNG to an exact size in KB — 20KB, 50KB, 100KB or any limit a form demands. Hits the target in your browser, nothing uploaded.",
    faqs: [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Is my photo uploaded anywhere?", a: "No. The crop and resize happen in your browser, so your photo never leaves your device — which matters more than usual for an ID picture." },
    ],
  },
  {
    slug: "exam-photo-resizer", category: "image", live: true,
    guide: { slug: "how-to-resize-a-photo-and-signature-for-an-exam-form", title: "How to Resize a Photo and Signature for an Exam Form" },
    title: "Exam Photo & Signature Resizer", h1: "Exam Photo and Signature Resizer",
    desc: "Resize a photo or signature to the exact pixels and KB an exam form demands — SSC, IBPS, SBI, NTA and UPSC presets. Free, in your browser.",
    faqs: [
      { q: "Which exams are covered?", a: "Presets for bank recruitment (IBPS, SBI, RBI), SSC one-time registration (CGL, CHSL, GD, MTS), NTA exams (JEE Main and NEET) and UPSC's OTR profile, for photographs, signatures and left thumb impressions. Every preset names where its numbers come from, and any size a form asks for can be typed in directly. Specs are restated in each notification and do change between cycles, so confirm yours against the official notification before uploading." },
      { q: "How does it hit a range like 20-50 KB and not just a maximum?", a: "Exam portals reject a file for being too small as well as too big, which ordinary compressors ignore. This one first paints your image at the exact pixel size required, then searches for the highest JPG quality whose encode still fits the ceiling — the largest acceptable file, and so the one most likely to clear the floor too. The result is checked against both ends of the range and the outcome is stated plainly." },
      { q: "What if the file comes out under the minimum size?", a: "That happens when a photo is soft or low in detail — there is simply not enough information to fill the bytes at the required dimensions. Because portals almost always publish the pixel size as 'preferred' while treating the KB range as a hard check, the tool can enlarge the image slightly, keeping the aspect ratio, until the file clears the floor. If it still falls short you are told so directly. Nothing is ever padded with filler bytes to fake the size." },
      { q: "Why does my signature scan look grey, and can that be fixed?", a: "Phone photos of paper pick up shadow and off-white tone, and forms that ask for a signature on white paper often reject the result. The background cleanup option maps everything lighter than an adjustable threshold to pure white, leaving the ink untouched. It also compresses much smaller, so a cleaned signature may need the enlarge option to reach a minimum size." },
      { q: "Is my photo uploaded anywhere?", a: "No. The image is read from your own device and every crop and re-encode happens in the page using your browser's canvas. Nothing is sent to a server, which matters for exactly this kind of file — an exam photo and a specimen signature are among the most sensitive things a form will ask you for." },
    ],
  },
  {
    ...
    desc: "Extract text from an image free. OCR a photo, screenshot or scan into editable text you can copy or download — English and Hindi, in your browser.",
    faqs: [
  ...
  },
  {
    ...
    desc: "See the hidden EXIF data in a photo — GPS, camera, serial number and date — then download a clean copy with it removed. Free, in your browser.",
    faqs: [
  ...
  {
    slug: "heic-to-jpg", category: "image", live: true,
    guide: { slug: "how-to-convert-heic-to-jpg", title: "How to Convert HEIC to JPG" },
    title: "HEIC to JPG", h1: "HEIC to JPG Converter",
    desc: "Convert HEIC photos from your iPhone to JPG, PNG or WebP. Batch convert at full resolution — free, in your browser, nothing uploaded.",
    faqs: [
      { q: "What is a HEIC file?", a: "HEIC is the format iPhones and iPads have saved photos in since iOS 11. It stores the picture with HEVC compression, which is roughly twice as efficient as JPEG — the same photo at about half the size. The catch is support: Windows needs a paid codec from the Microsoft Store, and plenty of websites, forms and older software still reject the files outright. Converting to JPG trades the space saving for something everything can open." },
      { q: "Can I convert several photos at once?", a: "Yes. Drop in as many as you like — a whole camera roll export is fine — and they are converted one after another, each with its own download button plus a Download all for the batch. They run in sequence rather than all at once because a decoded photo from a modern iPhone is around 50 MB of raw pixels, and doing a dozen in parallel is how a browser tab runs out of memory." },
      { q: "Why is the JPG bigger than the HEIC it came from?", a: "That is expected, and it is the honest cost of the conversion. HEIC's HEVC compression is far more efficient than JPEG's, so re-encoding the same picture as a JPG of comparable quality typically lands somewhere between one-and-a-half and two-and-a-half times the original size. Lowering the quality slider brings it back down; PNG goes the other way and will be several times larger, because it throws nothing away at all." },
      { q: "Will my photos come out sideways?", a: "No. HEIC files record rotation as a flag rather than by storing the pixels the right way up, which is why some converters hand back sideways photos. The decoder here applies that flag while decoding, so a portrait shot arrives as a portrait JPG, and the dimensions shown next to each result are the upright ones." },
      { q: "Are my photos uploaded anywhere?", a: "No. The decoder is a copy of libheif compiled to WebAssembly and served from this site; it runs inside your own browser, in a background worker, and your photos never leave your device. As a side effect of converting through a canvas the output also carries no EXIF at all — no GPS coordinates, no camera serial, no timestamp." },
    ],
  },
];
