...
export const TOOLS = [
  ...
  },
  {
    ...
    desc: "Extract tables from a PDF to Excel (XLSX) or CSV online for free. Finds each table separately across every page, previews them, and exports one sheet per table — all in your browser, no upload.",
    faqs: [
      { q: "How does it find the tables and columns?", a: "It reads the position of every piece of text and, where the table has drawn borders, uses those ruled lines to fix the exact rows and columns; otherwise it groups text into rows and finds columns from the vertical gaps. It detects each table on the page separately, so tables of different shapes aren't merged, and repeated page headers and footers are dropped." },
      { q: "What happens with several tables or multiple pages?", a: "Each detected table is previewed on its own and exported to its own sheet in the Excel file, so different structures stay separate. A table that continues across a page break is stitched back into one. You can also download any single table as CSV." },
      { q: "Should I download XLSX or CSV?", a: "Choose Excel (XLSX) to open straight in Excel, Google Sheets or Numbers with numbers kept as numbers and one sheet per table. Choose CSV for a plain, universal text file (one table at a time) — you can pick the delimiter: comma, semicolon or tab." },
