...
export const GUIDES = [
  ...
  // ---------------------------------------------------------------- UNLOCK-PDF
  {
    ...
    desc: "How to unlock a PDF for free: remove printing, copying and editing restrictions instantly, and remove an open password by entering it once. Learn the two kinds of PDF password.",
    category: "pdf",
    tool: { slug: "unlock-pdf", title: "Unlock PDF" },
    updated: "2026-08-10",
    body: [
      { t: "p", s: "You open a PDF, it displays perfectly, and then you try to print it - or copy a paragraph out of it - and the option is greyed out. The document is not asking for a password, it simply refuses to let you do anything with it. That is a restricted PDF. This guide explains the two very different kinds of password a PDF can carry, why one lifts in seconds without any password while the other genuinely needs the password you were given, how to remove both for free, and what to do when a password is lost." },
      ...
      { t: "p", s: "Telling them apart takes seconds. If the document opens straight to page one and only misbehaves when you print or select text, it carries an owner password with restriction flags. If you are stopped at a prompt before seeing anything, it carries a user password. The [Unlock PDF](/pdf/unlock-pdf) tool handles both: the first case is cleared automatically, and for the second it asks for the password once and saves a copy that never asks again. What no tool can do is open the second kind without the password." },
      ...
      { t: "ol", items: [
        ...
        "Drag the locked PDF onto the page, or click to select it.",
        "If the file needs a password to open, a prompt appears - enter the password once. A file that only carries printing or copying restrictions skips this step: it is decrypted with the empty user password every reader already knows.",
        "Check the file name and size that appear, then click Download unlocked PDF. The tool rebuilds the document without the encryption dictionary.",
        "Your browser saves the result as unlocked.pdf. Open it and confirm it no longer asks for a password, and that printing and text selection work.",
      ]},
      ...
      { t: "h2", s: "Removing an open password you know" },
      { t: "p", s: "A PDF that demands a password before it will display - a bank statement locked to your date of birth, a payslip locked to an employee number - is a different mechanism. The encryption key is derived from that password and the page content is genuinely encrypted with it. Enter the password when the tool prompts and it decrypts the file properly - AES-256 included - then saves a copy with no encryption dictionary at all. The copy opens instantly, forever, with no password and no restrictions, which is exactly what you want for a statement you file away monthly and are tired of unlocking." },
      { t: "p", s: "What no tool can do is open such a file without the password. There is no copy of the key stored in the file to find - without the password the bytes making up the pages are unreadable, not merely hidden behind a flag. Modern PDFs use AES-256 for this, the same class of encryption that protects disks and network traffic, and no amount of clever file handling gets around it. Any service promising to open a strongly protected file without its password is either guessing common passwords or misleading you." },
      ...
      { t: "p", s: "If a document opens fine but refuses to print or let you copy text - or asks for a password you're tired of typing - open the [Unlock PDF](/pdf/unlock-pdf) tool, drop the file in, and download an unlocked copy with its pages, fonts and text intact. It is free, runs entirely in your browser, and neither your document nor your password ever leaves your device." },
    ],
    faqs: [
      { q: "Can this open a PDF that asks for a password before it will display?", a: "Yes, if you know the password: enter it when the tool prompts and it decrypts the file - AES-256 included - and saves a copy with the password removed. Without the password it is impossible, for this tool or any other: the user password derives the key that actually encrypts the page content, so the pages are unreadable rather than merely hidden." },
  ...
  // ---------------------------------------------------------------- ROTATE-PDF
  {
    ...
    body: [
      ...
      { t: "p", s: "A PDF that demands a password before it will display is genuinely encrypted, so the tool asks for the password when you drop the file in - enter it once and rotation proceeds on an unlocked copy, with the password gone from the download. Without the password nothing can read the file. A file that opens normally but blocks printing or editing is a milder case: those restrictions are cleared automatically, and [Unlock PDF](/pdf/unlock-pdf) does the same if all you want is the unlocking." },
  ...
  // ------------------------------------------------------------- WATERMARK-PDF
  {
    ...
    body: [
      ...
      { t: "ul", items: [
        "The file asks for a password. A PDF that demands a password before it will even display is genuinely encrypted - enter its password when the tool prompts and watermarking proceeds on an unlocked copy.",
  ...
  // ---------------------------------------------------------------- ADD-PAGE-NUMBERS
  {
    ...
    body: [
      ...
      { t: "h2", s: "If something goes wrong" },
      { t: "ul", items: [
        "The file asks for a password. A PDF that demands a password before it will display is genuinely encrypted - enter its password when the tool prompts and numbering proceeds on an unlocked copy.",
  ...
  // ---------------------------------------------------------------- REMOVE-PAGES
  {
    ...
    body: [
      ...
      { t: "h2", s: "If something goes wrong" },
      { t: "ul", items: [
        "The file asks for a password. A PDF that demands a password before it displays is genuinely encrypted - enter its password when the tool prompts and trimming proceeds on an unlocked copy. [Unlock PDF](/pdf/unlock-pdf) is the dedicated page when the unlocking itself is all you need.",
  ...
  },
  {
    ...
    h1: "How to Remove EXIF Data from Photos",
    desc: "What EXIF data records, why the GPS tag matters most, and how to read and strip metadata from a JPG, PNG or WebP in your browser without re-encoding the picture or losing any quality.",
    category: "image",
    tool: { slug: "exif-viewer", title: "EXIF Viewer & Remover" },
    updated: "2026-08-04",
    body: [
      { t: "p", s: "Every photo you take is two things in one file: the picture, and a block of information your camera wrote beside it - when the shutter opened, which camera and lens took the shot, and, if location services were on, where you were standing. That block travels with the file. Email the photo, put it in a marketplace listing or send it as a document rather than an image, and it goes too. This guide covers what is in there, how to read it, and how to strip it without touching a single pixel, using the [EXIF Viewer & Remover](/image/exif-viewer)." },

      { t: "h2", s: "What EXIF actually records" },
      { t: "p", s: "EXIF (Exchangeable Image File Format) is the standard cameras and phones use to write technical information inside an image file. It is not the only such block - editing software adds XMP, and news and stock workflows add IPTC for captions, credits and keywords - but it holds the most. A typical phone photo carries some mixture of:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Most of that is harmless and genuinely useful. Two parts are not: the location, and anything identifying a specific device or person." },

      { t: "h2", s: "Why the GPS tag is the one that matters" },
      { t: "p", s: "A camera serial number links photos to each other - post enough shots and they are provably from the same body. That is a slow, indirect problem. Coordinates are an immediate one, because they are precise enough to be an address." },
  ...
  // ---------------------------------------------------------------- QR CODES
  {
    ...
    h1: "Do QR Codes Expire? Static vs Dynamic QR Codes Explained",
    desc: "Do QR codes expire? The pattern itself never does — but the redirect behind a dynamic QR code can be switched off. How to tell which kind you have, and how to make one that lasts.",
    category: "dev",
    tool: { slug: "qr-code-generator", title: "QR Code Generator" },
    updated: "2026-08-05",
    body: [
      { t: "p", s: "This question almost always arrives too late. Someone made a QR code months ago, printed it on a menu or five thousand flyers, and now it lands on a page asking them to start a subscription. The code looks fine. The camera reads it fine. It just does not go where it did." },
      { t: "p", s: "QR codes do not rot. But a lot of the ones being handed out do not point at your content at all - they point at somebody else's redirect service, and that service is the part with an expiry date." },

      { t: "h2", s: "The short answer" },
      { t: "p", s: "A QR code is a pattern of black and white squares that spells out a string of text. There is no timestamp inside it, no serial number, nothing that phones home. A code printed in 1994, when Denso Wave invented the format, would still scan on a phone made this year. The pattern is frozen the moment it is generated." },
      { t: "p", s: "So the code cannot expire. What can expire is whatever sits between it and the thing you wanted people to reach." },

      { t: "h2", s: "Static and dynamic QR codes" },
      { t: "p", s: "Every QR code you will meet is one of two kinds, and the difference is entirely about what got encoded." },
      { t: "ul", items: [
      ...
      { t: "p", s: "That middle step buys something real - you can change the destination after printing, and you get scan counts. It also means your poster depends on a third party staying in business and on your card not being declined." },

      { t: "h2", s: "You cannot tell them apart by looking" },
      { t: "p", s: "This is the trap. A QR code's size comes from how many bytes it carries, not from what kind it is. At the default error correction level, https://zenoply.com is 19 bytes and produces a 25x25 grid - QR version 2. A typical dynamic short link, https://qr.example/aB3xK9, is 25 bytes and produces the same 25x25 grid, also version 2. Identical dimensions, completely different life expectancies." },
      { t: "p", s: "One test settles it in five seconds: point your phone at the code and read the URL your camera previews before you tap. Your own domain means the code is static and yours forever. A domain you have never heard of means you are renting. The [QR Code Generator](/dev/qr-code-generator) also prints the exact string it is about to encode in an 'Encoded data' box under the preview, so you can check before you download anything." },

      { t: "h2", s: "What actually breaks a QR code" },
      { t: "p", s: "In practice, dead codes come from four causes:" },
      { t: "ul", items: [
      ...
      { t: "h2", s: "Making one that outlives you" },
      { t: "p", s: "A static code has nothing to expire, because no service is involved - the encoding happens on your device and the PNG or SVG that comes out is just an image." },
      { t: "ol", items: [
        "Open the [QR Code Generator](/dev/qr-code-generator) and choose what the code should do: a link, Wi-Fi details, a UPI payment request, an email, an SMS or a phone number.",
      ...
      { t: "p", s: "The one real limitation is the flip side of that strength: the destination is set in ink. Point it at a short path on your own site that you can quietly redirect later, and you get a dynamic code's flexibility with none of its dependence on someone else." },

      { t: "h2", s: "Error correction: how much damage it survives" },
      { t: "p", s: "QR codes carry redundant data so they still read when part of the pattern is missing. Four levels exist, and the trade never changes - more resilience means more squares in the same space:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "The cost is visible. This tool's own URL is 41 bytes: at level M it needs a 29x29 grid, at level H it needs 37x37. Denser pattern, so each square is smaller at a given print size - cranking error correction to H on a code you are printing tiny can make it harder to scan, not easier." },
      { t: "p", s: "The quiet zone is not optional: scanners use the blank margin around the pattern to find where the code begins, and the standard asks for four modules' worth. Designers trim it constantly. The generator defaults to four and warns if you go below." },

      { t: "h2", s: "Printing it big enough" },
      { t: "p", s: "The common rule of thumb: a code should be at least a tenth as wide as the distance it will be scanned from. A menu code read at 30 cm needs about 3 cm. A poster read from 2 m needs about 20 cm - far larger than most people assume." },
      { t: "p", s: "Watch the individual squares too. That 29x29 code printed 3 cm wide gives each module just over 1 mm, which any phone handles. Shrink it to 1.5 cm and each module is half a millimetre - about where cheap printing and camera shake start costing you scans. Download the SVG for print: it stays crisp at any size, while a small PNG blown up to poster scale goes soft exactly where sharpness matters." },

      { t: "h2", s: "So when is dynamic worth it?" },
      { t: "p", s: "When you genuinely cannot know the destination in advance - a campaign whose landing page is not built yet - or when scan analytics are worth paying for. That is a fair trade. Just go in knowing you are buying a redirect subscription, not a better QR code, and that everything you print inherits its lifespan." },
      { t: "p", s: "For most other uses - Wi-Fi on a guest room card, a UPI code taped by the till, a link on a business card - static is simply better. Make one with the [QR Code Generator](/dev/qr-code-generator): it runs entirely in your browser, so your link, Wi-Fi password or UPI ID never leaves your device, and the image has no account behind it, no scan limit and nothing to renew." },
    ],
    faqs: [
      { q: "Do QR codes have a scan limit?", a: "The code itself has no counter and no concept of how many times it has been read - it is a static image, exactly like a barcode on a tin. Scan limits come from dynamic QR services, where each scan is a request to the provider's redirect server and the plan caps how many of those you get per month. Cross the cap and the redirect stops resolving, which looks identical to the code breaking. A static code generated in your browser encodes your destination directly, so there is no server in the middle to meter anything and no limit to hit." },
      { q: "Can I change where a QR code points after it is printed?", a: "Not by editing the code - the destination is part of the pattern, so changing it means generating a new one. There is a way to get the same flexibility without a subscription, though: point the code at a URL on your own domain, such as yoursite.com/menu, and set up a redirect from that path to wherever the content actually lives. You can change that redirect whenever you like, and every code already printed follows it. The difference from a dynamic QR service is that the redirect is yours, so nobody can switch it off or start charging for it." },
      { q: "How long does a printed QR code last?", a: "As long as the physical copy stays legible and the destination stays live. The pattern has no shelf life, so the practical limits are ink and paper: sun-bleached window stickers, scuffed labels and rain-damaged signage all fail eventually. Error correction buys real margin here - level Q or H keeps a code readable with roughly a quarter to a third of it damaged - and printing generously sized helps more than anything else. Laminating outdoor codes and re-checking them with your own phone once or twice a year catches problems while they are still cheap to fix." },
    ],
  },
  ...
  // ------------------------------------------------------------------- GRADES
  {
    ...
    h1: "How to Convert CGPA to Percentage (CBSE 9.5, 10-Point and VTU Rules)",
    desc: "How to convert CGPA to percentage: where the CBSE 9.5 multiplier comes from, why your university's number differs, how to work out CGPA from grade points and credits, and a full worked example.",
    category: "convert",
    tool: { slug: "cgpa-to-percentage", title: "CGPA to Percentage Calculator" },
    updated: "2026-08-06",
    body: [
      { t: "p", s: "A CGPA of 8.47 is a perfectly clear number right up until a form asks for a percentage. Then you discover that your board, your university and the employer reading the application may each want a different figure from the same grade card, and none of them will tell you which rule they used." },
      { t: "p", s: "There is no single national conversion - only a small set of published rules, each belonging to a specific institution, plus a lot of folklore filling the gaps. This guide covers the rules that are actually written down, how a CGPA is built from grade points and credits, and what to do when yours is not on the list." },

      { t: "h2", s: "What a CGPA actually measures" },
      { t: "p", s: "CGPA stands for cumulative grade point average. Each subject earns a grade point - on the common Indian 10-point scale, O is 10, A+ is 9, A is 8 and so on down - and each subject carries a credit weight reflecting its teaching hours. The CGPA is the average of those grade points weighted by credits:" },
      { t: "p", s: "CGPA = sum of (grade point x credits) / sum of credits" },
      { t: "p", s: "The weighting is the part people skip, and it is the part that matters. A four-credit theory paper pulls roughly twice as hard as a two-credit lab, so a plain average of your grade points will not match what the university prints. SGPA is the same arithmetic over one semester; CGPA is the same arithmetic again, over every subject you have taken or over your semester SGPAs weighted by their credits. One formula, three names." },

      { t: "h2", s: "A worked example, end to end" },
      { t: "p", s: "Take a semester of five subjects:" },
      { t: "ul", items: [
        "Subject 1 - grade point 9, 4 credits, so 36 weighted points",
        "Subject 2 - grade point 8, 4 credits, so 32 weighted points",
        "Subject 3 - grade point 7, 3 credits, so 21 weighted points",
        "Subject 4 - grade point 9, 2 credits, so 18 weighted points",
        "Subject 5 - grade point 10, 2 credits, so 20 weighted points",
      ]},
      { t: "p", s: "The weighted points add up to 127 and the credits to 15, so the CGPA is 127 / 15 = 8.4667, printed as 8.47. An unweighted average of those grade points would have given 8.60 instead." },
      { t: "p", s: "Now convert that 8.4667 to a percentage under each of the three published rules, and watch them disagree:" },
      { t: "ul", items: [
        "CBSE / CISCE, multiply by 9.5: 80.43%",
        "Standard 10-point, multiply by 10: 84.67%",
        "VTU, subtract 0.75 then multiply by 10: 77.17%",
      ]},
      { t: "p", s: "Same grade card, a spread of more than seven percentage points. This is why \"what is my percentage\" has no answer until you know whose rule applies. The [CGPA to Percentage Calculator](/convert/cgpa-to-percentage) does all three, and its Grades to CGPA tab does the credit-weighted step above so you can start from your grade card rather than from a CGPA you are not sure about." },

      { t: "h2", s: "Where the CBSE 9.5 multiplier comes from" },
      { t: "p", s: "The 9.5 is not arbitrary, and it is not a rounding of 10. CBSE looked at its own results across several years and found that students landing in the top grade band, A1, which covers 91 to 100 marks, averaged about 95 marks. The grade point for A1 is 10. Divide 95 by 10 and you get 9.5, the factor that maps the grade scale back onto the mark scale it was derived from." },
      { t: "p", s: "Two consequences follow. First, a perfect 10.0 CGPA converts to 95%, not 100% - the ceiling is built into the rule. Second, the same multiplier works subject by subject, which is what a form asking for marks in one paper usually wants: a grade point of 8 corresponds to roughly 76% in that subject." },

      { t: "h2", s: "Why your university's number is different" },
      { t: "p", s: "Beyond school boards, conversion practice fragments:" },
      { t: "ul", items: [
        "Many 10-point universities and autonomous colleges simply multiply by 10, treating the CGPA as a percentage divided by ten.",
        "VTU applies a 0.75 offset first, so percentage = (CGPA - 0.75) x 10. A 10.0 tops out at 92.5%, and any CGPA below 0.75 produces a negative number, which is the formula telling you it does not apply.",
        "Some universities publish a banded table instead of a formula, so no single multiplier reproduces their output exactly.",
        "Others refuse to convert at all and will issue an equivalence certificate on request. If your university does this, that certificate is the number to use - it outranks any formula.",
      ]},
      { t: "p", s: "If your institution is not one of the three built in, do not guess a multiplier that looks close. Find the rule in your academic regulations or on the back of your marksheet, then enter it under Custom multiplier in the tool. On a 4-point CGPA scale rather than a 10-point one, the equivalent multiplier is 25, since 4 x 25 = 100." },

      { t: "h2", s: "Converting a percentage back to CGPA" },
      { t: "p", s: "Every rule runs backwards. Divide by 9.5 for CBSE, so 77.9% gives 8.20. Divide by 10 for the standard scale. For VTU, divide by 10 and add the 0.75 back, so 74.5% gives 8.20 as well. The Percentage to CGPA tab does this directly." },
      { t: "p", s: "One caution: a reverse conversion is only valid with the rule that produced the percentage, and running it through the wrong rule gives a confident wrong answer. Because the CBSE rule caps at 95%, anything above that yields a CGPA over 10 - the tool flags this rather than quietly printing an impossible grade." },

      { t: "h2", s: "Rounding, and the case of the missing 0.04" },
      { t: "p", s: "This is the discrepancy that sends people looking for a second calculator. Our example CGPA is 8.46667. Round it to 8.47 first and then multiply by 9.5 and you get 80.47%. Multiply the unrounded 8.46667 by 9.5 and you get 80.43%. Both are arithmetically correct; they differ because one of them rounded twice." },
      { t: "p", s: "Round once, at the very end: carry full precision through the calculation and round only the figure you write down. The calculator works this way, which is why a CGPA it displays as 8.47 converts to 80.43% rather than 80.47%. If your university's published percentage differs from yours by a few hundredths, double rounding is almost always the reason." },

      { t: "h2", s: "The 4.0-scale question" },
      { t: "p", s: "Applying abroad brings up the American 4.0 GPA. The tool shows a 4.0-scale figure - CGPA divided by 10, times 4 - deliberately labelled as reference only, because credential evaluators such as WES and individual admissions offices apply their own grade-band tables that are rarely linear. Send them your official transcript and use whatever their conversion produces; never put a self-computed 4.0 GPA on an application as if it were official." },

      { t: "h2", s: "Which number to write on the form" },
      { t: "p", s: "In order of authority: a percentage printed on your marksheet, then an equivalence certificate from your university, then the formula in your academic regulations, and only then a general rule. Whichever you use, say which one - \"78.0% (CGPA 8.2 x 9.5, CBSE conversion)\" is checkable, whereas a bare number invites the reader to redo it under a different rule and find a discrepancy." },
      { t: "p", s: "Work it out in the [CGPA to Percentage Calculator](/convert/cgpa-to-percentage): pick your rule and it prints the formula alongside the result, so you can copy both onto the form. It runs entirely in your browser, so your grades never leave your device." },
    ],
    faqs: [
      { q: "Is the CGPA x 9.5 formula valid for engineering or university marks?", a: "Only if your university says so. The 9.5 multiplier was derived by CBSE from its own school results - the average marks of students in the 91-100 grade band - so it describes the CBSE grading scheme and nothing else. Plenty of universities do use a x 9.5 or x 10 rule, but plenty use something else: VTU subtracts 0.75 before scaling, and several publish banded tables that no single multiplier reproduces. Check your academic regulations or the reverse of your marksheet, and if the rule there is not one of the built-in options, enter its multiplier under Custom multiplier." },
      { q: "Can I convert SGPA to percentage the same way?", a: "Yes - SGPA and CGPA sit on the same grade-point scale, so the same conversion rule applies to both. SGPA is just the credit-weighted average over one semester rather than over your whole programme, and the tool does not care which one you type in. The related question, converting several SGPAs into an overall CGPA, is also the same arithmetic: use the Grades to CGPA tab with one row per semester, putting that semester's SGPA in the grade point column and its total credits in the credits column. Semesters carrying more credits then pull proportionally harder, which is exactly how universities compute it." },
      { q: "My form needs a percentage but my university only issues a CGPA. What should I do?", a: "Ask your examination or controller's office for an equivalence certificate first - many universities issue one on request, and a certificate settles the question in a way no formula can. While you wait, use the conversion rule printed in your academic regulations and state it explicitly next to the number you write, for example \"84.7% (CGPA 8.47 x 10 per university regulations)\". Showing your working is what protects you: if the reader applies a different rule and gets a different figure, the discrepancy reads as two conventions rather than as a misstated mark." },
    ],
  },

  // ---------------------------------------------------------------- RUPEES-IN-WORDS
  {
    slug: "how-to-write-rupees-in-words-on-a-cheque",
    title: "How to Write Rupees in Words on a Cheque",
    h1: "How to Write Rupees in Words on a Cheque (India)",
    desc: "How to write the amount in words on an Indian cheque: lakh and crore grouping, writing paise, the word Only, hyphens and capitals, and the mistakes that get a cheque returned.",
    category: "convert",
    tool: { slug: "rupees-in-words", title: "Rupees in Words" },
    updated: "2026-08-07",
    body: [
      { t: "p", s: "The amount in words is the part of a cheque that decides what gets paid. Section 18 of the Negotiable Instruments Act, 1881 is explicit: where a sum is stated differently in figures and in words, the amount in words is the amount payable. A bank rarely has to invoke that rule, because a cheque whose two amounts disagree usually comes back unpaid instead - and under the Cheque Truncation System you cannot strike the line through and correct it. This guide covers the Indian conventions: lakh and crore grouping, how paise are written, the word Only, and the mistakes that actually get cheques returned. The [Rupees in Words](/convert/rupees-in-words) converter prints the finished line if you would rather check yours against it." },

      { t: "h2", s: "The two amounts on every cheque" },
      { t: "p", s: "A cheque states its value twice, and the clearing system names each. The digits in the boxed field are the courtesy amount; the words on the pre-printed \"Rupees\" line are the legal amount. The redundancy is deliberate: digits can be altered with one stroke - a 1 becomes a 4, a comma shifts a figure tenfold - whereas a line of words closed off with \"Only\" cannot be extended without the tampering being obvious." },
      { t: "p", s: "The word order is fixed for you by the form itself. Because the cheque already prints \"Rupees\" at the start of the line, you begin with the number and never repeat the currency at the end - otherwise the finished cheque reads \"Rupees One Lakh ... Rupees Only\"." },

      { t: "h2", s: "A worked example: 1,23,456.50" },
      { t: "p", s: "Five steps take a payment of 1,23,456.50 from figures to a finished line:" },
      { t: "ol", items: [
        "Split at the decimal point: 1,23,456 rupees, and 0.50 which is fifty paise.",
        "Group the rupees the Indian way - the last three digits, then twos above that: 1 | 23 | 456. Those groups are read as one lakh, twenty-three thousand, four hundred fifty-six.",
        "Write the rupee words in Title Case, hyphenating compounds from twenty-one to ninety-nine: One Lakh Twenty-Three Thousand Four Hundred Fifty-Six.",
        "Add the paise as a whole number of paise, never as digits after \"point\": and Fifty Paise.",
        "Close with Only, then rule a line through whatever space is left on the printed line.",
      ]},
      { t: "p", s: "The finished cheque reads 1,23,456.50 in the figures box and Rupees One Lakh Twenty-Three Thousand Four Hundred Fifty-Six and Fifty Paise Only on the words line. A round amount is shorter: 45,250 with no paise is simply Forty-Five Thousand Two Hundred Fifty Only. Do not pad it with \"and Zero Paise\" - the convention is to leave the phrase out entirely when there are none." },

      { t: "h2", s: "Writing the paise" },
      { t: "p", s: "This is where most errors creep in, and they are errors of place value rather than of spelling. A rupee is a hundred paise, so the fraction always has two decimal places, and .5 is not five of anything - it is fifty paise." },
      { t: "ul", items: [
        "0.50 is fifty paise: ... and Fifty Paise Only.",
        "0.05 is five paise: ... and Five Paise Only. If you meant five, the figure must be .05, not .5.",
        "No paise at all: leave the phrase out and go straight to Only.",
        "A rupee amount cannot carry more than two decimals, so round to the nearest paisa before you write anything - 99.999 is 100.00, and the line reads One Hundred Only.",
      ]},
      { t: "p", s: "Note the singular and plural: one paisa, fifty paise. Coins of 25 paise and below were withdrawn from circulation in 2011, but paise remain entirely valid on a cheque, in an invoice and in a ledger - interest and tax calculations produce them constantly." },

      { t: "h2", s: "Lakh and crore, not million" },
      { t: "p", s: "The Indian numbering system does not group in threes. Above the thousands it counts in lakh (1,00,000) and crore (1,00,00,000), and the commas follow the same rule: the last three digits, then a comma every two digits. So 12,34,567 is twelve lakh thirty-four thousand five hundred sixty-seven, and the same figure written for an international reader would be 1,234,567 - one million two hundred thirty-four thousand five hundred sixty-seven. Ten lakh makes a million; a crore is ten million." },
      { t: "p", s: "Above a crore, the system reuses its own words instead of inventing new ones. 1,00,00,00,000 is one hundred crore, 10,00,00,00,000 is one thousand crore, and a trillion rupees is one lakh crore - there is no separate word waiting at 10^9 the way \"billion\" arrives in the short scale, which is why converting between the two systems by eye goes wrong so often. The [Rupees in Words](/convert/rupees-in-words) converter has a toggle for this: leave it on Indian for a domestic cheque, switch it to international when \"crore\" would not land with the reader." },

      { t: "h2", s: "What actually gets a cheque returned" },
      { t: "p", s: "Very little of the fussiness around cheque wording is legal requirement; most of it is defensive habit built around the ways cheques are refused. The ones that matter:" },
      { t: "ul", items: [
        "The words and the figures disagree. Legally the words win, but the practical outcome is a returned cheque and a fee, so treat the two amounts as one thing to be checked together.",
        "Any correction or alteration on the cheque. Under the Cheque Truncation System, banks do not accept cheques carrying changes other than date validation - no overwriting, no initialled corrections. If you make a mistake in the amount, write a fresh cheque.",
        "Space left after the words. An open gap invites an addition; the trailing line through the remainder, plus the word Only, is what closes it off.",
        "A stale date. A cheque is valid for three months from the date written on it, after which no wording will get it paid.",
      ]},

      { t: "h2", s: "Hyphens, capitals and the word Only" },
      { t: "p", s: "The remaining details are conventions rather than rules, and knowing which is which saves you worrying about the wrong ones." },
      { t: "ul", items: [
        "Hyphens. Compound numbers from twenty-one to ninety-nine are hyphenated in standard written English - Twenty-Three, Forty-Five, Ninety-Nine. Larger numbers are not: it is Four Hundred Fifty-Six, never Four-Hundred-Fifty-Six.",
        "The word \"and\". Indian English commonly inserts it inside a hundreds number - One Hundred and Five - while American style drops it. No bank rejects a cheque over it, but reserving \"and\" for its one load-bearing use, separating the rupees from the paise, keeps the line unambiguous.",
        "Capitals. Title Case is the norm and block capitals are equally acceptable; what matters is that the line is legible and unbroken.",
        "Only. Universal on Indian cheques and worth keeping even though no statute demands it - it is the terminator that makes an addition to the line visible.",
      ]},

      { t: "h2", s: "Write your amount now" },
      { t: "p", s: "Type the figure into the [Rupees in Words](/convert/rupees-in-words) converter and it returns three wordings at once: the cheque line with its leading Rupees and closing Only, a lower-case version for invoices and contracts, and the bare number in words. It accepts the rupee symbol, \"Rs.\" and commas in either grouping style, and rounds anything beyond two decimals to the nearest paisa. Everything runs in your browser, so the amount never leaves your device." },
      { t: "p", s: "For amounts without a currency attached, the [Number to Words](/convert/number-to-words) converter handles plain short-scale English. If the figure itself is still being worked out, the [EMI Calculator](/finance/emi-calculator) and the [GST Calculator](/finance/gst-calculator) produce the amounts that most often end up on a cheque." },
    ],
    faqs: [
      { q: "How do you write 1,23,456.50 in words on a cheque?", a: "One Lakh Twenty-Three Thousand Four Hundred Fifty-Six and Fifty Paise Only. Because the cheque already prints \"Rupees\" at the start of the line you do not write the currency yourself, and because .50 means fifty paise it is written as a whole number of paise rather than as \"point five zero\". Finish with Only and rule a line through the leftover space so nothing can be appended. If there were no paise, the line would simply end after Fifty-Six with Only - the \"and Zero Paise\" phrasing is not used." },
      { q: "Is .50 fifty paise or five paise?", a: "Fifty. A rupee is a hundred paise, so the fractional part always has two decimal places: .50 is fifty paise, and five paise is written .05. This is the single most common mistake in cheque and invoice wording, and it is a factor-of-ten error rather than a spelling one, so it survives proofreading easily. The converter shows the paise as a separate whole number alongside the words, which makes a slip of this kind obvious at a glance." },
      { q: "What happens if the amount in words and the amount in figures do not match?", a: "Section 18 of the Negotiable Instruments Act, 1881 provides that the amount stated in words is the amount payable, so in law the words govern. In practice the bank is far more likely to return the cheque marked that the amount in words and figures differ than to pay the sum in words, which costs you a return charge and the time to issue a fresh cheque. You also cannot fix it on the spot: Cheque Truncation System rules mean banks do not accept cheques bearing corrections other than date validation, so a mismatched cheque has to be replaced rather than amended." },
    ],
  },
  ...
  // ---------------------------------------------------------------- TOKENS
  {
    ...
    title: "What Is a Token in an LLM?",
    h1: "What Is a Token in an LLM? How GPT and Claude Count Text",
    desc: "What is a token in an LLM? How tokens differ from words, why the same sentence costs more in Hindi than English, which encoding each GPT model uses, and how to count tokens exactly.",
    category: "dev",
    tool: { slug: "token-counter", title: "Token Counter" },
    updated: "2026-08-09",
    body: [
      { t: "p", s: "Every price list, rate limit and context window published by an AI company is denominated in tokens, and almost nobody can look at a paragraph and say how many tokens it is. The unit is precisely defined - it is just not one humans think in. This guide covers what a token is, why the count never matches your word count, why the same sentence costs three times more in one language than another, and how to get an exact number." },

      { t: "h2", s: "What a token actually is" },
      { t: "p", s: "A language model reads neither characters nor words. Your text is first chopped into tokens - fragments drawn from a fixed vocabulary the model was trained with - and each fragment is swapped for the integer that identifies it. The model only ever sees that list of integers." },
      { t: "p", s: "The vocabulary is built by byte pair encoding: starting from raw bytes, the algorithm repeatedly merges the most frequent adjacent pair until it has as many entries as the designers wanted - roughly 100,000 for GPT-4, 200,000 for GPT-4o and GPT-5. Because the merges follow frequency, common sequences become single entries and rare ones stay broken apart, which explains nearly every surprise in a token count." },

      { t: "h2", s: "Why the token count never matches the word count" },
      { t: "p", s: "Common words are one token each, including the space in front of them: in GPT-4o's vocabulary the four characters of \" the\" are a single token, and so are \" The\" and \" and\". Rare words are not so lucky. \"Antidisestablishmentarianism\" is one word and 28 characters, but six tokens - \" ant\" + \"idis\" + \"est\" + \"ablishment\" + \"arian\" + \"ism\". Brand names fare badly too: \" Zenoply\" costs three, because the model has never seen it often enough to earn an entry of its own." },
      { t: "p", s: "This is why a [word counter](/text/word-counter) cannot substitute for a token counter. For plain English prose the ratio is fairly stable at roughly 1.1 to 1.3 tokens per word, so you can estimate in your head. The moment your text contains code, JSON, identifiers or unusual names, that breaks down: a small JSON object of 84 characters comes to 25 tokens." },

      { t: "h2", s: "A worked example, token by token" },
      { t: "p", s: "The [Token Counter](/dev/token-counter) ships with a sample that mixes the awkward cases on purpose: an English sentence, a paragraph containing a deliberately rare word, a line of JavaScript, and a sentence of Japanese. It runs to 266 characters and 39 words." },
      { t: "p", s: "Under o200k_base - the encoding GPT-5, GPT-4.1 and GPT-4o use - that is 84 tokens, or 3.17 characters per token. Switch the dropdown to cl100k_base, which is what GPT-4 and GPT-3.5 Turbo use, and the same text becomes 88. Nothing changed except the vocabulary; the newer one has twice as many entries, so it finds longer matches and needs fewer of them." },
      { t: "p", s: "The breakdown shades every token separately, so you can see where the splits fall - product names, UUIDs, timestamps and indented JSON are the usual offenders." },

      { t: "h2", s: "Non-English text costs more tokens" },
      { t: "p", s: "The training data behind these vocabularies is overwhelmingly English, so English gets the efficient merges and everything else is assembled from smaller pieces. One instruction - \"Please summarise this document in three short bullet points.\" - and its translations, under o200k_base:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "The counter is keyed on the encoding rather than the model name, deliberately: line-ups get renamed and replaced every few months, but the encoding a family uses is fixed for its lifetime. Pick the encoding and the answer survives the next rename." },

      { t: "h2", s: "Why Claude and Gemini counts are estimates, not facts" },
      { t: "p", s: "Anthropic and Google do not publish tokenizer files you can run in a browser, so any tool claiming an exact offline count for Claude or Gemini is guessing. The honest fallback is the rule of thumb their own documentation uses - about four characters per token for English - and that is what the counter shows for those models, labelled as an estimate rather than dressed up as a measurement." },
      { t: "p", s: "For English prose that rule is usually within 10 to 20 per cent; for code, JSON or any non-Latin script it can be badly wrong, for exactly the reasons above. When the number has to be right, both vendors expose a counting endpoint - Anthropic's /v1/messages/count_tokens and the Gemini API's countTokens - that returns the real figure for a specific model. Use the estimate to size a prompt; use the API to reconcile a bill." },

      { t: "h2", s: "Context windows have to hold the reply too" },
      { t: "p", s: "A context window is the total number of tokens a model can consider at once, and the common mistake is treating it as a budget for the prompt alone. It has to hold your system prompt, the conversation so far, any documents you pasted in, and the model's answer. Fill 127,000 tokens of a 128,000-token window and the model has room for about a sentence before it is cut off. The meter shows what fraction of a window your text fills and how many tokens remain, switching to a hatched bar once you go over; the presets run from 4,096 up to a million." },

      { t: "h2", s: "Turning tokens into money" },
      { t: "p", s: "Providers quote a price per million tokens, input and output at different rates. Because those prices change constantly, the counter takes the rate as an input rather than shipping a table that would go stale." },
      { t: "p", s: "A worked example. A 4,000-word English document runs to roughly 4,400 tokens at 1.1 tokens per word. At $3 per million input tokens that is about $0.013 to send once - trivially cheap. Send it on every request in a pipeline running a hundred times a day and it becomes about $1.33 a day, or $485 a year, for one document you never cached. Tokens only matter in aggregate, which is why nobody notices them until the invoice." },

      { t: "h2", s: "Count your own text" },
      { t: "p", s: "Paste your prompt into the [Token Counter](/dev/token-counter) for the exact figure under the OpenAI encodings, an honest estimate for the rest, the per-token breakdown, the share of a context window it fills and - if you enter a rate - what it costs. The encoding tables download to your browser and the tokenizing runs there, so a prompt you are still drafting never leaves your device." },
    ],
    faqs: [
      { q: "How many words is 1,000 tokens?", a: "About 750 to 900 words of plain English prose, because English runs at roughly 1.1 to 1.3 tokens per word. Treat that as a planning figure and nothing more. The same 1,000 tokens might be only 400 words of dense technical writing full of product names and identifiers, around 250 lines of nothing but JSON punctuation, or a far shorter passage in Hindi or Japanese. Whenever the number actually matters - a context limit you are close to, or a cost you are forecasting - measure the real text rather than converting from a word count." },
      { q: "Why is my token count different from another site's?", a: "Almost always because the two tools are using different encodings. The same sentence is 84 tokens under o200k_base and 88 under cl100k_base, so a site pinned to GPT-4's tokenizer will disagree with one set to GPT-4o. The other common cause is that the other site is estimating rather than tokenizing - a characters-divided-by-four figure presented without a caveat looks identical to a real count on screen. Check which encoding each tool names; if it does not name one, it is probably guessing." },
      { q: "Do tokens include spaces and punctuation?", a: "Yes, and the way they are grouped surprises people. A leading space is usually absorbed into the word that follows, so \" the\" is one token rather than a space plus a word - which is why splitting text on spaces and counting the pieces gives the wrong answer. Punctuation is often merged into runs: in JSON, a sequence such as a quote, comma and brace together can be a single token. Line breaks cost tokens too, so heavily indented code and pretty-printed JSON are measurably more expensive to send than their minified equivalents." },
    ],
  },
  {
    slug: "how-to-compress-an-image-to-a-specific-size",
    title: "How to Compress an Image to a Specific Size in KB",
    h1: "How to Compress an Image to a Specific Size in KB",
    desc: "How to compress a photo to exactly 20 KB, 50 KB or 100 KB for an exam or job application form - what the limit means in bytes, why quality alone is not always enough, and how to hit the number without wrecking the picture.",
    category: "image",
    tool: { slug: "compress-image-to-size", title: "Compress Image to Size (KB)" },
    updated: "2026-08-10",
    body: [
      { t: "p", s: "The form says the photograph must not exceed 50 KB. The photograph on your phone is 4 MB - eighty times too big. Upload portals enforce these limits strictly and almost none of them offer a way to meet one, so the job falls to you. The usual advice, \"just compress it\", skips the part that decides the outcome: a fixed byte budget can be spent well or badly, and the difference is whether the person at the other end sees a face or a grid of grey squares." },

      { t: "h2", s: "What the limit actually means in bytes" },
      { t: "p", s: "A kilobyte is almost always 1,024 bytes rather than 1,000, so a 50 KB cap means 51,200 bytes. That is the figure Windows, macOS and most upload validators use, and it is the one worth working to." },
      { t: "p", s: "A minority of portals do their arithmetic in round decimal thousands, making the same 50 KB limit 50,000 bytes - 1,200 bytes tighter, a difference of about 2.4 per cent. It is small, but it is exactly the margin a tool aiming to land just under 51,200 will occupy. If a file that looks comfortably inside the limit is rejected anyway, this is the usual reason: set the target a couple of kilobytes lower and try again." },

      { t: "h2", s: "The two dials that change a file's size" },
      { t: "p", s: "Only two things determine how large a JPG comes out, and understanding the split is most of the battle." },
      { t: "ul", items: [
        "Quality - how aggressively the encoder discards detail. JPEG works on 8x8 blocks of pixels; lowering the quality rounds off more of the fine variation inside each block. Push it far enough and the blocks stop blending and become visible as squares.",
        "Pixel count - how many pixels there are to encode in the first place. This one multiplies everything: halving both width and height quarters the number of pixels, and roughly quarters the file.",
      ]},
      { t: "p", s: "PNG has neither dial in a useful form. It is lossless, so there is no quality setting to turn - the only way to shrink a PNG is to remove pixels or reduce the number of colours. That is why every tool that hits a byte target writes JPG or WebP instead, and why a PNG you feed in comes back as one of those two." },

      { t: "h2", s: "A worked example: spending a 50 KB budget" },
      { t: "p", s: "Convert the target into a per-pixel allowance and the whole problem becomes obvious. 50 KB is 51,200 bytes, or 409,600 bits. How far that stretches depends entirely on how many pixels have to share it." },
      { t: "p", s: "Take a 640x480 photo - 307,200 pixels. Each pixel gets 409,600 / 307,200, about 1.33 bits. Uncompressed colour costs 24 bits per pixel, so this asks JPEG for roughly an 18:1 reduction. That is unremarkable; JPEG holds up well down to about 1 to 2 bits per pixel, and the result will look like a normal, slightly soft photograph." },
      { t: "p", s: "Now hand the same 50 KB to a 4000x3000 phone photo - 12,000,000 pixels. Each pixel now gets 409,600 / 12,000,000, about 0.034 bits: a 700:1 reduction. The encoder will still produce a file, because there is always some quality setting low enough. It will simply look terrible, because there is not enough information left to describe twelve million pixels." },
      { t: "p", s: "That comparison is the whole argument for resizing. When a target sits far below what your image's dimensions can carry, cutting the pixel count is not a compromise on the way to the answer - it is the answer." },

      { t: "h2", s: "Hitting the number" },
      { t: "p", s: "Guessing at a quality slider and re-checking the size in your file manager is slow, and it is why people end up with a 47 KB file when they were allowed 50. The [Compress Image to Size](/image/compress-image-to-size) tool does the search instead: it re-encodes repeatedly, hunting for the highest quality whose output still fits your limit, and reduces the dimensions only when quality alone cannot get there." },
      { t: "ol", items: [
        "Drop in your JPG, PNG or WebP. The original dimensions and file size appear underneath so you know what you are starting from.",
        "Type the limit in the target box, or use one of the presets - 20 KB, 50 KB, 100 KB, 200 KB, 500 KB and 1 MB cover most forms.",
        "Leave the output as JPG unless you know the portal accepts WebP. WebP is usually 25 to 35 per cent smaller at the same visual quality, which buys real detail at a tight target - but only if allowed.",
        "Leave \"shrink the dimensions\" ticked. It only acts when the target is unreachable at your current size, and it is what makes a 20 KB cap achievable on a phone photo.",
        "Press compress, then check the result panel: it shows the byte count against your budget, the final dimensions, and the quality it settled on.",
      ]},
      { t: "p", s: "If the image already fits the limit, it is handed back untouched rather than re-encoded - re-compressing an already-small JPG throws away detail for no reason, and at a generous target can even come out larger than it went in." },

      { t: "h2", s: "The limits forms tend to set" },
      { t: "p", s: "Exam boards, job portals and government applications converge on a familiar set of caps, usually with dimension rules alongside the byte limit:" },
      { t: "ul", items: [
        "Passport-style photograph: commonly 20 KB to 50 KB, sometimes with a minimum size as well as a maximum.",
        "Scanned signature: commonly 10 KB to 20 KB, and usually wider than it is tall.",
        "Supporting documents and certificates: commonly 100 KB to 500 KB.",
      ]},
      { t: "p", s: "Treat those as the shape of the problem, not the rule for your form. The instructions on the page you are uploading to are the only figures that count, and many portals also impose a minimum file size or fixed pixel dimensions that a byte target alone will not satisfy." },

      { t: "h2", s: "When the result still looks bad" },
      { t: "p", s: "If the output is legible but ugly, the usual cause is that too many pixels are sharing too few bytes. Three things help, in order of how much they buy you:" },
      { t: "ul", items: [
        "Crop before you compress. A passport photo needs head and shoulders, not the room behind you. The [crop tool](/image/crop-image) cuts the pixel count so every remaining byte goes to the part that matters.",
        "Choose the dimensions yourself with the [image resizer](/image/image-resizer) before compressing, rather than letting the search pick them. If you know the form wants 200x230, resize to it first.",
        "Switch to WebP if the portal accepts it, for a meaningful gain at no visual cost.",
      ]},
      { t: "p", s: "If you have no specific number to hit and simply want the file smaller, the plain [image compressor](/image/image-compressor) is the better fit - it optimises for quality per byte instead of chasing a cap." },

      { t: "h2", s: "Why this runs in your browser" },
      { t: "p", s: "The images that come with a byte limit attached are rarely trivial ones. They are passport photographs, scanned signatures and identity documents - precisely the material worth not handing to an unfamiliar server. Every step here happens on your own device using the browser's canvas: the file is read locally, re-encoded locally, and the download comes from memory. Nothing is uploaded, and no copy is left on anyone else's disk." },
      { t: "p", s: "Open the [Compress Image to Size](/image/compress-image-to-size) tool, set your limit, and let it find the quality that fits." },
    ],
    faqs: [
      { q: "How do I compress an image to exactly 20 KB?", a: "Set 20 as the target and let the tool search - but understand that \"exactly\" is not what you want. Byte limits are maximums, so the goal is the largest file that still fits under 20,480 bytes, which is what the search finds. Landing at 19.9 KB is a better result than 20.0 KB, not a worse one, because it satisfies the rule with a margin. On a phone photo a 20 KB cap will almost certainly require reducing the dimensions as well as the quality; leave the shrink option ticked or the target may be unreachable. If the portal counts a kilobyte as 1,000 bytes rather than 1,024, aim at 19 KB to stay clear of the discrepancy." },
      { q: "Does compressing to a size limit reduce the resolution?", a: "Only when it has to. The search always tries quality first and keeps your original dimensions if any quality setting produces a file under the limit. It reduces the pixel dimensions only when even the lowest quality is still too big - a 12-megapixel photo squeezed into 20 KB, for instance, where no quality setting can get there and holding the resolution would just produce an unrecognisable image. The result panel always reports the final dimensions and whether they changed, and you can prevent resizing entirely by unticking the option, in which case an unreachable target is reported rather than silently missed." },
      { q: "Why is my file rejected even though it is under the limit?", a: "Three common causes beyond the byte count. The portal may compute kilobytes as 1,000 bytes, making its real cap about 2.4 per cent tighter than yours - drop the target a kilobyte or two. It may enforce pixel dimensions or an aspect ratio as well as a file size, which a byte target alone will not satisfy; resize or crop to the stated dimensions first. Or it may accept only specific formats, typically JPG and sometimes PNG - a WebP file will be refused on format grounds no matter how small it is, so switch the output to JPG unless the instructions say otherwise. A few forms also set a minimum size, rejecting files that are too small." },
    ],
  },
  {
    slug: "how-to-compress-a-pdf-to-a-specific-size",
    title: "How to Compress a PDF to 100KB (or Any Size Limit)",
    h1: "How to Compress a PDF to 100KB (or Any Size Limit)",
    desc: "How to get a PDF under a hard 100 KB, 200 KB or 500 KB upload cap - what the limit means in bytes, where a PDF actually spends them, and how to hit the number without making the document unreadable.",
    category: "pdf",
    tool: { slug: "compress-pdf-to-size", title: "Compress PDF to Size (KB)" },
    updated: "2026-08-11",
    body: [
      { t: "p", s: "The upload box says \"PDF, maximum 100 KB\". Your scanned certificate is 2.8 MB - twenty-eight times too big. The portal will not negotiate, it will not tell you how to fix it, and the browse button simply refuses the file. Unlike a photo, a PDF gives you no obvious dial to turn." },

      { t: "h2", s: "What a 100 KB limit means in bytes" },
      { t: "p", s: "A kilobyte is almost always 1,024 bytes rather than 1,000, so a 100 KB cap means 102,400 bytes. That is what Windows, macOS and most upload validators use, and it is the number worth working to." },
      { t: "p", s: "A minority of portals do the arithmetic in round decimal thousands, making the same cap 100,000 bytes - about 2.4 per cent tighter. That gap is exactly the margin a tool aiming to land just under 102,400 will be sitting in. If a file that looks comfortably inside the limit is refused anyway, this is the usual reason: set the target a couple of kilobytes lower and compress again." },

      { t: "h2", s: "Where a PDF actually spends its bytes" },
      { t: "p", s: "A PDF is a container. Some of its bytes describe the document itself - the page tree, the cross-reference table, the object that says how big each page is - and the rest hold the content. The container part is small but not free, and it is why very small targets fail." },
      { t: "p", s: "Measured on real output, a rebuilt PDF costs roughly 330 bytes per page plus about 600 bytes fixed: a three-page file spends about 1,600 bytes on structure, a ten-page file about 3,900. So a ten-page document cannot be squeezed into 2 KB no matter how bad the pages look, because the empty container already costs twice that. Everything left over after the structure is the real budget for the pages." },
      { t: "p", s: "That leftover is spent one way: the pages are re-drawn as compressed JPEG images. Two things then decide the size - the JPEG quality, and how many pixels each page is drawn with. The second one multiplies everything, because halving the width and height quarters the pixel count." },

      { t: "h2", s: "A worked example: spending a 200 KB budget" },
      { t: "p", s: "Turn the target into an allowance per pixel and the whole problem becomes visible. Take a three-page scan with a 200 KB cap." },
      { t: "ul", items: [
      ...
      { t: "p", s: "JPEG holds up well at around 1 bit per pixel and falls apart well below it, so the second version is the readable one. Nothing was added to the budget; the same 200 KB was simply shared between fewer pixels. That is the entire trick, and it is why a tool that only lowers the quality slider will hand you a blurry mess while insisting it did what you asked." },
      { t: "p", s: "It also predicts the answer. Given that scan and a 200 KB target, the [Compress PDF to Size](/pdf/compress-pdf-to-size) tool settles on about 75 DPI - the resolution the arithmetic above points at - and produces a 198.7 KB file." },

      { t: "h2", s: "Hitting the number" },
      { t: "p", s: "Guessing at a quality setting and re-checking the file size is slow, and it is how people end up with a 71 KB file when they were allowed 100. The tool searches instead: it renders your pages once, then hunts for the highest quality whose rebuilt PDF still fits, and lowers the page resolution only when quality alone cannot get there." },
      { t: "ol", items: [
      ...
      { t: "p", s: "Results land just under the cap rather than far below it - typically 98 to 99 per cent of the budget - because every unused byte is detail you were entitled to and did not get. If your PDF already fits, it is handed back untouched rather than rebuilt, so nothing is lost and its text stays selectable." },

      { t: "h2", s: "What you give up" },
      { t: "p", s: "Reaching a fixed byte target means the pages become images, so text stops being selectable or searchable. Every size-target compressor makes this trade, and it is worth knowing before you send the file somewhere that might need to copy text out of it." },
      { t: "p", s: "It also means the approach suits some documents far better than others. Scans, photographed certificates and image-heavy reports shrink enormously and lose little that matters. A plain text PDF is a poor candidate: it is already small and efficient, and rasterising it can make it larger while destroying its text. If you have no exact number to hit and just want a smaller file, the plain [Compress PDF](/pdf/compress-pdf) tool is the better fit - it optimises for quality per byte instead of chasing a cap." },

      { t: "h2", s: "When the target is out of reach" },
      { t: "p", s: "Sometimes the number is simply not achievable at a readable quality. Three things help, in order of how much they buy you:" },
      { t: "ul", items: [
        "Send fewer pages. The budget is shared between them, so dropping a page makes every remaining page sharper. Use [Remove Pages](/pdf/remove-pages) to cut what the form does not need, or [Split PDF](/pdf/split-pdf) if the portal accepts separate uploads.",
        "Check whether an image is really wanted. Many forms asking for a 20 KB signature or a 50 KB photograph want a JPG, not a PDF - in which case [Compress Image to Size](/image/compress-image-to-size) does the job with the container overhead removed entirely.",
      ...
      { t: "p", s: "When a target genuinely cannot be met, the tool says so plainly and still offers the smallest file it managed, rather than silently handing back something over the limit." },

      { t: "h2", s: "Why this runs in your browser" },
      { t: "p", s: "Documents that come with a byte limit attached are rarely trivial ones - they are certificates, identity documents, signed forms and bank statements. Every step here happens on your own device: the file is read locally, its pages are rendered and re-encoded by your browser, and the download comes from memory. Nothing is uploaded, and no copy is left on anyone else's disk." },
      { t: "p", s: "Open [Compress PDF to Size](/pdf/compress-pdf-to-size), name your limit, and let it find the resolution and quality that fit." },
    ],
    faqs: [
      { q: "How do I compress a PDF to exactly 100KB?", a: "Set 100 as the target and let the search run - though \"exactly\" is not what you want. Upload caps are maximums, so the goal is the largest file that still fits under 102,400 bytes, which is what the search finds; landing at 99 KB satisfies the rule with a margin, and is a better outcome than 100.0 KB, not a worse one. On a multi-page scan a 100 KB cap will almost certainly require lowering the page resolution as well as the quality, so leave that option ticked or the target may be unreachable. If the portal counts a kilobyte as 1,000 bytes, aim at 97 KB to stay clear of the discrepancy." },
      { q: "Will compressing to a size limit make my PDF unreadable?", a: "It depends entirely on how much budget each page gets. A one-page certificate at 100 KB has plenty to work with and will look fine. Ten pages sharing the same 100 KB gives each page about 10 KB, which forces a very low resolution and will be hard to read. The result panel always reports the resolution it settled on in DPI, so you can judge before sending the file: roughly 150 DPI is comfortable for reading, 100 DPI is usable, and below about 70 DPI small print starts to disappear. If the number looks too low, the fix is fewer pages rather than more compression." },
      { q: "Why can a PDF not be compressed below a certain size?", a: "Two independent floors. First, the container itself costs bytes - about 330 per page plus a fixed overhead - so a fifty-page document has a hard floor of roughly 17 KB before a single pixel is drawn. Second, below a certain number of bits per pixel JPEG stops producing anything meaningful, so there is a practical limit to how little a readable page can cost. When a target sits under those floors the tool reports that it could not be met instead of pretending otherwise, and the way through is to reduce the page count, not the quality." },
    ],
  },
  {
    slug: "how-to-resize-a-photo-and-signature-for-an-exam-form",
    title: "How to Resize a Photo and Signature for an Exam Form",
    h1: "How to Resize a Photo and Signature for an Exam Form",
    desc: "How to get a photograph and signature past an SSC, IBPS, NTA or UPSC upload check - what the pixel and KB rules mean, why the minimum size is usually the harder one to meet, and how to hit both at once.",
    category: "image",
    tool: { slug: "exam-photo-resizer", title: "Exam Photo & Signature Resizer" },
    updated: "2026-08-14",
    body: [
      { t: "p", s: "Application portals for competitive exams are unforgiving about images in a way almost nothing else on the web is. The form wants a photograph at a stated pixel size, in JPG, between two file sizes - and it will refuse the upload for any of those three reasons without telling you which. The registration window is usually short, so this becomes the step that eats an afternoon." },
      { t: "p", s: "The rules are simple once separated out. The difficulty is that the one people expect to be easy is usually the one that fails." },

      { t: "h2", s: "The three rules a portal checks" },
      { t: "ul", items: [
        "Format. Almost universally JPG or JPEG. A PNG straight from a phone screenshot is rejected before anything else is looked at.",
        "Dimensions. Either in pixels (IBPS asks for 200x230 for a photograph) or in centimetres (SSC asks for 3.5 x 4.5 cm). A centimetre figure needs a scanning resolution to become pixels; 300 DPI is the standard assumption and turns 3.5 x 4.5 cm into 413 x 531 pixels.",
        "File size, as a range. This is the part that catches people: forms specify a minimum as well as a maximum. IBPS wants a photograph between 20 KB and 50 KB, and a file of 12 KB fails just as surely as one of 80 KB.",
      ]},
      { t: "p", s: "Ordinary compressors only understand the maximum. They will happily hand back an 8 KB photograph, having done exactly what you asked and left you with a file the portal will not take." },

      { t: "h2", s: "Why the minimum is usually the harder end" },
      { t: "p", s: "Whether a range is easy or hard depends entirely on how many pixels have to share the bytes. Work it out in bits per pixel and the answer falls out immediately." },
      { t: "p", s: "Take the bank recruitment photograph: 200 x 230 pixels, which is 46,000 pixels in total. The 50 KB ceiling is 51,200 bytes, or 409,600 bits - about 8.9 bits for every pixel. The 20 KB floor is 163,840 bits, about 3.6 bits per pixel. For comparison, the same image stored raw with no compression at all, at 24 bits per pixel, would be roughly 135 KB." },
      { t: "p", s: "So the form is asking JPEG for a reduction of only about 2.7:1 at the top of the range and 6.7:1 at the bottom. JPEG routinely achieves 10:1 or 20:1 on photographs without looking bad. A 200 x 230 photo saved at a sensible quality will land somewhere around 12 to 18 KB entirely on its own - comfortably under the ceiling and comfortably under the floor too. The maximum was never the problem. The minimum is." },
      { t: "p", s: "Now take the SSC photograph at 413 x 531 pixels, or 219,303 pixels. The same 50 KB ceiling now buys only 1.87 bits per pixel, and the 20 KB floor 0.75. That is a normal, even demanding, JPEG target, and here the ceiling is what binds. Same stated range, opposite problem, purely because there are 4.8 times as many pixels - which is why one setting never works for two exams." },

      { t: "h2", s: "Meeting both ends at once" },
      { t: "p", s: "The [Exam Photo & Signature Resizer](/image/exam-photo-resizer) works the problem in the order that makes it solvable. It paints your image at the exact pixel size required first, since that figure is not negotiable. Then it searches for the highest JPG quality whose encode still fits under the ceiling - the best-looking result available and also the largest, so it has the best chance of clearing the floor too. Finally it checks both ends and tells you which, if either, it missed." },
      { t: "ol", items: [
        "Drop in your photo or scan.",
        "Pick your exam and document from the list - bank recruitment, SSC, NTA and UPSC presets are included for photographs, signatures and thumb impressions - or type your own dimensions and KB range.",
        "For a photograph, drag the image to centre your face in the frame and use the zoom slider to fit it. For a signature, the framing switches automatically to fit the whole image with white padding, so nothing is cropped off.",
        "Press the button and read the result panel: the required size, the bytes produced, the final dimensions and the quality it settled on.",
      ]},

      { t: "h2", s: "When the file lands under the minimum" },
      { t: "p", s: "By the arithmetic above this is the common failure for small photographs, and it means one thing: there is not enough detail in your image to fill the bytes at that pixel size. A soft photo, a flat background or a picture already compressed once will produce a small file no matter how high the quality is pushed." },
      { t: "p", s: "The honest fix is to give the encoder more pixels. Portals almost always publish their dimension as preferred or recommended while treating the file size range as a hard check - IBPS words it exactly that way - so enlarging the image slightly, keeping the aspect ratio, satisfies the rule that is actually enforced. That option is on by default and the result panel says when it was used." },
      { t: "p", s: "What it will not do is pad the file with filler bytes to fake the size. That trick does get files past a validator, but it produces a document claiming to contain information it does not. If the minimum still cannot be reached you are told directly, and the answer is to re-scan the original more sharply." },

      { t: "h2", s: "Signatures are a different problem" },
      { t: "p", s: "A signature has to be legible end to end, so it must never be cropped to fill a frame - it is fitted whole and padded with white. The wide, short shapes forms ask for (140 x 60 pixels for banks, 4.0 x 2.0 cm for SSC) make this obvious the moment you get it wrong." },
      { t: "p", s: "The other half of the job is the paper. Sign in black ink on genuinely white paper and shoot it in flat, even light - a phone photo of a page picks up shadow and a grey cast that portals reject as an unclear signature. The background cleanup option maps everything lighter than an adjustable threshold to pure white and leaves the ink alone. Note the side effect: a large flat white area compresses extremely well, so a cleaned signature gets smaller and is more likely to need enlarging to clear its minimum." },

      { t: "h2", s: "Check your own notification" },
      { t: "p", s: "Every preset names where its numbers come from, and the bank figures are taken from the official IBPS scanning and uploading guidelines. Even so, commissions restate these specifications in each notification and they do change between cycles. The figures in the notification you are applying under are the only ones that count, and every field in the tool can be typed over when yours differ." },

      { t: "h2", s: "Why this runs in your browser" },
      { t: "p", s: "An exam photograph and a specimen signature are close to the most sensitive pair of files a form will ever ask you for - together they are most of what is needed to impersonate you on paper. Every step here happens on your own device: the image is read, cropped and re-encoded by your browser's canvas, and the download comes out of memory. Nothing is uploaded." },
      { t: "p", s: "Open the [Exam Photo & Signature Resizer](/image/exam-photo-resizer), pick your exam, and get both files right the first time. If the form wants a scanned certificate under a size cap as well, [Compress Image to Size](/image/compress-image-to-size) handles the plain maximum-only case." },
    ],
    faqs: [
      { q: "How do I resize a photo to 20 KB to 50 KB for an exam form?", a: "Set the dimensions the form asks for first, then let a search find the highest JPG quality that still fits under 50 KB - that is the largest acceptable file, so it is also the one most likely to clear the 20 KB floor. At small dimensions like 200x230 the ceiling is rarely the difficulty; a photo that size naturally encodes to around 12 to 18 KB, which is under the minimum. If that happens, enlarging the pixel dimensions slightly is the fix, because portals treat the size range as a hard check while describing the dimensions as preferred." },
      { q: "Is 1 KB counted as 1,024 bytes or 1,000?", a: "Almost always 1,024, so a 50 KB ceiling means 51,200 bytes and a 20 KB floor means 20,480. That is how Windows, macOS and most upload validators count, and the exact byte figure is shown next to the result so you can check. A small number of portals use round decimal thousands instead, which makes the same limits 50,000 and 20,000 bytes. If a file that looks like it fits is rejected anyway, this is the usual reason - aim a kilobyte or two inside both ends of the range." },
      { q: "Why does the portal keep saying my signature is unclear?", a: "Usually the background rather than the signature. A phone photo of paper carries shadow and a grey tone that automated checks read as a dirty scan, and a signature in blue ink on cream paper has weak contrast to begin with. Sign in black ink on white paper, shoot it in flat even light with no shadow across the page, and use the background cleanup option to push the paper to pure white. Also check the shape: these fields want a wide, short image, and a squarish crop with the signature floating in the middle looks wrong to a reviewer even when it passes the automated check." },
    ],
  },
  {
    ...
    desc: "How to convert HEIC photos from an iPhone to JPG - what the format is, why Windows and web forms refuse it, how to convert a whole batch for free in your browser, and why the JPG comes out larger.",
    category: "image",
    tool: { slug: "heic-to-jpg", title: "HEIC to JPG" },
    updated: "2026-08-15",
    body: [
      { t: "p", s: "You email yourself some photos from an iPhone, open them on a Windows machine, and nothing happens. Or a job application refuses the file. Or a printing shop asks you to send them again \"as normal photos\". The files are HEICs, and the fix is a conversion to JPG - which takes seconds and does not need any software installed." },
      { t: "p", s: "This guide covers what the format actually is, the quickest way to convert a batch, and the two things that surprise people afterwards: the JPG being bigger than the HEIC, and photos arriving sideways from some converters." },

      { t: "h2", s: "What a HEIC file is" },
      { t: "p", s: "HEIC is what iPhones and iPads have saved photos as by default since iOS 11 in 2017, under the Settings option called High Efficiency. The file is a HEIF container - the same family of container used by video files - holding a still image compressed with HEVC, also known as H.265. That is the same codec used for 4K video, applied to a single frame." },
      { t: "p", s: "The efficiency is real and it is the whole point. HEVC gets roughly twice as much out of the same number of bytes as JPEG's much older compression does, which is how a phone fits twice as many photos in the same storage without a visible drop in quality. Apple did not invent it and it is not proprietary to Apple: HEIF is an MPEG standard, and Android has supported saving HEIC since Android 10." },

      { t: "h2", s: "Why so little will open them" },
      { t: "p", s: "Support is the price of that efficiency, and it splits along predictable lines:" },
      { t: "ul", items: [
        "Apple devices open them everywhere - Photos, Preview, Quick Look, Safari.",
        "Windows will not, out of the box. Windows 11 ships the container support but not the HEVC decoder, which Microsoft sells separately in the Store. Double-clicking a HEIC on a fresh machine gets you an error or a prompt to buy a codec.",
        "Web forms and uploaders mostly reject them. Anything that says \"JPG or PNG only\" means it, and government portals, job boards and photo printers are the usual places this bites.",
        "Browsers do not display them. Chrome, Firefox and Edge have no HEIC decoder, so a HEIC will not render in an img tag even though it is a perfectly valid image file.",
      ]},
      { t: "p", s: "So the practical answer, whenever a file has to leave the Apple ecosystem, is to convert it. JPG is thirty years old and universally understood, which is exactly why it is still the right target." },

      { t: "h2", s: "Converting a batch in your browser" },
      { t: "p", s: "Open [HEIC to JPG](/image/heic-to-jpg) and the whole job is four steps. Nothing is uploaded - the decoder is a build of libheif compiled to WebAssembly that runs inside your own browser tab, so the photos never leave your device." },
      { t: "ol", items: [
        "Drop in your photos. Select as many as you like at once; a full camera roll export is fine, and each file gets a Remove button in case something slipped in.",
        "Choose the output format. JPG is the default and the right answer nearly always.",
        "Set the quality if you want to. It starts at 92%, which is visually indistinguishable from the original for photographs; drop it to around 80% if you need smaller files for email.",
        "Press Convert. Each photo appears as it finishes with a thumbnail, its upright dimensions and its before-and-after size, with a Download button each and a Download all for the batch.",
      ]},
      { t: "p", s: "The first conversion of a session pauses for a moment while the decoder itself downloads - it is a 3 MB file, because the compression code is genuinely that large - and after that your browser caches it and conversions start immediately." },

      { t: "h2", s: "Why the JPG comes out bigger" },
      { t: "p", s: "This is the most common surprise, and it is not a bug: it is precisely the efficiency you were being sold, being handed back." },
      { t: "p", s: "Take a real 3840 x 2160 photograph - 8.29 megapixels, which is in the same range as an iPhone's 12 MP main camera. Stored raw, with no compression at all, those pixels would be 23.7 MB. Saved as HEIC by Apple's own encoder it comes to 783 KB, or about 0.77 bits per pixel. Re-encode the identical decoded image as a JPEG at quality 90 and it is 1.77 MB - 1.79 bits per pixel, and 2.3 times the size of the HEIC. At quality 80 the JPEG is 1.37 MB, still 1.8 times the HEIC. Saved as a lossless PNG it is 9.83 MB, nearly thirteen times the HEIC." },
      { t: "p", s: "So a 2 MB HEIC turning into a 4 MB JPG is normal and expected. If the result needs to be smaller, there are two honest levers: pull the quality slider down, which costs a little detail, or run the JPG through [Compress Image to Size](/image/compress-image-to-size) afterwards when there is a specific limit like 500 KB to hit. What you cannot do is get JPEG to match HEIC's size at the same quality - no setting does that, because the format is simply older." },

      { t: "h2", s: "JPG, PNG or WebP" },
      { t: "p", s: "JPG unless you have a specific reason otherwise. PNG is lossless, so it will not degrade the image any further, but as the numbers above show it produces enormous files for photographs - it is built for screenshots, logos and line art, not camera output. WebP is the interesting middle: roughly JPEG-quality compression at noticeably smaller sizes, supported by every current browser, but still refused by a lot of desktop software and upload forms. Pick it for the web, not for a form." },

      { t: "h2", s: "Rotation, and why some converters get it wrong" },
      { t: "p", s: "A HEIC does not necessarily store its pixels the right way up. Like a JPEG with an EXIF orientation tag, it can record the rotation as a flag in the container and leave the image data as the sensor read it - so a portrait photo may physically be a landscape image with a note attached saying \"turn this ninety degrees\"." },
      { t: "p", s: "A converter that reads the pixels and ignores the flag hands back a sideways photo, which is why some free converters have a reputation for it. The decoder used here applies the rotation while decoding, so portrait shots arrive as portrait JPGs - and the dimensions shown next to each result are the upright ones, which is a quick way to confirm it worked before you download." },

      { t: "h2", s: "What happens to the metadata" },
      { t: "p", s: "Converting through a canvas rewrites the picture from decoded pixels, and nothing else in the original file travels with it. The JPG you get carries no EXIF at all: no GPS coordinates, no camera model or serial number, no timestamp." },
      { t: "p", s: "For most people that is a quiet benefit - the location tag on a photo taken at home is the single most sensitive thing in a camera roll. If you were relying on the capture date for sorting, note it before converting. And if you want to see what a photo you already have is carrying, or strip a JPG you did not convert, the [EXIF viewer and remover](/image/exif-viewer) reads and cleans files in place." },
      { t: "p", s: "When you are ready, [convert your HEIC photos here](/image/heic-to-jpg) - free, no account, no upload." },
    ],
    faqs: [
      { q: "How do I convert HEIC to JPG on Windows for free?", a: "The simplest route needs no installation at all: open a converter that runs in the browser, such as this one, drop the files in and download the JPGs. Because the decoding happens inside the browser tab, it works the same on a locked-down work machine as on your own, and the photos are never uploaded. The alternative is buying Microsoft's HEVC Video Extensions from the Store, which teaches Windows itself to open HEIC files - useful if you deal with them constantly, but it only makes them viewable, and any form that wants a JPG will still want a JPG." },
      { q: "Does converting HEIC to JPG lose quality?", a: "Technically yes, in the way any lossy re-encode does, but at a sensible quality setting it is not something you can see. The HEIC is decoded to exact pixels first, so nothing is lost in that step; the loss is in the JPEG encode that follows, and at the default 92% quality the difference from the original is invisible on a photograph even side by side. It only becomes visible if you push the quality slider low, or convert the same file back and forth repeatedly. If you genuinely need zero further loss - for archiving or editing - choose PNG instead and accept a file several times larger." },
      { q: "How do I stop my iPhone saving photos as HEIC?", a: "Settings, then Camera, then Formats, and choose Most Compatible instead of High Efficiency. New photos will be saved as JPG from then on, at roughly double the storage cost, and existing HEICs on the phone stay as they are. There is also a middle option worth knowing about: leave the camera on High Efficiency and instead go to Settings, Photos, and under Transfer to Mac or PC choose Automatic, which converts photos to JPG as they are copied off the phone. That keeps the storage saving on the device and hands compatible files to everything else." },
    ],
  },
  ...
  // --------------------------------------------------------------- PROTECT-PDF
  {
    ...
    h1: "How to Password Protect a PDF (and What the Password Really Protects)",
    desc: "How to password protect a PDF for free with AES-256 encryption, in your browser. Which of the two PDF passwords actually protects a file, how to choose one, and what to do before you send it.",
    category: "pdf",
    tool: { slug: "protect-pdf", title: "Protect PDF" },
    updated: "2026-08-16",
    body: [
      { t: "p", s: "You are about to email a scanned passport, a payslip or a signed contract, and you would rather it did not sit readable in someone's inbox forever. Putting a password on the PDF is the right instinct. The confusing part is that the format offers two different passwords, they protect completely different things, and only one of them is real security. This guide covers which one you want, how to add it without uploading the document anywhere, and the one mistake that makes the whole exercise pointless." },

      { t: "h2", s: "The two passwords, and why only one matters" },
      { t: "p", s: "A PDF can carry a user password, also called the document open password, and an owner password, also called the permissions password. They are not two strengths of the same thing - they are different mechanisms." },
      { t: "ul", items: [
        "The user password encrypts the document. Its text is what derives the encryption key, and the pages, fonts and images are genuinely scrambled with that key. Without it a reader shows a prompt and nothing else: no pages, no preview, no thumbnail in a file manager.",
        "The owner password sets permission flags - do not print, do not copy text, do not edit. The file is not sealed. It opens for anybody, and the flags are a request that a well-behaved reader chooses to honour.",
      ]},
      { t: "p", s: "That difference decides everything. If you want a document that cannot be read by the wrong person, you want the user password - the one [Protect PDF](/pdf/protect-pdf) sets. Permission flags are deliberately not offered: any reader may ignore them, and any PDF tool can strip them in about a second, including this site's own [Unlock PDF](/pdf/unlock-pdf). A checkbox promising nobody can print your file would be decoration." },

      { t: "h2", s: "How to password protect a PDF" },
      { t: "p", s: "The whole thing takes under a minute and needs no account or installation:" },
      { t: "ol", items: [
        "Open the [Protect PDF](/pdf/protect-pdf) tool.",
        "Drag your PDF onto the page, or click to choose it.",
        "Type the password, then type it again to confirm. Tick Show password to read back what you typed - worth doing, because a typo here locks the file with a password you do not know.",
        "Check the strength note underneath, then click Protect PDF and download.",
        "Your browser saves the result as protected.pdf. Open it and confirm your reader asks for the password before it shows anything.",
      ]},
      { t: "p", s: "That last step is not a formality - it is the only check that proves the file is encrypted rather than merely marked. If the document opens straight to page one without asking, you have a restricted file, not a protected one." },
      { t: "p", s: "Files that are already locked work too: you will be asked for the current password first, since the document has to be opened before it can be re-sealed, and the copy you download then uses the new one. Changing a PDF password is a single pass rather than an unlock-then-protect round trip." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take a 3-page document of 1,325 bytes. Protected, it comes out at 2,159 bytes. Encryption made it larger, which surprises people, but the growth is not proportional to the file: AES adds an initialisation vector and padding to every string and stream, so the overhead tracks the number of objects in the document - roughly 50 bytes each here, and invisible on a 4 MB scan." },
      { t: "p", s: "Ask the locked file how many pages it has and it refuses to answer - not because of a flag, but because the structure it would have to read is encrypted. Supply the password and it reports 3 pages, with the text of every page identical to the original. Nothing is re-rendered, re-compressed or flattened into images: text stays selectable text, links stay links, and the document looks exactly as it did." },

      { t: "h2", s: "What AES-256 means here" },
      { t: "p", s: "The tool writes AES-256, which the PDF specification calls revision 6 - the strongest encryption the format defines, and the same class of algorithm behind full-disk encryption and HTTPS. Streams, strings and the file structure are all covered." },
      { t: "p", s: "Older PDFs often use 40-bit encryption or RC4, and both are broken: files using them can be opened without the password by readily available software. Those modes are deliberately not offered, because a file protected with them looks protected while not being protected - worse than an unlocked file you know is unlocked." },

      { t: "h2", s: "Choosing a password that holds" },
      { t: "p", s: "Because the encryption is sound, the password is the whole of your security - an attacker guesses passwords rather than attacking AES. Length defeats guessing, and it matters far more than symbols do." },
      { t: "ul", items: [
        "Four unrelated words beat a scrambled eight characters, and you can actually retype them: harbour-quilt-ledger-9 is stronger and easier than Xk!7pQ2z.",
        "Do not reuse a password you use for accounts. This one gets typed into a colleague's laptop or read over the phone, and it travels with the file for as long as the file exists.",
        "Do not use a date of birth or account number just because banks do. Those are short, structured and guessable - convenience, not strength.",
        "Save it in a password manager before you download, not after. This is the step people skip.",
      ]},

      { t: "h2", s: "If you lose the password" },
      { t: "p", s: "The document cannot be opened again - not by this tool, not by any other, and not by us. There is no copy of the key in the file and no back door in the format: the password derives the key, so without it your pages are unreadable rather than hidden behind something that can be switched off. Any service claiming otherwise is either guessing common passwords or misleading you." },
      { t: "p", s: "That is encryption working as intended rather than a limitation, but it does make a protected PDF a genuine commitment. Keep the unprotected original somewhere safe." },

      { t: "h2", s: "Sending a protected PDF to someone" },
      { t: "p", s: "Send the password through a different channel than the file. Putting it in the same email as the attachment defeats the protection entirely, and it is easily the most common mistake here - text it, or say it on a call. Your recipient needs no special software: a document open password is part of the PDF standard, so Acrobat, Preview, Chrome and every phone viewer simply prompt for it." },
      { t: "p", s: "Do the other work first. A password goes on a finished document, so [merge](/pdf/merge-pdf) it, [compress](/pdf/compress-pdf) it for email, or [remove the pages](/pdf/remove-pages) you are not sending before you protect it - afterwards, every one of those steps needs the password again." },

      { t: "h2", s: "Is it private?" },
      { t: "p", s: "Yes, and here it is more than a nicety. [Protect PDF](/pdf/protect-pdf) runs entirely in your browser: the document and the password are both handled on your own device and neither is ever uploaded. A service that asks you to upload a confidential file along with the password meant to protect it has already handed a stranger's server both halves - and the files people want to protect are passports, medical letters, contracts and payslips, exactly the category that should not be leaving your machine." },

      { t: "h2", s: "Protect your PDF now" },
      { t: "p", s: "If you are about to send something you would rather not see forwarded, open [Protect PDF](/pdf/protect-pdf), set a password you have written down, and download the encrypted copy. It is free, and neither your document nor your password leaves your browser." },
    ],
    faqs: [
      { q: "How do I password protect a PDF for free without Acrobat?", a: "Use a browser-based tool like this one: open Protect PDF, drop the file in, type a password twice and download the protected copy. It needs no account, no installation and no Acrobat subscription, and because the encryption runs inside the browser tab the document is never uploaded. The result is a standard encrypted PDF, so the person you send it to can open it in Acrobat, Preview, Chrome or any phone viewer - they just get a password prompt first." },
      { q: "Can a password-protected PDF be cracked?", a: "Not the encryption itself. This tool writes AES-256, and there is no known practical attack against it - which is why the password is the part that matters. A short or common password can be guessed by software that tries millions of candidates, and that is the realistic threat, so length is what protects you: four unrelated words are far harder to guess than eight scrambled characters. What you should not trust is an old PDF using 40-bit or RC4 encryption, since those are genuinely broken and can be opened without the password." },
      { q: "What is the difference between protecting a PDF and restricting printing or copying?", a: "Protecting encrypts the document, so it cannot be opened at all without the password. Restricting only sets flags asking readers not to print, copy or edit - the file still opens for anyone, and the flags are honoured voluntarily, so any tool can strip them in seconds. If your goal is that the wrong person cannot read the file, you need the open password. Restrictions are useful only as a polite signal to colleagues, never as security." },
    ],
  },
  // ------------------------------------------------- PDF metadata
  {
    ...
    h1: "How to Remove Metadata from a PDF (and What It Reveals)",
    desc: "How to remove metadata from a PDF for free. See the author, software and dates a PDF records about you, why clearing the author box is not enough, and how to strip it in your browser.",
    category: "pdf",
    tool: { slug: "pdf-metadata", title: "PDF Metadata Viewer & Editor" },
    updated: "2026-08-19",
    body: [
      { t: "p", s: "Every PDF carries a second document inside it: a small set of records describing who made the file, what software they used and when they last touched it. You never see these while reading, they survive being emailed and re-saved, and they are filled in automatically - which is exactly why they catch people out. A CV sent to an employer, a quote sent to a client or a form sent to a government portal can all name you, name your employer and timestamp your evening, without a word of it appearing on the page. This guide explains what a PDF actually stores, why deleting the author box in Word often fails to remove it, and how to strip the lot in your browser." },

      { t: "h2", s: "What a PDF records about you" },
      { t: "p", s: "The visible half lives in a structure called the information dictionary. It has eight entries, and almost every PDF writer fills at least half of them in:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "None of that is unreasonable on its own. Taken together it is a fingerprint. The Author field alone regularly leaks a full legal name from a document that was meant to be anonymous, and the pair of dates reveals when a document was really prepared - which is awkward when a file was supposed to have been finished a week earlier." },

      { t: "h2", s: "The part no PDF reader shows you" },
      { t: "p", s: "Alongside those eight fields a PDF can carry an XMP packet: a block of XML, stored as a separate object, that repeats the title, author and producer in a different format and often adds more. Acrobat, InDesign and many export pipelines write one. Individual pages can carry their own packet too, and applications can stash private data in a structure called PieceInfo." },
      { t: "p", s: "The important thing about all three is that no ordinary PDF reader displays them. If you open the document properties in Acrobat or Preview, you are shown the information dictionary and nothing else - so a file can look clean in every viewer you have while still naming you in a block sitting a few hundred bytes further down. The [PDF Metadata Viewer](/pdf/pdf-metadata) counts these separately from the visible fields for exactly that reason." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take an ordinary invoice exported from Word on a work laptop. Dropping it into the viewer typically reports something like eleven metadata entries: Title reading Invoice template final v3, Author reading the name the IT department registered when the laptop was set up, Creator reading Microsoft Word, Producer naming the PDF engine and its version number, a creation date of 14 March 2026 at 23:41 and a modification date three minutes later - plus an XMP packet and a per-page block that repeat several of those." },
      { t: "p", s: "Nine of those eleven were written without anyone choosing them. The dates say the invoice was produced at twenty to midnight; the title says it came from a template on its third revision. Neither is on the page, and neither is something you would knowingly send a customer." },

      { t: "h2", s: "How to remove it" },
      { t: "ol", items: [
        "Open the [PDF Metadata Viewer and Remover](/pdf/pdf-metadata) and drop your PDF onto the box. The file is read inside the browser tab, so it is not uploaded anywhere.",
        "Read the list. Every field that has a value is shown, along with a count of any XMP or application blocks found.",
        "Choose Remove everything.",
        "Click Download clean copy. You get the same document back, named with a -no-metadata suffix, with the eight fields, the XMP packet, the per-page packets and the private blocks all gone.",
      ]},
      { t: "p", s: "Nothing about the pages changes. Text, images, fonts, page size and page count come through untouched, because metadata lives in structures alongside the page content rather than inside it. Open the cleaned file next to the original and they are indistinguishable to read." },

      { t: "h2", s: "Why clearing the author box is often not enough" },
      { t: "p", s: "Two things go wrong with the obvious approach of blanking the fields in the authoring application and exporting again." },
      { t: "p", s: "The first is that the export writes fresh metadata of its own. Producer and the modification date are set by the software as it saves, so a document you carefully cleaned comes back naming the tool that cleaned it and timestamped to the moment you did it." },
      { t: "p", s: "The second is subtler and affects tools too. Removing a metadata block from a PDF properly means overwriting the object that holds it, not merely removing the pointer to it. If a tool only unlinks the reference, the XML sits in the saved file exactly as it was - invisible to every reader, still perfectly readable to anyone who runs a text search across the raw bytes. This tool overwrites the object first and then unlinks it, so the packet is genuinely gone from the file rather than just detached from the document tree." },

      { t: "h2", s: "Editing the fields instead of clearing them" },
      { t: "p", s: "Removing everything is not always what you want. A report going into a library, a public tender or a document management system is easier to find later if it has a real title and author, and blank fields can look careless. Choosing Edit the fields lets you set each entry by hand and download the result; leaving a box empty drops that field entirely." },
      { t: "p", s: "One deliberate behaviour is worth knowing about. Even when you are editing rather than removing, the XMP packet is deleted rather than rewritten. That is because some readers prefer XMP over the information dictionary, so leaving an old packet in place would let it override the title you just set - the file would show your new title in one application and the old one in another. Dropping it keeps every reader in agreement." },

      { t: "h2", s: "What stripping metadata does not do" },
      { t: "p", s: "It is a fix for one specific leak, not a general anonymiser. It does not touch anything printed on the page, so a name in a letterhead, a signature block or a footer stays exactly where it is, and it is not redaction. If the sensitive material is part of the document content, it has to be edited out of the content." },
      { t: "p", s: "It also has nothing to do with access control. If the point is that only certain people should read the file, encrypt it with [Protect PDF](/pdf/protect-pdf) - metadata and passwords solve different problems and are worth doing together. And if you are sending photographs rather than documents, they carry the same class of hidden data in a different format: a JPG straight from a phone records the camera model, often its serial number, and frequently the GPS coordinates where the shot was taken. The [EXIF Viewer and Remover](/image/exif-viewer) handles those." },
    ],
    faqs: [
      { q: "Does removing PDF metadata reduce the file size or change quality?", a: "Barely, and not in a way you would notice. Metadata is typically a few hundred bytes to a few kilobytes, so a cleaned file is fractionally smaller. Quality is completely unaffected, because nothing is re-encoded: the page content streams, fonts and images are copied across byte for byte, and only the descriptive entries are dropped. The cleaned document renders identically to the original at any zoom level." },
      { q: "Can someone still tell what software created the PDF after I remove the metadata?", a: "Sometimes, from indirect clues rather than the metadata itself. The way a particular writer lays out objects, names fonts or structures its cross-reference table can hint at its origin to somebody analysing the file closely. What is gone is the explicit, trivially readable statement - the Producer and Creator entries that any tool prints in a second, and that get indexed and passed along when a file is shared. For ordinary purposes such as sending a CV or an invoice, removing them is what matters." },
      { q: "Will the recipient be able to tell I stripped the metadata?", a: "There is nothing added to say so. The tool does not stamp its own name into the file - which is a real risk with PDF libraries, since several write their own Producer string on every save unless explicitly told not to, meaning a careless metadata remover hands back a file advertising the remover. The output here simply has empty entries, which is also what you get from plenty of ordinary PDF writers, so it is unremarkable." },
    ],
  },
  // ---------------------------------------------------------------- PDF to Text
  {
    ...
    desc: "How to extract text from a PDF without the jumbled lines. Why copy and paste breaks, how to keep paragraphs intact, and what to do when the PDF is a scan.",
    category: "pdf",
    tool: { slug: "pdf-to-text", title: "PDF to Text" },
    updated: "2026-08-20",
    body: [
      { t: "p", s: "Selecting text in a PDF reader and pressing Ctrl+C works right up until it does not. The text arrives with a line break after every line rather than at the end of each paragraph, columns interleave into nonsense, or nothing is selectable at all. None of that is your reader misbehaving - it follows from how a PDF stores text. This guide explains what is going on, and how to get clean text out with the [PDF to Text](/pdf/pdf-to-text) converter." },

      { t: "h2", s: "Why copying text out of a PDF goes wrong" },
      { t: "p", s: "A word processor document stores a paragraph as a paragraph. A PDF does not. A PDF is a set of drawing instructions, and text is drawn the same way a rectangle is: put this glyph at these coordinates in this font at this size. There is no record of where a line begins, where a paragraph ends, or which block of text is a column and which is a footnote. All of that is something your eye reconstructs from the layout." },
      { t: "p", s: "So extracting text means rebuilding structure that was never stored. Three consequences follow, and they explain almost every bad paste you have ever had:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "A good extractor therefore works from geometry rather than from file order: it groups text by vertical position into lines, sorts each line left to right, and inserts a space wherever the horizontal jump between two runs is wider than the gap inside a word. That is what happens when you drop a file into the tool." },

      { t: "h2", s: "Extract the text in four steps" },
      { t: "ol", items: [
        "Open [PDF to Text](/pdf/pdf-to-text) and drop your PDF onto the box. It is read inside your browser tab, so the file is not uploaded.",
        "Pick a layout. Flowing paragraphs stitches wrapped lines back together; Keep line breaks leaves every line exactly where it was. The next section covers which to choose.",
        "Optionally type a page range such as 1-3, 7 to take the text from only part of the document, and tick Mark where each page starts if you want a marker between pages.",
        "Click Extract text. The result appears in an editable box - tidy it there if you like, then Copy it to the clipboard or Download .txt.",
      ]},

      { t: "h2", s: "Flowing paragraphs or keep line breaks?" },
      { t: "p", s: "This one setting decides whether the output is pleasant or infuriating, and the right answer depends entirely on what the page contains. Take a short document with a wrapped paragraph followed by an address block:" },
      { t: "p", s: "Our records show the invoice was issued on 4 March and remains / unpaid as of today. Please settle it at your earliest convenience. / Accounts Department / 14 Residency Road / Bengaluru 560025" },
      { t: "p", s: "The first two lines are one sentence that happened to wrap; the last three are an address where the line breaks are the point. Flowing paragraphs rejoins the wrapped pair into one sentence, and the address - set off by a wider vertical gap - starts a new block. Keep line breaks gives you five lines instead, with the sentence still split after the word 'and'." },
      { t: "p", s: "The rule of thumb is straightforward. Choose flowing paragraphs for anything you intend to re-read or re-flow: articles, reports, letters, book chapters. Choose keep line breaks wherever a line is a unit of meaning: addresses, tables, code, poetry, and any file a script will read line by line." },
      { t: "p", s: "How does it tell a wrapped line from a new paragraph? By measuring. It takes the vertical gap between consecutive lines, finds the typical value for that document, and starts a new paragraph wherever a gap is noticeably larger. Documents vary in leading, so a fixed threshold in points would break on the next file; measuring the document against itself does not." },

      { t: "h2", s: "Taking the text from only some pages" },
      { t: "p", s: "Long documents rarely need extracting whole. A page range accepts individual pages and spans together - 1-3, 7, 12-14 - and is validated against the real page count as you type, so a typo is caught before anything runs. It is the quickest way to pull one clause out of a contract, and it keeps the output small enough to read." },

      { t: "h2", s: "When the PDF is a scan" },
      { t: "p", s: "If extraction returns nothing, the usual reason is that there is nothing to return. A scanned document, or a photograph of a page saved as a PDF, contains an image and no characters whatsoever. Your eye reads the words; the file holds pixels. The quick test is to try selecting a word in any PDF reader: if no selection highlight appears, there is no text layer, and no extractor of any kind can produce text from it." },
      { t: "p", s: "The fix is optical character recognition, which looks at the picture and works out which letters it shows. Run the file through [OCR PDF](/pdf/ocr-pdf) first to add a real, selectable text layer, then extract from the result. Accuracy depends on the scan: clean 300 DPI print does very well, while an angled phone snapshot or heavy handwriting needs proofreading." },

      { t: "h2", s: "What extraction cannot carry across" },
      { t: "p", s: "Plain text is plain by definition, so bold, italics, font sizes, colours and images are all dropped. That is usually the point - text is what you wanted - but it does mean you should pick the right tool for the job. If you want an editable document that keeps a document shape, [PDF to Word](/pdf/pdf-to-word) produces a DOCX from the same extraction. If what you actually need is the numbers out of a table, [PDF to Excel](/pdf/pdf-to-excel) detects rows and columns from the same coordinates and gives you a spreadsheet, which is far better than trying to rescue a table from a wall of text." },
      { t: "p", s: "Two smaller things are worth knowing. Text drawn inside an image is not text, so a chart with its labels baked into the picture extracts nothing even in an otherwise selectable document. And the downloaded .txt is UTF-8 with a byte order mark, which is what makes accents, currency symbols and non-Latin scripts open correctly in Notepad on Windows rather than as mojibake." },

      { t: "h2", s: "Nothing leaves your browser" },
      { t: "p", s: "Extraction runs entirely on your own machine. The PDF is opened, the text layer read and the .txt assembled by code running in the page you have open, and no part of the document is sent anywhere - which matters, because the files people most need text out of are contracts, statements and medical letters. You can confirm it the blunt way: load [PDF to Text](/pdf/pdf-to-text), disconnect from the internet, and extract a file. It still works." },
    ],
    faqs: [
      { q: "Why does my extracted text have a line break after every line?", a: "Because the PDF does, in effect - each visual line is stored as its own run of text, with nothing recording that five of them are one paragraph. That is what the Flowing paragraphs layout is for: it measures the document's usual line spacing and rejoins lines that are merely wrapped, breaking only where the vertical gap is bigger than normal. If you are getting one line per line, switch the layout from Keep line breaks to Flowing paragraphs." },
      { q: "Can I extract text from a password-protected PDF?", a: "Yes, if you know the password. Dropping a protected file in prompts you for it, unlocks the document in your browser and then extracts as usual - the password and the file both stay on your device. What no tool can do is extract from a file whose password you do not have; that is the whole purpose of the encryption." },
      { q: "Does the text come out in the right order for a two-column page?", a: "Usually yes for straightforward layouts, because lines are ordered by their position on the page rather than by the order the file happens to draw them in. Genuine multi-column pages are the hard case: two columns sit side by side, so a line from the left column and a line from the right can share the same height and be read as one line. Newsletters, academic papers and magazines are worth checking over after extraction, and often extract more cleanly one column at a time." },
    ],
  },
  // ---------------------------------------------------------------- Resize PDF pages
  {
    slug: "how-to-change-pdf-page-size",
    title: "How to Change PDF Page Size (A4, Letter, Legal)",
    h1: "How to Change PDF Page Size: A4, Letter and Legal",
    desc: "How to change PDF page size without losing quality. Convert a PDF to A4 or Letter, fix mixed page sizes, and understand why resizing does not shrink the file.",
    category: "pdf",
    tool: { slug: "resize-pdf-pages", title: "Resize PDF Pages" },
    updated: "2026-08-21",
    body: [
      { t: "p", s: "A document written in the United States arrives on US Letter; one written almost anywhere else arrives on A4. They look identical on screen and differ just enough that printing one on the other clips a line or leaves a stripe of white down the side. Here is what page size really is inside a PDF, and how to change it with [Resize PDF Pages](/pdf/resize-pdf-pages)." },

      { t: "h2", s: "Page size is a rectangle, not a resolution" },
      { t: "p", s: "Every page carries a rectangle called the MediaBox saying how big the sheet is, measured in points of 1/72 inch. It is the only thing deciding what size the page prints. A4 is 595 x 842 points; US Letter is 612 x 792. Letter is wider, A4 is taller, and neither fits inside the other - the root of every problem here." },
      { t: "p", s: "A second rectangle, the CropBox, says how much of that sheet a reader actually displays. Most files leave it equal to the MediaBox, but scanned and press-ready documents crop in. The CropBox is what you see, so it is what a resize has to work from - otherwise trim marks and scanner edges you have never seen would reappear." },
      { t: "p", s: "What is not in either rectangle is quality. Text is stored as instructions to draw glyphs at coordinates, not as pixels, so resizing re-draws it and it stays perfectly sharp. Photographs are the one exception, and even they only soften if you enlarge a page well past its original size." },

      { t: "h2", s: "Change the page size in four steps" },
      { t: "ol", items: [
        "Open [Resize PDF Pages](/pdf/resize-pdf-pages) and drop your PDF onto the box. It is read inside your browser tab, not uploaded.",
        "Pick the new page size. A4, US Letter, Legal, A3, A5, A6, B5, Executive and Tabloid are listed, and Custom size takes any width and height in millimetres or inches.",
        "Choose an orientation. Keep each page's is the safe default; Portrait and Landscape force every page the same way round.",
        "Leave Content on Fit to page unless you have reason not to, then click Resize and download. The line above the button says exactly what will happen first.",
      ]},

      { t: "h2", s: "A worked example: Letter to A4" },
      { t: "p", s: "Take a US Letter page, 612 x 792 points, and convert it to A4 at 595 x 842. Fitting scales the content until the first edge touches, so it takes the smaller of the two ratios:" },
      { t: "ul", items: [
        "Width ratio: 595.28 / 612 = 0.9727",
        "Height ratio: 841.89 / 792 = 1.0630",
        "Scale used: the smaller of the two, 0.9727 - shown in the tool as 97%",
      ]},
      { t: "p", s: "The content becomes 595 points wide, exactly filling the A4 width, and 770 points tall on a page 842 points tall. The 72 points left over are split evenly, so the content sits centred with about 12.7 mm of white above and below. Nothing is cut off and nothing is stretched, because the same factor is applied to width and height." },
      { t: "p", s: "Going the other way, A4 to Letter, the height binds instead: 792 / 841.89 = 0.9407, or 94%, leaving about 9 mm of white down each side. That is the honest answer to \"why is there a border on my converted PDF\": the two shapes differ, and removing the border would mean cropping or distorting your content." },

      { t: "h2", s: "Fit to page, actual size, or scale by percentage" },
      { t: "p", s: "The Content setting decides how the existing page is mapped onto the new one, and picking the wrong one is the usual cause of a disappointing result." },
      { t: "ul", items: [
        "Fit to page scales the content so all of it lands inside the new page, keeping its proportions. This is what you want for almost every A4-to-Letter conversion.",
        "Keep actual size leaves the content at its original scale and changes only the sheet around it. Use it when physical size matters - a technical drawing, a music score. If the old page is larger, the edges are cut off, and the tool says so before you download.",
        "Scale by % applies a factor from 10% to 200%, for when the scaling itself is the goal - shrinking to 70% to fit two pages on one sheet.",
      ]},
      { t: "p", s: "Fit to page adds two controls. Margin keeps a band clear on all four sides, for printers that cannot reach the paper edge or for binding. Never enlarge stops a small page being blown up to fill a large one, so a receipt stays receipt-sized in the middle of an A4 sheet." },

      { t: "h2", s: "Mixed page sizes and rotated pages" },
      { t: "p", s: "Documents assembled from several sources are rarely uniform - a report might be A4 throughout except for two landscape A3 charts and a scanned receipt. Dropping the file in reports how many distinct sizes it holds, one preview line per size with the pages it covers, so you see what each group becomes before anything is written." },
      { t: "p", s: "Orientation is what stops this going wrong. On Keep each page's, a landscape page becomes landscape A4 and a portrait page portrait A4 - one paper size, each page still the way round it was drawn. Force Portrait and those charts are turned upright: right for a uniform stack to bind, wrong if you need to read them." },
      { t: "p", s: "Rotation deserves its own note, because a PDF can store a page one way round and display it another: a sideways scan is often stored portrait with a flag telling the reader to turn it. The resize works from what you see, not what is stored, and preserves the flag rather than baking it in." },

      { t: "h2", s: "Resizing is not compressing" },
      { t: "p", s: "This is the commonest mix-up, because \"resize\" means something different for images. Changing page size changes the sheet, not the data: the same text, fonts and images are still there, drawn at a different scale. Converting a 4 MB PDF from Letter to A4 gives you a 4 MB PDF." },
      { t: "p", s: "A smaller file is a different job: [Compress PDF](/pdf/compress-pdf) reduces image quality, and [Compress PDF to Size](/pdf/compress-pdf-to-size) searches for the setting that lands the file under a limit you name - what you want when an upload form refuses anything over 2 MB. They compose: resize first, compress afterwards." },

      { t: "h2", s: "What survives the conversion" },
      { t: "p", s: "Many resize tools re-draw each page as one flattened graphic. That looks right and quietly destroys everything interactive: links stop working, form fields become pictures of form fields, bookmarks vanish. Resizing in place avoids that - links, form fields, comments and outlines are carried across, each moved and scaled by the same amount as the content so it still sits over what it points at." },
      { t: "p", s: "Text stays selectable and searchable, because nothing is converted to an image at any point. To confirm it, resize a document with a link in it, then click the link in the result and select a sentence while you are there." },

      { t: "h2", s: "Nothing leaves your browser" },
      { t: "p", s: "The whole conversion runs on your own machine, in the page you have open, and no part of the document is sent anywhere - which matters, because the files most often needed on a specific paper size are contracts, application forms and scanned identity documents. You can check it the blunt way: load [Resize PDF Pages](/pdf/resize-pdf-pages), disconnect from the internet, and resize a file. It still works." },
    ],
    faqs: [
      { q: "Does changing the page size reduce the quality of my PDF?", a: "No. Text and vector graphics are stored as drawing instructions rather than pixels, so they are simply re-drawn at the new size and stay perfectly sharp however much you scale them. Photographs inside the page keep their original pixels, so shrinking a page actually increases their effective resolution, and only a large enlargement - well past double - starts to look soft." },
      { q: "Why does my Letter page get a white border when I convert it to A4?", a: "Because A4 and Letter are different shapes, not just different sizes. A4 is narrower and taller, so fitting Letter content into it scales until the width fits exactly and leaves roughly 12.7 mm spare split between the top and the bottom. The only ways to remove that band would be to crop your content or to stretch it out of proportion, and neither is what you want. Setting the margin to 0 keeps the band as small as the two paper shapes allow." },
      { q: "Can I resize only some of the pages in a document?", a: "The tool applies your chosen size to every page, because the usual reason for resizing is to make a document uniform. If you genuinely need mixed sizes, split the file with Split PDF, resize the part that needs it, and merge the pieces back together with Merge PDF. If your worry is landscape pages being turned upright, you do not need to split anything - leave Orientation on \"Keep each page's\" and they stay landscape." },
    ],
  },
  {
    slug: "how-to-crop-a-pdf",
    title: "How to Crop a PDF (Trim Margins Without Losing Text)",
    h1: "How to Crop a PDF and Trim Page Margins",
    desc: "How to crop a PDF online: drag a box over what you want to keep, trim white margins automatically, and understand why a crop is not a redaction.",
    category: "pdf",
    tool: { slug: "crop-pdf", title: "Crop PDF" },
    updated: "2026-08-22",
    body: [
      { t: "p", s: "Scanned pages arrive with a black border where the lid did not close. Academic papers arrive with margins wide enough to write an essay in, which is exactly what you cannot do on a tablet screen. Slides exported to PDF arrive with a strip of nothing down one side. All three are the same job: move the page boundary inwards. Here is what that does inside the file, how to do it with [Crop PDF](/pdf/crop-pdf), and the one thing about cropping that catches people out." },

      { t: "h2", s: "What cropping really changes" },
      { t: "p", s: "A PDF page is a set of drawing instructions plus two rectangles saying where the paper is. The MediaBox is the sheet, measured in points of 1/72 inch - A4 is 595 x 842 points. The CropBox says how much of it a reader displays. Most files leave the two equal; scanned and press-ready documents already crop in." },
      { t: "p", s: "Cropping moves those rectangles and touches no drawing instruction. That is why a cropped PDF keeps perfectly sharp text - nothing is re-drawn or turned into a picture - why it is instant on a 300-page document, and why the file size barely moves." },
      { t: "p", s: "Both rectangles get written, not just the CropBox. On paper the CropBox alone is what the specification calls a crop, but plenty of software - print pipelines especially - lays the page out from the MediaBox and would ignore a CropBox-only change, so the trim would appear to have silently failed on the one machine you needed it on." },

      { t: "h2", s: "Crop a PDF in four steps" },
      { t: "ol", items: [
        "Open [Crop PDF](/pdf/crop-pdf) and drop your file on. It is read in your browser tab, not uploaded.",
        "Drag the box on the preview over the part you want to keep - inside it to move, a corner or edge to resize.",
        "Check the other pages with Previous and Next, or click Trim white margins to have it measured for you.",
        "Choose whether to crop every page or only some, then click Crop and download.",
      ]},
      { t: "p", s: "Four boxes under the preview show the trim on each edge in millimetres, so you can type an exact figure instead of dragging - useful when a printer has asked for 10 mm off every side." },

      { t: "h2", s: "Letting the tool find the margins" },
      { t: "p", s: "Trim white margins renders the pages, finds where the ink stops on each, and sets the box to the smallest crop that keeps every page whole - the fastest route for a scanned book or a paper to read on a phone." },
      { t: "p", s: "Two details make it work on real scans, not just clean digital files. A single speck of scanner dust would otherwise hold the whole margin open, so a mark only counts once it is connected to something: joined-up strokes and hairlines are kept, isolated specks are not. And blank pages are skipped rather than treated as full-bleed, which would drag the crop back out to the whole sheet." },
      { t: "p", s: "On a long document it measures an evenly spaced sample and tells you how many pages it checked - a cap you cannot see reads as \"I checked everything\" when it did not. Drag the result tighter if you want." },

      { t: "h2", s: "A worked example: an A4 scan with a black edge" },
      { t: "p", s: "Take an A4 page, 595.28 x 841.89 points, whose text block starts 60 points in from the left and right, 70 below the top and 80 above the bottom. Keeping 5 points of breathing room, the trim on each edge is:" },
      { t: "ul", items: [
        "Left and right: 60 - 5 = 55 points, which is 55 / 2.8346 = 19.4 mm",
        "Top: 70 - 5 = 65 points, or 22.9 mm",
        "Bottom: 80 - 5 = 75 points, or 26.5 mm",
      ]},
      { t: "p", s: "The page that comes out is 595.28 - 110 = 485.28 points wide and 841.89 - 140 = 701.89 tall, about 171 x 248 mm. The area kept is (485.28 x 701.89) / (595.28 x 841.89), or 68% of the sheet. On a tablet that is the difference between text you can read and text you have to pinch at." },

      { t: "h2", s: "Rotated pages and mixed page sizes" },
      { t: "p", s: "A PDF can store a page one way round and display it another: a sideways scan is usually stored portrait with a flag telling the reader to turn it 90 degrees. That matters here, because on such a page the edge you see at the top is the edge stored on the left. Crop PDF works from what you see, and leaves the rotation flag alone." },
      { t: "p", s: "Documents assembled from several sources are often not uniform. The crop is held as a proportion of each page rather than a fixed number of millimetres, so the same trim lands sensibly on an A4 page and on an A3 chart and can never ask for more margin than a small page has. Where sizes differ, the tool says so." },
      { t: "p", s: "If only some pages need it - a cover that should stay full-bleed, or scanned pages inside a digital report - switch Crop to Chosen pages and enter something like 2, 5-7. The rest keep their full size." },

      { t: "h2", s: "A crop is not a redaction" },
      { t: "p", s: "This is the part worth reading twice. Cropping hides content; it does not delete it. The words outside your box are still in the file, in the same untouched drawing instructions. Anyone who sets the page rectangles back gets them all returned, in seconds." },
      { t: "p", s: "What makes this genuinely dangerous is that it often looks like it worked. Many readers and text extractors - including any built on the pdf.js engine, which is most browser-based ones - drop text falling outside the page box, so you can crop a signature block off a page, extract the text, and see it come back clean. It is not clean. The next tool will still find it." },
      { t: "p", s: "So crop for margins, borders and framing. If something must actually go, delete it in whatever produced the document and export a fresh PDF, or drop the whole page with [Remove Pages from PDF](/pdf/remove-pages). Never crop a bank statement to hide an account number." },

      { t: "h2", s: "What cropping will not do" },
      { t: "p", s: "It will not shrink the file. Every piece of the original is still present, so a 4 MB PDF cropped to two thirds of its area is still close to 4 MB. For a smaller file run [Compress PDF](/pdf/compress-pdf) afterwards, or [Compress PDF to Size](/pdf/compress-pdf-to-size) when a form caps the upload." },
      { t: "p", s: "It will not give you a standard paper size either - cropping produces whatever rectangle you drew, almost never exactly A4 or Letter. If you need a specific size, crop first, then run [Resize PDF Pages](/pdf/resize-pdf-pages) to fit the result onto that sheet." },
      { t: "p", s: "And it will not flatten anything. Pages are cropped in place rather than re-drawn, so text stays selectable and searchable, and links, bookmarks, form fields and comments stay exactly where they were." },

      { t: "h2", s: "Nothing leaves your browser" },
      { t: "p", s: "The preview is rendered and the crop applied entirely on your own machine, in the tab you have open. Nothing is sent anywhere - which matters, since the files that most need trimming are scanned contracts, statements and identity documents. Check it bluntly: load [Crop PDF](/pdf/crop-pdf), disconnect from the internet, and crop a file. It still works." },
    ],
    faqs: [
      { q: "Can someone recover the part I cropped off?", a: "Yes. A crop moves the page boundary; it does not delete anything, and the hidden content is still sitting in the file's drawing instructions. Anyone who sets the page rectangles back gets it all returned. It is worse than it sounds, because many readers and text extractors hide that content too, so a crop can look like a successful redaction when it is not. To genuinely remove something, delete it in the program that made the document and export a new PDF, or delete the whole page." },
      { q: "Does cropping a PDF reduce the file size?", a: "Barely. Cropping changes the two rectangles that describe the page, not the content inside it, so every piece of text and every image is still there. Expect the file to come out within a percent or so of where it started. If you need it smaller, use Compress PDF or Compress PDF to Size on the cropped file afterwards - the two steps compose fine." },
      { q: "Will cropping blur my text or images?", a: "No. Nothing is re-drawn or rasterised, so text stays vector-sharp and selectable and images keep their original pixels at their original resolution. That is the advantage of cropping in place over tools that redraw each page as a flattened graphic, which also lose links, bookmarks and form fields along the way." },
    ],
  },
  {
    slug: "how-to-fill-in-a-pdf-form-and-flatten-it",
    title: "How to Fill In a PDF Form and Flatten It",
    h1: "How to Fill In a PDF Form and Flatten It",
    desc: "How to fill a PDF form in your browser, why some PDFs have no fields to fill, and what flattening actually does to the answers you typed.",
    category: "pdf",
    tool: { slug: "fill-pdf-form", title: "Fill PDF Form" },
    updated: "2026-08-23",
    body: [
      { t: "p", s: "Two things go wrong with PDF forms. The first is that you cannot type into them at all. The second is subtler: you fill one in, send it off, and are told to resend it \"not editable\", because every answer is still a box someone could change. Both come down to the same piece of PDF machinery - what it is, how to work it with [Fill PDF Form](/pdf/fill-pdf-form), and what flattening really does." },

      { t: "h2", s: "Why some PDFs can be typed into and others cannot" },
      { t: "p", s: "A fillable PDF carries an AcroForm: a list of named fields, each with a type, a value and a rectangle on the page where it is drawn. Click one in a reader and you are editing that field's value, not the page. The page underneath never changes." },
      { t: "p", s: "Most documents that look like forms have none of this. The boxes and ruled lines are printed artwork - part of the same drawing instructions as the headings - so there is nothing to click and nothing to fill. A scan of a paper form is the extreme case: it is a photograph of a form, and no amount of software turns it back into one." },
      { t: "p", s: "There is a third kind, XFA, built with Adobe LiveCycle: the fields live in an XML layout the PDF merely carries, which is why those files show \"Please wait... if this message is not eventually replaced\" in anything but Acrobat. Fill PDF Form detects XFA and says so rather than showing an empty list." },

      { t: "h2", s: "Filling a form in four steps" },
      { t: "ol", items: [
        "Open [Fill PDF Form](/pdf/fill-pdf-form) and drop your PDF on. It is read in your browser tab, not uploaded.",
        "Check the count - it reports how many fillable fields it found, and any values the form already holds are shown filled in, so you only change what needs changing.",
        "Work down the list. Text boxes, tick boxes, dropdowns and multi-select lists each get the right kind of input.",
        "Decide whether to flatten, then click Fill and download.",
      ]},
      { t: "p", s: "Each field is labelled with its tooltip where the form has one - on a well-made form that is the actual question, \"Full name as it appears on your ID\", rather than the database-ish name the designer gave it. Underneath sits the technical detail: the field's real name, which page it is on, whether it is required, and any character limit. Fields the form marks read-only, like a pre-printed reference number, are shown greyed out with their value visible so you can read them but not break them." },

      { t: "h2", s: "What flattening actually does" },
      { t: "p", s: "Tick Flatten and each field's drawn appearance is painted into the page itself, then the interactive field is deleted. The words look identical. They still select, copy, search and print. But there is no longer a box to click into, so nobody can type over your answers or clear them by accident." },
      { t: "p", s: "The clearest way to see the difference is to extract the text. Fill a form without flattening and pull the text out of the page: the answers are not there. They were never on the page - they live in the field objects alongside it, which is why a text extractor, a printer with an old driver, or a viewer that renders fields differently can all disagree about what the document says. Flatten the same file and the answers come out with everything else, because now they genuinely are the page." },
      { t: "p", s: "That is what an office is asking for when it says \"not editable\" - but be precise about what it is not. Flattening locks the text, not the file: anyone can still open it. To keep it shut without a password, add one with [Protect PDF](/pdf/protect-pdf); the two are different jobs and they compose fine." },

      { t: "h2", s: "Signature fields are left alone" },
      { t: "p", s: "If your form ends with a signature box, it is listed but never filled. A digital signature is a cryptographic operation performed with a certificate only you hold; nothing running in a browser tab can produce a valid one on your behalf, and a tool that drew a picture of your name in that box would be handing you something that looks signed and is not." },
      { t: "p", s: "Flattening removes the empty box rather than faking it. If you need a real signature, sign the flattened file in whatever tool holds your certificate - that order matters, because signing locks the document and any fill afterwards would break the signature." },

      { t: "h2", s: "The rupee sign, and other characters that cannot be drawn" },
      { t: "p", s: "Field appearances have to be redrawn whenever a value changes, and the built-in font used for that is WinAnsi-encoded. It covers Latin letters, accents and most European punctuation - Jose Muller, cafe, an em dash and a euro sign are all fine. It does not cover the rupee sign, Devanagari, CJK or emoji." },
      { t: "p", s: "Type one of those and the field warns you, naming the exact characters. The value is still written into the file, and readers with real font support - Acrobat, and browsers' built-in viewers - draw it correctly from the form's own fonts. What you cannot do is flatten that field, because flattening bakes in the drawing and there would be nothing to bake: the answer would be flattened away to nothing. So the tool refuses rather than silently handing you a blank box, and tells you which field is holding things up." },
      { t: "p", s: "If a flattened file is non-negotiable, the workaround is to enter something the font can draw - \"Rs 1,250\" rather than the rupee symbol - or to fill the form in Acrobat, which embeds a font that can. It is a limit of drawing without a Unicode font, not a fault in your file." },

      { t: "h2", s: "A worked example: a 12-field application" },
      { t: "p", s: "Say a form has 12 fields: eight text boxes, two tick boxes, a country dropdown and a signature. You fill the eleven fillable ones. Without flattening the saved file still reports 12 fields, with the answers held in the field objects. With flattening it reports zero, the eleven answers have become page content, and the empty signature box is gone." },
      { t: "p", s: "The file size barely moves either way - the answers are a few hundred bytes against a document that was already there. If the form has to fit an upload cap, that is a separate job for [Compress PDF to Size](/pdf/compress-pdf-to-size), run after flattening rather than before." },

      { t: "h2", s: "What is left untouched" },
      { t: "p", s: "The pages, text, images and page sizes are unchanged; only field values, and on flattening the field objects, are rewritten. Links, bookmarks and comments come across unmoved. The title, author and dates are left as they were rather than re-stamped with today's date and the name of the library that saved the file - a detail most PDF tools get wrong, and one you can check with [PDF Metadata](/pdf/pdf-metadata) before you send it." },
      { t: "p", s: "All of it happens in your own tab. The form is read, filled and saved on your machine and nothing is sent anywhere, which matters more here than for most tools: the fields on a form are, by definition, the parts holding your name, address, income or claim number. Load [Fill PDF Form](/pdf/fill-pdf-form), disconnect from the internet, and fill a form. It still works." },
    ],
    faqs: [
      { q: "Can I undo flattening?", a: "Not from the flattened file. Flattening paints the answers into the page and deletes the fields, so there is nothing left to click into and no record of which words used to be which field. Keep the unflattened copy if you might need to change an answer later - fill the form, download it without flattening, and only flatten the version you are actually sending. The tool names the two files differently for exactly this reason." },
      { q: "Why can I type into my PDF in Acrobat but this tool shows no fields?", a: "Because Acrobat can add fields, and can also let you place free-floating text on a page that has none. Neither is the same as the document having a form in it. If Acrobat is showing you a typewriter-style text tool rather than highlighted boxes, the PDF is flat and there are no fields for any other tool to find. The other possibility is an XFA form, which Acrobat fills through its own XML layout - this tool detects those and tells you rather than showing an empty list." },
      { q: "Will flattening make my form print correctly?", a: "It usually helps. Field values are drawn from a separate appearance stream that some print pipelines and older drivers regenerate, ignore, or render with different spacing - which is the usual cause of a form that looks right on screen and prints with blank or clipped boxes. Flattening removes that whole layer of indirection: the answers become ordinary page content, so anything that can print the page can print them." },
    ],
  },
  {
    slug: "how-to-extract-images-from-a-pdf",
    title: "How to Extract Images from a PDF (Without Losing Quality)",
    h1: "How to Extract Images from a PDF",
    desc: "How to pull the photos out of a PDF at full resolution, why they come out bigger than they looked, and when the original JPEG can be recovered untouched.",
    category: "pdf",
    tool: { slug: "extract-images-from-pdf", title: "Extract Images from PDF" },
    updated: "2026-08-24",
    body: [
      { t: "p", s: "Someone sends you a brochure or a conference programme as a PDF, and the photograph you actually want is in the middle of page four. Screenshotting it gives you whatever your screen was showing. Converting the page to an image gives you the photo plus the headline, the caption and half a column of body text. What you want is the picture itself, at the size it was put in - and it is usually in there at a far higher resolution than the page lets on. [Extract Images from PDF](/pdf/extract-images-from-pdf) reaches inside the file and takes it out." },

      { t: "h2", s: "Why the pictures are bigger than they look" },
      ...
      { t: "p", s: "That is why extraction so often beats every other route, and why picture-heavy PDFs are so much larger than people expect - most of the weight is full-resolution originals nobody ever downsampled." },
      { t: "p", s: "It is also the difference between this tool and [PDF to JPG](/pdf/pdf-to-jpg). PDF to JPG photographs whole pages at a resolution you choose: text, background, borders and all. Extraction ignores the page and takes out the stored images. If you want a picture of a page, use the first; if you want the pictures on it, use the second." },

      { t: "h2", s: "Extract images from a PDF in four steps" },
      { t: "ol", items: [
        "Open [Extract Images from PDF](/pdf/extract-images-from-pdf) and drop your file on it. It is read in your browser tab and never uploaded.",
        "Leave the page box blank to sweep the whole document, or type a range like 4-9 to look at part of it.",
        "Choose Original, PNG or JPG, and set how small an image has to be before it is ignored.",
        "Click Find images. Every picture appears as a thumbnail with its true pixel size; download one with the arrow on its card, or take the lot as a zip.",
      ]},
      { t: "p", s: "The whole sweep happens on your machine - which matters when the document is a client's brochure, a passport scan or a set of medical images." },

      { t: "h2", s: "Original, PNG or JPG" },
      { t: "p", s: "Original is the setting to leave alone. When a PDF embeds a photograph it very often stores the photographer's JPEG untouched, and where that is true the file is handed back byte for byte - not decoded, not re-compressed, not re-saved. What lands in your downloads folder is the same file that went in, down to the last bit." },
      { t: "p", s: "That only holds when the stored bytes would open correctly on their own, and a PDF has several ways of making that untrue: transparency held in a separate mask a plain JPEG cannot carry, an inverted image, CMYK - which browsers cannot display at all - or a colour palette, so the numbers in the file are palette entries rather than colours. In any of those the picture is rebuilt from the decoded pixels instead, because the raw bytes would give you the wrong colours. You never have to work out which case you are in; the tool checks each image and picks." },
      { t: "p", s: "Pick PNG when you want one format throughout and transparency kept - a logo lifted from a brand PDF is the usual reason. It throws no detail away on an opaque image, where every pixel comes back bit for bit, but it is not built for photographs: a rebuilt PNG of a photo is normally several times the size of the JPEG it came from. Pick JPG for small files where transparency does not matter; anything transparent is flattened onto white. If they still need to be smaller, [Compress Image to Size](/image/compress-image-to-size) takes them to an exact figure in KB." },

      { t: "h2", s: "A worked example: the photo that was 600 PPI all along" },
      { t: "p", s: "Take a product catalogue. On page three is a photograph of a chair, printed about two inches wide - a modest thumbnail in a column of text. Screenshotting it at that size gets you roughly 200 pixels across, unusable for anything but the web." },
      ...
      { t: "p", s: "That photo is almost certainly stored as a plain JPEG, so on Original you get the photographer's file back untouched. PNG gives you the same 1200 x 800 pixels in a considerably larger file. JPG re-encodes them - visually fine, but a second round of JPEG on top of the first, which is exactly what Original exists to avoid." },

      { t: "h2", s: "When a PDF comes back with nothing in it" },
      { t: "p", s: "Plenty of documents genuinely contain no images, which surprises people who can see pictures on the page. Charts, logos, icons, maps and diagrams from a word processor or a design tool are usually vector artwork - lines, curves and fills, drawn fresh each time the page is displayed. There is no picture inside to take out, only instructions for drawing one. The same goes for the one-colour stencils PDFs use for shapes and shading." },
      { t: "p", s: "The other reason is the size filter, and it is doing you a favour: documents are full of images that are not pictures - one-pixel gradient strips stretched across a header, rule artwork, dither tiles. The default skips anything under 24 pixels on either side. Slide it to 0 to see everything a file contains." },
      { t: "p", s: "If the page really is a photograph - a scan, where the whole sheet is one big image - extraction will find it, and it comes out as the full scanned page. To pull the text out of that instead, use [OCR PDF](/pdf/ocr-pdf)." },

      { t: "h2", s: "One picture, one file" },
      { t: "p", s: "A logo that appears on all forty pages of a report is stored once in the file and drawn forty times. It is saved once, too, rather than handed back as forty identical files with forty different names. The same goes for a photograph placed twice on one page." },
      { t: "p", s: "That works because images are matched by their identity inside the document rather than by the name they turn up under - the same picture reached from a second page is announced under a completely different one, so matching on names would hand you duplicates." },

      { t: "h2", s: "What you get in the zip" },
      { t: "p", s: "One file per image, named after the PDF plus where it was found: catalogue-p3-2.jpg is the second image on page three of catalogue.pdf. That keeps the folder sortable by page when you are matching pictures back to the document. The archive stores the images rather than compressing them again - JPEG and PNG are already compressed, so a second pass would cost time and save close to nothing." },
      { t: "p", s: "One thing extraction quietly does for you: images rebuilt as PNG or JPG carry no EXIF, so the camera model, timestamps and any GPS coordinates that travelled into the PDF do not travel out of it. Files taken out with Original keep whatever metadata they had, which is the honest trade for byte-identical bytes - run those through [EXIF Viewer & Remover](/image/exif-viewer) if you are about to publish them." },
    ],
    faqs: [
      { q: "Is this the same as converting a PDF to images?", a: "No, and the difference matters. Converting a PDF to images photographs each whole page - text, background and layout included - at whatever resolution you choose, which is what you want for previews or for sending a page to someone who cannot open PDFs. Extracting images reaches inside the page and takes out the pictures themselves, at the resolution they are stored, with no text or background around them. If you want a picture of the page, convert it; if you want the pictures on it, extract them." },
      { q: "Why is the extracted image a different size from what I see on the page?", a: "Because those are two separate numbers in the file. A PDF records the picture at one resolution and separately records how large to draw it, so a photograph placed as a small thumbnail can easily be several thousand pixels wide underneath. Extraction always gives you the stored version, which is the larger and more useful one. It can also be smaller than expected - a low-resolution image stretched across a page comes out at its real size, which is a useful thing to discover before you print it." },
      { q: "Are the images uploaded anywhere?", a: "No. The PDF is opened and its images taken out entirely inside your browser tab, using your own machine's processor. Nothing is sent to a server, nothing is stored, and the tool keeps working with the network disconnected. For a document holding photographs of people, a portfolio you have not published, or scanned identity papers, that is the whole point." },
    ],
  },
  // ---------------------------------------------------------------- JSON-TO-YAML
  {
    slug: "how-to-convert-json-to-yaml",
    title: "How to Convert JSON to YAML",
    h1: "How to Convert JSON to YAML (Without Corrupting Your Values)",
    desc: "How to convert JSON to YAML: how objects and arrays become indented blocks, which values have to be quoted to survive, why long IDs and 1.50 get rounded by naive converters, and how multi-line text is written - with a worked example.",
    category: "convert",
    tool: { slug: "json-to-yaml", title: "JSON to YAML" },
    updated: "2026-08-26",
    body: [
      { t: "p", s: "JSON is what machines emit: an API response, a package manifest, a config export. YAML is what people edit - Docker Compose files, Kubernetes manifests and CI pipelines are all YAML, because indentation reads better than nested braces. Converting between them is one of the most common chores in a developer's week, and quietly one of the easiest to get wrong, because YAML infers the type of every unquoted value from its shape. This guide explains how the [JSON to YAML](/convert/json-to-yaml) converter maps one format onto the other, which values it has to quote and why, and how it keeps your numbers exactly as you wrote them." },

      { t: "h2", s: "How JSON maps onto YAML" },
      { t: "p", s: "The two formats describe the same three building blocks, so the mapping itself is simple. A JSON object becomes a YAML mapping: each key: value pair on its own line, with no braces and no commas. A JSON array becomes a YAML sequence: one dash-prefixed item per line, with no brackets. A JSON scalar - string, number, true, false or null - becomes a YAML scalar, usually written without any quotes at all." },
      ...
      { t: "p", s: "Two cases have no indentation to give them away, so YAML borrows JSON's own syntax: an empty object is written {} and an empty array []." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take this service configuration as JSON - a few top-level settings, an array, a nested object and a two-line message:" },
      { t: "ul", items: [
        "{",
        "  \"name\": \"web-server\",",
        "  \"version\": \"1.20\",",
      ...
      { t: "p", s: "Paste it into the [JSON to YAML](/convert/json-to-yaml) converter and you get this back:" },
      { t: "ul", items: [
        "name: web-server",
        "version: '1.20'",
        "port: 8080",
      ...
      { t: "p", s: "The converter checks every string against the union of the YAML 1.1 and YAML 1.2 rules and quotes it if either one would re-type it. That is deliberately conservative: you rarely know which loader will read the file at the other end, and a quoted string that did not need quotes costs two characters, while an unquoted one that did can cost you an outage. It is why version: '1.20' and region: 'NO' came back quoted above." },

      { t: "h2", s: "Numbers keep their exact digits" },
  ...
  // ------------------------------------------------------------- XML-FORMATTER
  {
    ...
    body: [
      ...
      { t: "p", s: "Paste it into the [XML Formatter](/dev/xml-formatter) and it comes back like this:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "What this is not is validation against a DTD or XSD schema. Well-formed means the syntax holds together; valid means the document also matches a declared model - the right elements, in the right order, with the right attributes. A document can be perfectly well-formed and still be rejected by the service you send it to, and checking that needs the schema, which only you have." },

      { t: "h2", s: "When to minify instead" },
      { t: "p", s: "The Minify tab does the reverse job: it drops the whitespace between elements and puts the document on one line. That is worth doing for XML that travels - SOAP bodies, an RSS feed, a sitemap - where indentation is pure overhead, and the tool reports how many characters it saved. The two directions agree: minifying a document and beautifying it again gives the same result as beautifying the original, because both modes make the same promise about your content. Both also offer 'Remove comments', useful before publishing a config file that still carries a developer's notes." },

      { t: "h2", s: "It runs entirely in your browser" },
      { t: "p", s: "XML is the format of API payloads, invoices and export files, so it routinely holds hostnames, order numbers, customer names and API keys. The [XML Formatter](/dev/xml-formatter) parses and rewrites everything inside your browser tab - nothing is uploaded, nothing is stored, and it keeps working with the network off. If your data is in a different format, the [JSON Formatter](/dev/json-formatter) does the same job for JSON, the [HTML Minifier](/dev/html-minifier) handles markup destined for the browser, and [JSON to YAML](/convert/json-to-yaml) and [YAML to JSON](/convert/yaml-to-json) cover the config-file pair." },
    ],
    faqs: [
      { q: "Why did my paragraph stay on one line instead of being indented?", a: "Because it holds text as well as tags, and indenting it would change the text. An element with mixed content - a paragraph with a bold word inside, a description containing a link - has real character data between its child elements, so pushing those children onto separate lines inserts newlines and indentation into the string itself. The formatter writes any element holding real text on a single line and only indents elements whose content is entirely other elements, which is what keeps the document you get back identical to the one you pasted in." },
      { q: "What is the difference between well-formed and valid XML?", a: "Well-formed means the syntax is correct: every tag is closed and properly nested, attribute values are quoted, and special characters like & are escaped. Valid means the document additionally matches a DTD or XSD schema - the expected elements, in the expected order, with the expected attributes and types. This tool checks well-formedness, which is what a parser needs before it can read the file at all, and reports the line and column of the first problem. It does not check validity, because that requires the schema your system defines." },
      { q: "Is my XML uploaded anywhere?", a: "No. The document is parsed and formatted entirely in your browser and nothing is sent to a server, so an API payload, an invoice export or a config file holding hostnames and keys never leaves your device." },
    ],
  },
  // -------------------------------------------------------- STRING-ESCAPER
  {
    slug: "how-to-escape-a-string",
    title: "How to Escape a String",
    h1: "How to Escape a String for JSON, HTML, XML, CSV and SQL",
    desc: "How to escape a string properly: what changes per format, the ordering bug in nearly every hand-rolled escaper, the characters XML cannot hold at all, and why escaping is not a security control.",
    category: "dev",
    tool: { slug: "string-escaper", title: "String Escaper" },
    updated: "2026-08-30",
    body: [
      { t: "p", s: "Sooner or later a piece of text breaks something. A customer called O'Brien ends a SQL query halfway through. A product description with an ampersand stops an XML feed parsing. A company name containing a comma splits into two spreadsheet columns. The fix in every case is escaping - rewriting the characters the destination treats as punctuation so they arrive as plain text. This guide covers what escaping changes, the mistake nearly every hand-rolled escaper makes, and where escaping stops being enough. The [String Escaper](/dev/string-escaper) does all eight formats below in your browser." },

      { t: "h2", s: "Escaped for what?" },
      { t: "p", s: "The most useful thing to know is that there is no such thing as an escaped string. Escaping is always escaping for something, and the rules do not overlap. Take one apostrophe. In JSON nothing happens to it. In an HTML attribute it becomes &#39;, in XML &apos;, in SQL two apostrophes. On a command line it turns into a four-character sequence that closes the quoted string, adds an escaped quote and reopens it. Five destinations, five answers, and applying the wrong one is not a partial fix - it is a new bug." },
      { t: "p", s: "So the first question is never how to escape but where the text is going. That is why the tool asks for a target format first, and why each one names the exact position its output is valid in: the inside of a JSON string, one RFC 4180 field, one shell argument." },

      { t: "h2", s: "One sentence, eight formats" },
      { t: "p", s: "Here is a line of ordinary marketing copy - an ampersand, an apostrophe, two double quotes and a comma:" },
      { t: "ul", items: ["Tom & Jerry's \"Best of\", 50% off"] },
      { t: "p", s: "Paste it into the [String Escaper](/dev/string-escaper) and switch the target format:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Building the output one character at a time, rather than by a chain of replacements, makes the bug structurally impossible - which is how this tool works. It also handles what a regex-based escaper gets wrong quietly: astral characters, and unpaired surrogates left when a string was truncated at the wrong byte." },

      { t: "h2", s: "The characters that cannot be escaped at all" },
      { t: "p", s: "Most escaping questions have an answer. A few do not, and XML has the sharpest example. XML 1.0 permits exactly three control characters - tab, line feed and carriage return - and there is no escape for the rest. Writing &#1; does not produce character one; it is itself a parse error. A document holding a stray control character, common in text pulled from a legacy database, cannot be made into valid XML 1.0 without removing something." },
      { t: "p", s: "A tool that just swaps the five predefined entities hands you that document and says nothing; you find out when the parser at the other end rejects it. The [String Escaper](/dev/string-escaper) names the code points instead and offers to strip them. Whitespace holds a related trap: a parser normalises a literal carriage return to a line feed, and inside an attribute value turns tabs and newlines into plain spaces. If those matter they must be written as &#13;, &#9; and &#10;, which is what the attribute-value setting does." },

      { t: "h2", s: "Escaping is not a security feature" },
      { t: "p", s: "This is worth saying plainly, because escaping is often reached for as a defence. Escaping a SQL value protects it only where it already sits inside quotes. It does nothing for:" },
      { t: "ul", items: [
  ...
  // ------------------------------------------------------------------- CRON
  {
    ...
    body: [
      ...
      { t: "p", s: "Four symbols fill them in. A * means every value the field allows. A hyphen makes a range, so 9-17 is every hour from nine in the morning to five in the afternoon. A comma makes a list: 0,30 is on the hour and on the half hour. And a slash makes a step, so */15 in the minute field is 0, 15, 30 and 45." },
      { t: "p", s: "That is the whole language. Everything else is combinations of those four." },

      { t: "h2", s: "Reading one, left to right" },
      { t: "p", s: "Take 0 9 * * 1-5. Minute 0, hour 9, any day of the month, any month, days one to five of the week. Monday is 1, so that is nine in the morning on weekdays - the single most common cron line in existence." },
      { t: "p", s: "Now a busier one: */15 9-17 * * 1-5. Minute steps of 15, hours nine through seventeen, weekdays. Every fifteen minutes during the working day, which is 36 runs a day and 180 a week - worth multiplying out before you deploy something that sends an email. Writing one is the same process backwards, and the [Cron Expression Generator](/dev/cron-expression-generator) has a Build tab that assembles the fields from dropdowns." },

      { t: "h2", s: "The trap: day-of-month and day-of-week are OR, not AND" },
      { t: "p", s: "This is the one that bites everybody, and it is not a quirk of any particular implementation - it is in the crontab manual. If both the day-of-month field and the day-of-week field are restricted, meaning neither of them is a *, cron runs the job when EITHER matches. Not both." },
      { t: "p", s: "The manual's own example is 30 4 1,15 * 5. Read it the natural way and you get \"half past four on the 1st and the 15th, if that day is a Friday\" - a couple of times a year. What it actually means is half past four on the 1st and the 15th of every month, plus every Friday. In September 2026 that fires on the 1st, the 4th, the 11th, the 15th, the 18th and the 25th: six times, not zero." },
      { t: "p", s: "There is a further wrinkle. Vixie cron, the crontab on most Linux systems and on macOS, decides whether a field is \"restricted\" by looking at whether its text starts with a *. So */2 counts as unrestricted and 1-31 does not, even though the two cover almost the same days: change the character and you change the logic. The generator flags whenever this rule is in play. If you genuinely want \"the 1st, but only if it is a Monday\", cron cannot say it - run on every 1st and let the script check the weekday and exit." },

      { t: "h2", s: "The trap: a step does not wrap" },
      { t: "p", s: "0 */7 * * * looks like \"every seven hours\". It is not. A step counts from the start of the field and stops at the end, so this gives hours 0, 7, 14 and 21 - and then the day rolls over and the next run is at 0 again, three hours later instead of seven. Three even gaps and one short one, every day." },
      { t: "p", s: "The same thinking sinks \"every 90 minutes\". Write */90 in the minute field and there is nothing to match past 59, so you get minute 0 and nothing else: an hourly job in disguise. Cron has no memory of when it last ran, so any interval that does not divide evenly into an hour or a day has to be spelled out. Every 90 minutes is two lines:" },
      { t: "ul", items: [
        "0 */3 * * * - midnight, 03:00, 06:00 and so on",
        "30 1-22/3 * * * - 01:30, 04:30, 07:30 and so on",
      ]},
      { t: "p", s: "Between them those cover 00:00, 01:30, 03:00, 04:30 and the rest of the sixteen slots in a day. Steps that divide cleanly - */5, */15, */30 in minutes, */2, */3, */4, */6, */12 in hours - are safe for exactly that reason." },

      { t: "h2", s: "The trap: whose five fields?" },
      { t: "p", s: "Cron syntax has dialects, and pasting one into another is a quiet way to schedule the wrong thing. Standard crontab takes five fields. node-cron, Spring's @Scheduled and many other libraries put a seconds field in front, making six. Java's Quartz - and AWS EventBridge, which copies its shape - takes six or seven, with an optional year on the end." },
      { t: "p", s: "Two differences matter more than the field count. Quartz requires a ? in exactly one of the two day fields, because it refuses to schedule on day-of-month and day-of-week at once; that is how it sidesteps the OR problem above. And Quartz numbers the week from 1 = Sunday, so 2 is Monday and 6 is Friday - one higher than a crontab all the way along. The same digit in the same position means a different day. Set the flavour in the [Cron Expression Generator](/dev/cron-expression-generator) before you read anything back, because it changes the answer." },
      { t: "p", s: "Quartz also says things plain cron cannot. L in day-of-month is the last day of the month, LW the last weekday, L-3 three days before the end; 15W is the weekday nearest the 15th without crossing into another month; 6L is the last Friday and 6#3 the third Friday. Month-end billing is the usual reason people find these." },

      { t: "h2", s: "Shorthands, and the clocks" },
      { t: "p", s: "Vixie cron accepts a few nicknames: @yearly, @monthly, @weekly, @daily, @hourly and @reboot. The first five are plain aliases - @daily is exactly 0 0 * * *. @reboot is different in kind: no schedule at all, it runs once when cron starts, and most other schedulers, including systemd timers and Kubernetes CronJob, do not accept it." },
      { t: "p", s: "Cron reads the machine's clock, which is very often not yours - a container is usually on UTC while you are not. That matters twice a year. The crontab manual says a job skipped by a forward clock change runs soon after the change, so a 02:30 job runs at 03:00 that night; going the other way, a job set for a particular time is not re-run during the repeated hour, while jobs with a wildcard in the hour or minute just follow the new wall clock. The tool shows next runs in whichever zone you pick and flags any that land in a skipped or repeated hour. To reconcile those against log timestamps, the [Unix Timestamp Converter](/convert/epoch-converter) reads them in both UTC and local time." },

      { t: "h2", s: "A short checklist" },
      { t: "ol", items: [
      ...
      { t: "p", s: "All of this runs in your browser on the [Cron Expression Generator](/dev/cron-expression-generator) - the expression and any job names in it are never uploaded. For the other end of a scheduled job, [JSON Formatter](/dev/json-formatter) and [String Escaper](/dev/string-escaper) handle the payloads it produces." },
    ],
    faqs: [
      { q: "What is the difference between */5 and 0-59/5?", a: "In how the days are worked out, nothing: both give minutes 0, 5, 10 and so on. In one specific case they behave differently, and it is a real one. Vixie cron decides whether the day-of-month and day-of-week fields are \"restricted\" by checking whether the text starts with a *, and that decides whether the two are combined with OR or with AND. So in the day-of-month field, */1 counts as a wildcard and gives you AND, while 1-31 is treated as restricted and gives you OR, even though the two match every day of the month. In the minute and hour fields there is no difference at all, and */5 is the clearer thing to write." },
      { q: "Can cron run a job on the last day of the month?", a: "Not in plain crontab syntax, which has no way to say \"last\" - the month is 28, 29, 30 or 31 days long and the expression cannot ask. Writing 0 0 31 * * skips February, April, June, September and November entirely. There are two honest fixes. If you are on Quartz, Spring or AWS EventBridge, use L in the day-of-month field. On an ordinary crontab, schedule the job every day and have the script exit immediately unless tomorrow's date is the 1st, which is one line at the top and works in every month length. Scheduling 28-31 and hoping is the version that fires four times in March." },
      { q: "Why did my cron job not run at all?", a: "In rough order of likelihood: the expression matched a date that does not exist, such as 0 0 30 2 * on 30 February, which is valid syntax and will never fire; the server is in a different time zone than you assumed, so it ran, just not when you were watching; the crontab has no trailing newline on the last line, which some cron implementations silently ignore; or the job ran and failed, because cron uses a minimal PATH and a bare command that works in your shell may not resolve. Paste the expression into the generator first - if the next-run list is empty or the dates look wrong, the problem is the schedule, and if the dates look right the problem is the environment." },
    ],
  },
  // --------------------------------------------------------------- HTML FMT
  {
    slug: "how-to-format-html",
    title: "How to Format HTML Without Changing How It Renders",
    h1: "How to Format HTML Without Changing How It Renders",
    desc: "How to beautify and indent HTML safely. Why a line break between inline tags adds a visible space, which elements you can indent freely, and what pre, script and custom elements need.",
    category: "dev",
    tool: { slug: "html-beautifier", title: "HTML Beautifier" },
    updated: "2026-09-04",
    body: [
      { t: "p", s: "Pasting minified HTML into a formatter feels like a free operation. It is not. HTML is one of the few languages where whitespace is content, and a beautifier that indents everything the way a JSON formatter would will quietly change what your page looks like. This guide explains exactly where the line is - which whitespace a browser throws away and which it paints on the screen - so you can indent messy markup and know that nothing moved. The [HTML Beautifier](/dev/html-beautifier) applies these rules for you." },

      { t: "h2", s: "Why formatting HTML is not free" },
      { t: "p", s: "Browsers process whitespace with a rule usually called collapsing. Inside a run of text, any sequence of spaces, tabs and newlines is treated as one single space. That is why you can wrap a paragraph across ten source lines and it still reads as one flowing sentence." },
      { t: "p", s: "The catch is the word \"one\". A newline does not disappear - it collapses to a space. So if two tags were touching and a formatter puts them on separate lines, the browser now sees a space between them that was not there before." },
      { t: "p", s: "Here is the whole problem in one line of markup:" },
      { t: "p", s: "<div class=\"tags\"><span>new</span><span>sale</span></div>" },
      { t: "p", s: "That renders as newsale - two badges pressed together, which is almost certainly what the CSS expects. Now let a naive formatter indent it:" },
      { t: "p", s: "<div class=\"tags\">, newline, two spaces, <span>new</span>, newline, two spaces, <span>sale</span>, newline, </div>" },
      { t: "p", s: "The newline plus indent between the two spans collapses to a single space, and the page now reads new sale. Nothing warned you. The HTML is still valid, the diff looks like pure whitespace, and the layout shifted by exactly one space character - enough to break a pill row, a set of inline-block cards, or an icon sitting flush against its label." },

      { t: "h2", s: "The rule that makes it safe: block versus inline" },
      { t: "p", s: "The fix is not to avoid line breaks. It is to only put them where the browser is guaranteed to discard them, and CSS is precise about where that is. Whitespace is removed when it sits at the start of a line box, at the end of one, or next to a block-level box." },
      { t: "p", s: "In practice that gives two families of element:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "So the safe behaviour is: indent block-level elements one per line, and keep anything that holds text or inline tags on a single line with its text untouched. A paragraph such as <p>Some <b>bold</b> text.</p> stays exactly as written, because breaking it apart would insert line breaks into the sentence itself." },
      { t: "p", s: "This is the biggest difference between formatters. Many will split a run of inline tags to keep lines short, and on most pages you never notice - until the one time you do." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take a fragment of real, minified markup:" },
      { t: "p", s: "<div class=\"card\"><h2>Title</h2><ul><li>One<li>Two</ul><p>Some <b>bold</b> text.</p></div>" },
      { t: "p", s: "Run it through the [HTML Beautifier](/dev/html-beautifier) and the div, h2, ul, the two li items and the p each get their own line, indented by depth. The contents of the h2 and the p are left alone, and the bold tag stays inside the sentence." },
      { t: "p", s: "Notice the two bare li tags. That is legal HTML - the closing tag on a list item is optional, and a browser closes one li when the next one opens. A formatter needs that rule to indent the second item at the right depth, and it must not \"helpfully\" add the missing tags: in a framework template, an added closing tag changes what gets parsed." },
      { t: "p", s: "A quick way to check any formatter: strip all whitespace from what you pasted in and from what came out. If the two strings are not identical, the tool rewrote your markup rather than formatting it." },

      { t: "h2", s: "The parts that must never be touched" },
      { t: "p", s: "Four elements carry content that indentation would corrupt outright:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Shifting a script or style block to a new indentation level looks far better than leaving it at column zero. The honest approach is to shift it only when the block provably holds no multi-line string, and to say so when it does not." },
      { t: "p", s: "One more case is visible from the markup itself: an inline style=\"white-space: pre\" turns preservation back on for any element, and a formatter can honour that. A utility class doing the same thing cannot be seen from the HTML at all - worth remembering if you use one." },

      { t: "h2", s: "Custom elements and framework templates" },
      { t: "p", s: "An element the browser does not recognise gets display: inline by default. That includes every custom element and web component, so <my-badge>a</my-badge><my-badge>b</my-badge> behaves exactly like the two spans from earlier - splitting them adds a space." },
      { t: "p", s: "If your components are styled as blocks, that default is too cautious, which is why the beautifier has a switch to treat custom elements as block-level. It is off by default because it is the one option that can change how a page renders, and it should be a decision you make rather than one made for you." },
      { t: "p", s: "Templates bring a second hazard: case. HTML tag names are case-insensitive, so plenty of formatters lower-case them as a tidying step. In a Vue single-file component or any JSX-style template, <MyComponent> and <mycomponent> are different things, and lower-casing breaks the build. Attribute quoting is the same story - rewriting single quotes to double can break a value holding a double quote of its own." },

      { t: "h2", s: "Beautify or minify?" },
      { t: "p", s: "The two serve different moments. Beautify when reading or debugging: markup from a network response, a template someone else wrote, or build output you need to audit. Minify when shipping, to cut bytes on the wire - the [HTML Minifier](/dev/html-minifier) does the reverse trip." },
      { t: "p", s: "Because both only ever touch whitespace that is not rendered, they compose: minify a document, beautify it again months later, and you land back on the same structure with the same text. If a round trip does not come back clean, one of the two is editing your markup rather than formatting it." },
    ],
    faqs: [
      { q: "Does beautifying HTML change how the page looks?", a: "It should not, but many formatters do change it. The risk is entirely about inline elements: in HTML a newline collapses to a single space rather than to nothing, so putting two touching inline tags such as <span>a</span><span>b</span> on separate lines adds a visible gap. Whitespace next to a block-level element - a div, p, li, tr and so on - is discarded by the browser, so indenting those is free. A safe formatter breaks lines only at block-level boundaries and keeps anything holding text on one line." },
      { q: "Why is my whole paragraph on a single long line after formatting?", a: "Because breaking it would change it. A paragraph that mixes text with inline tags, like <p>Some <b>bold</b> text.</p>, cannot have its children pushed onto separate lines without inserting line breaks into the sentence, and those line breaks become spaces on the page. The same applies to a row of links in a nav or a set of badges in a div. The trade is deliberate: a long line in your editor, in exchange for a page that renders exactly as it did before." },
      { q: "Can a formatter add the closing tags my HTML is missing?", a: "It can, and it should not do it silently. A bare li, p, tr, td or option is valid HTML - the specification makes those closing tags optional, and browsers close them by rule when the next sibling or the parent's end tag appears. A formatter needs those rules to indent correctly, but inserting the tags is an edit rather than a formatting change, and in a framework template it alters what gets parsed. The better behaviour is to indent them correctly, tell you how many were found, and leave your markup exactly as it was." },
    ],
  },
  // ------------------------------------------------------------- SORT-LINES
  {
    slug: "how-to-sort-a-list-alphabetically",
    title: "How to Sort a List Alphabetically (and Why Your Sort Put Banana First)",
    h1: "How to Sort a List Alphabetically",
    desc: "How to sort a list alphabetically online, and why most sorters get it wrong: capitals sorted before lowercase, item10 before item9, accented words dumped past Z. What real dictionary order does instead.",
    category: "text",
    tool: { slug: "sort-lines", title: "Sort Lines" },
    updated: "2026-09-04",
    body: [
      { t: "p", s: "Sorting a list alphabetically sounds like the most solved problem in computing, and then you paste one in and get back Banana, Cherry, apple. Or item1, item10, item2. Or every accented surname dumped in a clump after Z. That is not a bug in your list - it is what almost every online sorter does, because the code behind it is one line long and that line is not sorting alphabetically at all. Here is what it does instead, and how to get the order you expected with [Sort Lines](/text/sort-lines)." },

      { t: "h2", s: "Why your sorted list came back wrong" },
      ...
      { t: "p", s: "Feed it banana, Apple, Cherry and you get Apple, Cherry, banana. That looks almost right, which is what makes it easy to ship - until one name is typed in lowercase and the list splits into two alphabets." },
      { t: "p", s: "It gets worse past ASCII. An accented character has a far higher code number than any plain letter, so a code-order sort files Arger - spelled with an A-umlaut - after Zug rather than next to Apfel. A list of European names comes back scrambled, and a non-Latin script is ordered by the internal numbering of the character set, which matches nothing a reader would recognise." },

      { t: "h2", s: "Alphabetical order is a property of a language, not of the alphabet" },
      { t: "p", s: "You cannot patch this by lowercasing everything first, because alphabetical order is a per-language convention and the languages disagree." },
      { t: "ul", items: [
      ...
      { t: "p", s: "Browsers already ship all of this: the Unicode collation data behind Intl.Collator is the same table your operating system uses to sort a folder of files, and it is what [Sort Lines](/text/sort-lines) sorts with. The Language dropdown picks whose rules apply - leave it on English unless the list is in another language." },
      { t: "p", s: "Collation also handles case the way a dictionary does: apple and Apple are the same word and sort together instead of in two blocks. Tick Case sensitive only if capitals must count as different - and even then they stay adjacent." },

      { t: "h2", s: "The item9 and item10 problem" },
      { t: "p", s: "The second thing character comparison gets wrong is numbers inside text. item10 and item9 are decided at the fifth character, 1 against 9; 1 is lower, so item10 wins, and twelve files come back as 1, 10, 11, 12, 2, 3 - as anyone with a folder of numbered files has seen." },
      { t: "p", s: "The fix is natural order: when a comparison reaches a run of digits on both sides, compare the run as one number instead of digit by digit. Then 9 beats 10 and the list reads item1, item2, item9, item10. That is the Read digits as numbers option, on by default because it is what people mean nearly every time." },

      { t: "h2", s: "How to sort a list alphabetically" },
      { t: "ol", items: [
        "Open [Sort Lines](/text/sort-lines) and paste your list, one item per line. For a run-on like red, green, blue, set Items are separated by to Commas.",
      ...
      { t: "h2", s: "What sorting does and does not change" },
      { t: "p", s: "Every option that affects how items are compared builds a sort key behind the scenes and leaves the line alone. Ignoring case does not lowercase the output; ignoring a leading The - which files The Godfather under G, as a library would - does not delete the word. What comes out is what you put in, reordered." },
      { t: "p", s: "Two things do change content, and both are reported. Each line is trimmed, so copied indentation does not decide the order. And blank lines are dropped - the one that quietly matters, because nearly every pasted list ends with a newline, and keeping it puts an empty row at the top. To strip repeats without reordering, use [Remove Duplicate Lines](/text/remove-duplicate-lines)." },

      { t: "h2", s: "When you actually want character order" },
      { t: "p", s: "Code-point order is the wrong default, but it is not useless - it is a different question. It is what sort gives you in a Unix shell under LC_ALL=C, what a database returns from ORDER BY on a binary collation, and how version control orders file names; comparing a list against one of those in dictionary order shows differences that are not really there. That is what the Code point (exact) mode is for, and it deliberately ignores the case and article options, because the point of asking for it is to reproduce something exactly." },

      { t: "h2", s: "Sort your list now" },
      { t: "p", s: "If a sort has handed you back Banana, Cherry, apple, it was comparing character codes and calling it alphabetical. Paste your list into [Sort Lines](/text/sort-lines) for real dictionary order, numbers inside text read as numbers, and blanks and duplicates handled the way you would by hand - free, in your browser, and your list never leaves your device." },
    ],
    faqs: [
      { q: "Why does my sorted list put all the capital letters first?", a: "Because the sort is comparing characters by their numeric code rather than alphabetically. Every capital ASCII letter has a lower code number than every lowercase one - A is 65 and a is 97 - so a plain sort produces all the capitalised words first, then all the lowercase ones. A collation-based sort treats apple and Apple as the same word and keeps them together, which is what a dictionary does and what Sort Lines does by default." },
      { q: "How do I sort file names so that item9 comes before item10?", a: "Use natural order, which is the Read digits as numbers option and is on by default. Character-by-character comparison decides item9 against item10 at the 1 versus the 9 and files item10 first; natural order compares the whole run of digits as a number instead, giving item1, item2, item9, item10. It is the same rule your file manager uses when it lists a numbered series correctly." },
      { q: "Can it sort a comma-separated list instead of one item per line?", a: "Yes. Set Items are separated by to Commas and paste the list exactly as you have it, such as banana, apple, cherry. Each item is trimmed of surrounding spaces, sorted, and joined back together with a comma and a space, so the result drops straight back into a sentence, a config value or a spreadsheet cell." },
    ],
  },
  {
    slug: "how-to-convert-binary-to-decimal",
    title: "How to Convert Binary to Decimal (and Back Again)",
    h1: "How to Convert Binary to Decimal",
    desc: "How to convert binary to decimal by adding place values, and decimal to binary by dividing by two. With hex, fractions, and the rounding bug that makes most online converters wrong above 9 quadrillion.",
    category: "convert",
    tool: { slug: "base-converter", title: "Base Converter" },
    updated: "2026-09-05",
    body: [
      { t: "p", s: "Every number you write down is already in a base. Writing 214 means two hundreds, one ten and four ones - the digits are the same three symbols in any order, and what gives them value is the column they sit in. Binary works exactly the same way, with one change: the columns go up by two instead of by ten, so there are only two digits to write with. Once you see that, converting between bases stops being a trick to memorise and becomes arithmetic you can do on paper. Here is how to do it by hand, and where to hand it to [Base Converter](/convert/base-converter) instead." },

      { t: "h2", s: "What the columns are worth" },
      { t: "p", s: "In decimal the column values are powers of ten, read right to left: 1, 10, 100, 1000. In binary they are powers of two: 1, 2, 4, 8, 16, 32, 64, 128. A bit can only be 0 or 1, so each column is either counted or it is not. There is no other case, which is why the arithmetic is easy and why machines use it." },
      { t: "p", s: "Hexadecimal, base 16, climbs faster: 1, 16, 256, 4096. It needs sixteen digits and there are only ten numerals, so a to f stand in for ten to fifteen." },

      { t: "h2", s: "Binary to decimal: add up the columns that are set" },
      { t: "p", s: "Write the place values above the bits, right to left, then add the ones with a 1 under them. Take 11010110:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "That is the whole method. Columns with a 0 contribute nothing, so you are only ever adding a handful of numbers, each a power of two you can double your way to from 1." },

      { t: "h2", s: "Decimal to binary: divide by two and read the remainders upward" },
      { t: "p", s: "Going back, divide by two repeatedly and keep each remainder. The remainders are the bits, but they arrive backwards - the first is the rightmost bit - so read them bottom to top. Converting 214 back:" },
      { t: "ol", items: [
      ...
      { t: "p", s: "Reading the remainders from the bottom up gives 11010110, which is where we started. The same procedure works for any base - divide by the base instead of by two, and use the remainders as digits." },

      { t: "h2", s: "Hexadecimal is just binary in groups of four" },
      { t: "p", s: "Sixteen is two to the fourth power, and that one fact makes hex and binary interchangeable without any arithmetic at all. Four bits hold exactly the numbers 0 to 15, which is exactly one hex digit, so you can slice a binary number into groups of four from the right and convert each group on its own." },
      { t: "p", s: "Split 11010110 into 1101 and 0110. The first is 8 + 4 + 1 = 13, which is d. The second is 4 + 2 = 6. So the number is d6 in hex - and checking against decimal, 13 times 16 plus 6 is 214, the same value again. This is the real reason hex is everywhere in computing: it is a shorthand for bits that a person can actually read, where one hex digit always means one nibble of four bits and a pair of them always means one byte." },

      { t: "h2", s: "Numbers with a point after them" },
      { t: "p", s: "Fractional columns carry on past the point, halving each time: 0.5, 0.25, 0.125, 0.0625. So 10.625 in binary is 1010.101, because 0.625 is 0.5 plus 0.125 - the first and third fractional columns - and the whole part 10 is 1010." },
      { t: "p", s: "But a fraction that ends neatly in one base often runs forever in another, and this catches people out. One tenth is a perfectly tidy 0.1 in decimal, and in binary it never terminates: it runs 0.0001100110011... with 0011 repeating for as long as you care to write it. The rule is that a fraction terminates in a given base only when the bottom of the fraction is built from that base's own prime factors. Ten is two times five, so halves, quarters, fifths and tenths all end cleanly in decimal. Two only has itself, so only halves, quarters and eighths end cleanly in binary - a fifth or a tenth cannot." },
      { t: "p", s: "This is why a program can add 0.1 and 0.2 and get 0.30000000000000004: it never had a tenth, only the nearest binary approximation of one." },

      { t: "h2", s: "Why most online converters go wrong on big numbers" },
      { t: "p", s: "There is a bug so widespread in small conversion tools that it is worth knowing how to spot. The usual one-line implementation reads the input with a function that hands back an ordinary floating point number, and a floating point number holds whole numbers exactly only up to 9,007,199,254,740,992 - two to the power 53, roughly nine quadrillion. Above that, values are rounded to the nearest representable one before anything is even converted." },
      { t: "p", s: "The failure is not a small one at the edges. Ask such a converter for ffffffffffffffff - sixteen f characters, the largest unsigned 64-bit value - in binary, and it answers with a 1 followed by 64 zeros. The correct answer is 64 ones. Every single bit is wrong, and there is one digit too many, because the value was rounded up to the next power of two on the way in and nothing downstream knew." },
      { t: "p", s: "That range covers most of what people actually paste into a base converter: 64-bit hashes, database and snowflake IDs, checksums, register dumps, 128-bit UUIDs. If you want to check a tool before you trust it, give it 9007199254740993 - two to the 53rd plus one - and ask for decimal back. A converter that rounds will hand you 9007199254740992, one short." },

      { t: "h2", s: "Letting the tool do it" },
      { t: "p", s: "[Base Converter](/convert/base-converter) does all of the above at once: type a number in any base from 2 to 36 and binary, octal, decimal and hex all appear together, with a box to add any other base you need. It runs on arbitrary-precision integers, so length is not a limit and nothing is rounded on the way through. Fractions are expanded exactly, with a repeating run shown in brackets - 0.1 in decimal comes out as 0.0(0011) in binary rather than being quietly truncated into a number that looks exact and is not." },
      { t: "p", s: "Negative numbers get a two's complement panel underneath - the form a computer actually stores them in at 8, 16, 32 and 64 bits - and a value too large for a width is reported rather than silently wrapped. Spaces, underscores and commas are ignored, so a grouped value out of a spreadsheet or a hex dump pastes straight in. Everything runs in your browser, so an internal ID never leaves your machine." },
    ],
    faqs: [
      { q: "How do I convert binary to decimal quickly in my head?", a: "Learn the powers of two up to 128 and read the bits from the left, doubling as you go: start at 0, and for each bit double what you have and add the bit. For 11010110 that runs 1, 3, 6, 13, 26, 53, 107, 214. It is the same answer as adding the place values, but you never have to remember which column you are in - you just double and add, once per bit, left to right." },
      { q: "Why does 0.1 have no exact binary form?", a: "Because a fraction only terminates in a base when its denominator is built from that base's prime factors. One tenth is 1/10, and 10 factors into 2 and 5. Binary only has the factor 2, so the 5 can never be cleared and the expansion repeats forever as 0.0(0011). Decimal has both 2 and 5, which is why a tenth is exact there and why a third - denominator 3 - is not exact in either." },
      { q: "What is the largest number this can convert?", a: "There is no fixed limit. The conversion uses arbitrary-precision integers rather than floating point numbers, so a 64-bit hash, a 128-bit UUID or a 200-digit value all convert exactly. That is the difference from converters built on the usual one-line approach, which silently round anything above about nine quadrillion before converting it." },
    ],
  },
  // ------------------------------------------------------------- XML-TO-JSON
  {
    ...
    h1: "How to Convert XML to JSON (And What Gets Lost)",
    desc: "How to convert XML to JSON: how attributes and text are mapped, why a one-item list silently changes shape, what happens to namespaces, entities and mixed content, and how to keep IDs and versions exact - with a worked example.",
    category: "convert",
    tool: { slug: "xml-to-json", title: "XML to JSON" },
    updated: "2026-09-06",
    body: [
      { t: "p", s: "XML is what older systems speak: SOAP responses, RSS feeds, bank statements, government APIs, Android layouts, Maven builds. JSON is what everything written this decade expects. So converting between them is routine - and unlike JSON to YAML, there is no single correct answer, because XML can express several things JSON simply cannot. This guide explains how the [XML to JSON](/convert/xml-to-json) converter maps one onto the other, which decisions you have to make yourself, and which parts of a document cannot survive the trip at all." },

      { t: "h2", s: "Why there is no official mapping" },
      { t: "p", s: "JSON has objects, arrays, strings, numbers, booleans and null. XML has all of that plus four things with no JSON equivalent:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Paste it into the [XML to JSON](/convert/xml-to-json) converter and you get this:" },
      { t: "ul", items: [
        "{",
        "  \"order\": {",
        "    \"@id\": \"ORD-0098\",",
      ...
      { t: "p", s: "Four things happened there. The id attribute became @id. The two item elements became an array. Each item had both an attribute and text, so its text went into #text. And Tom &amp; Jerry Ltd came back as Tom & Jerry Ltd, because &amp; is an XML entity and JSON has no entities - leaving it as written would have been a bug. Three of those four are worth understanding properly." },

      { t: "h2", s: "The one-item list problem" },
      { t: "p", s: "This is the single biggest trap in XML to JSON conversion, and it does not show up until production. XML has no way to mark an element as a list, so a converter can only count: two item tags become an array, one item tag becomes a plain object. Delete the second item from the example above and the same document converts to this instead:" },
      { t: "ul", items: [
        "\"items\": { \"item\": { \"@sku\": \"A1\", \"#text\": \"Widget\" } }",
      ]},
      { t: "p", s: "The key is no longer an array. Code that did items.item.map(...) now throws, on the first order that happens to contain a single line. The shape of your JSON is being decided by your data rather than by your schema, and the failure only appears once real traffic produces a one-element list." },
      { t: "p", s: "The fix is to stop counting. Switch on Always arrays and every child element becomes an array whether it repeats or not, so items.item is a one-element array in the case above and your loop works in both. It is more verbose to read, and that is the trade: use the default when you are eyeballing a document, and Always arrays whenever code will consume the output. The converter tells you which situation you are in - whenever repeat-counting actually decided a key's shape, it says so." },

      { t: "h2", s: "Attributes and why they need a prefix" },
      { t: "p", s: "An attribute and a child element are allowed to share a name. This is valid XML:" },
      { t: "ul", items: ["<book id=\"1\"><id>2</id></book>"] },
      { t: "p", s: "Without a prefix both want the key id, and one silently overwrites the other. That is why the default is @id rather than id, and you can switch it to _, $ or nothing at all. If you do choose no prefix and a genuine collision occurs, the converter names the key rather than quietly dropping a value." },
      { t: "p", s: "The same reasoning covers an element's own text when it also has attributes: it has to go somewhere, and #text is the conventional place. An element with no attributes and no children skips all of this and becomes a plain string, which is why name above is just a string rather than an object." },

      { t: "h2", s: "Keeping numbers, IDs and versions intact" },
      { t: "p", s: "XML is untyped: every value is text, and any typing is an assumption the converter makes. That assumption is where data gets destroyed. Turn a zip code of 007 into a number and it becomes 7. Turn a version of 1.50 into a number and it becomes 1.5. Turn a 20-digit order ID into a number and JavaScript rounds it to a double, replacing the last few digits with zeros." },
      { t: "p", s: "So values stay strings by default - which is lossless, and why 007 survived the example above. When you do switch type conversion on, a number is only written if its digits come back identical, so 8080 becomes a number while 007, 1.50, +1 and 12345678901234567890 all stay text. true and false become booleans under the same switch." },

      { t: "h2", s: "Namespaces, SOAP and the soap: prefix" },
      { t: "p", s: "A SOAP response is mostly prefixes, and they make the JSON painful to navigate - every key is soap:Envelope or m:GetPrice, which means bracket syntax everywhere in your code. Turning on Remove namespace prefixes maps soap:Envelope to Envelope and drops the xmlns declarations along with it, since the prefixes they declare no longer appear. If two different names would collide once stripped - a:id and b:id both becoming id - you are told rather than losing one." },

      { t: "h2", s: "What cannot survive" },
      { t: "p", s: "Some things have no JSON form at all, and the honest answer is to report them. Comments, processing instructions and the XML declaration are dropped and counted. Mixed content - an element holding text and tags together - loses the order between them: <p>Hello <b>world</b>!</p> becomes a b key and a #text of \"Hello !\", which is enough for data but not for a document. And a named entity nobody declared, &nbsp; being the usual one, is left exactly as written, because only &lt; &gt; &amp; &apos; and &quot; exist in XML without a DTD and inventing HTML's table would be putting content into your data that was never there." },

      { t: "h2", s: "It runs entirely in your browser" },
      { t: "p", s: "API payloads carry customer names, order values, hostnames and tokens, so where a conversion happens is not a detail. The [XML to JSON](/convert/xml-to-json) converter parses and converts inside your browser tab - nothing is uploaded, so you can paste a live response safely. If the XML will not parse, the [XML Formatter](/dev/xml-formatter) reports the exact line and column of the problem, and the [JSON Formatter](/dev/json-formatter) will pretty-print or validate the result once you have it." },
    ],
    faqs: [
      { q: "Why did my array turn into a single object?", a: "Because that element appeared only once. XML cannot mark an element as a list, so a converter counts occurrences: repeated names become arrays, a single name becomes a plain object. It means the shape of your JSON depends on the data, and code that loops over the list breaks the first time a real record has one entry. Switch on Always arrays to get an array for every child element regardless of how many times it appears - that is the setting to use whenever code, rather than a person, will read the output." },
      { q: "Will a leading zero or a long ID be damaged?", a: "No. Every value stays a string unless you turn type conversion on, so 007 stays \"007\" and a 20-digit ID keeps all twenty digits. With conversion on, a number is only written when its digits come back exactly the same, so 8080 becomes a number while 007, 1.50 and anything past 2^53 stay text. Converters built on parseFloat get all three of those wrong." },
      { q: "Is my XML uploaded anywhere?", a: "No. The XML is parsed and converted entirely in your browser and nothing is sent to a server, so a SOAP response, an API payload or a config file holding hostnames and keys never leaves your device." },
    ],
  },
  // ---------------------------------------------------------------- JSON to XML
  {
    ...
    h1: "How to Convert JSON to XML (Without Losing Your Data)",
    desc: "How to convert JSON to XML: the mapping rules, a worked example, why keys get renamed, how arrays and nulls are handled, and the escaping mistakes that corrupt values.",
    category: "convert",
    tool: { slug: "json-to-xml", title: "JSON to XML" },
    updated: "2026-09-07",
    body: [
      { t: "p", s: "Converting JSON to XML sounds like a formatting change but is really a translation between two formats that do not describe the same thing. JSON has arrays, nulls and keys that can be any string. XML has attributes, exactly one root element, a narrow rule about element names, and no concept of a list or an empty value. Every converter must choose where those do not line up, and the difference between a good one and a bad one is whether it tells you what it chose. This guide covers the mapping, a worked example, and the places a value quietly changes." },

      { t: "h2", s: "The mapping in five rules" },
      { t: "p", s: "Almost every JSON-to-XML converter, including the [JSON to XML](/convert/json-to-xml) tool here, works from the same five rules:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "The last two rules are a convention rather than a standard, but they are the most widely used one, and they matter because they are reversible: read backwards, they are how an XML to JSON converter decides what was an attribute." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "A small order payload:" },
      { t: "p", s: "{ \"order\": { \"@id\": \"A-4471\", \"customer\": { \"name\": \"Priya Nair\", \"city\": \"Pune\" }, \"line\": [ { \"@sku\": \"TSH-01\", \"qty\": \"2\", \"price\": \"499.00\" }, { \"@sku\": \"MUG-07\", \"qty\": \"1\", \"price\": \"249.50\" } ] } }" },
      { t: "p", s: "Applying the rules gives:" },
      { t: "p", s: "<order id=\"A-4471\"> <customer> <name>Priya Nair</name> <city>Pune</city> </customer> <line sku=\"TSH-01\"> <qty>2</qty> <price>499.00</price> </line> <line sku=\"MUG-07\"> <qty>1</qty> <price>249.50</price> </line> </order>" },
      { t: "p", s: "Three things worth noticing: the single top-level key became the root element, the two-entry line array became two <line> elements rather than a wrapper, and the price 249.50 kept its trailing zero - not automatic." },

      { t: "h2", s: "Choosing the root element" },
      { t: "p", s: "An XML document must have exactly one root element and JSON has no such rule. When your JSON is an object with a single top-level key, that key names the root and nothing is invented. When it has several keys, or is an array, a wrapper has to be added:" },
      { t: "p", s: "{\"first name\":\"Priya\",\"order id\":4471,\"tags\":[],\"note\":null} becomes <root> <first_name>Priya</first_name> <order_id>4471</order_id> <note/> </root>" },
      { t: "p", s: "That one small input carries four losses: two keys renamed, one array vanished, one null now indistinguishable from an empty string. A converter worth using names all four; the next three sections take them in turn." },

      { t: "h2", s: "Keys that XML will not accept" },
      { t: "p", s: "An XML element name cannot start with a digit or contain a space or most punctuation. JSON keys have no such restriction, so \"first name\" has to become <first_name> and \"2024\" has to become <_2024>. There is no way round this - it is the format, not the tool - so the only question is whether you are told." },
      { t: "p", s: "The case to watch for is subtler. If two keys in the same object sanitise to the same name, they become two elements sharing a name, and any reader hands that back as a two-item array rather than two fields:" },
      { t: "p", s: "{\"first name\":\"a\",\"first_name\":\"b\"} becomes <first_name>a</first_name><first_name>b</first_name>" },
      { t: "p", s: "Two distinct fields have silently merged into a list, which is worth an explicit warning rather than a footnote. The same name under two different parents is not a collision, so flagging those is just noise." },

      { t: "h2", s: "Arrays: the two cases that bite" },
      { t: "p", s: "Repeated siblings work for an ordinary list, but two array shapes have no clean XML form. An empty array produces no elements at all, so \"tags\": [] simply disappears and becomes indistinguishable from a missing key. And an array sitting directly inside another has no key to take a name from, so its entries need one invented:" },
      { t: "p", s: "{\"grid\":[[\"a\",\"b\"],[\"c\"]]} becomes <root> <grid> <item>a</item> <item>b</item> </grid> <grid> <item>c</item> </grid> </root>" },
      { t: "p", s: "Here <item> came from the converter, not your data. It is the right answer - flattening both levels into one run of siblings would destroy the structure - but you should know a name was invented and be able to change it." },

      { t: "h2", s: "What to do with null" },
      { t: "p", s: "XML has no null, so there are three honest options and the right one depends on who reads the file. An empty tag, <note/>, is simplest and reads back as an empty string. The attribute xsi:nil=\"true\" marks the element as genuinely absent, which is what a schema-aware reader expects, at the cost of one namespace declaration on the root. Or the element can be left out, so the key is simply missing. Guessing is the one thing a converter should not do here." },

      { t: "h2", s: "The escaping that most converters get wrong" },
      { t: "p", s: "Ampersands and angle brackets obviously need escaping. Two less obvious rules decide whether your values survive. First, a literal > has to be escaped too: the sequence ]]> is a parse error in XML content, so a value containing it produces a document that will not load. Second, an XML parser normalises whitespace before your code sees it - a carriage return becomes a line feed everywhere, and inside an attribute value a tab or newline becomes a plain space - so those must be written &#13;, &#9; and &#10; or they are silently lost, and only on the far side of the parser, which makes it hard to trace." },
      { t: "p", s: "Some characters cannot be carried at all: most control codes are invalid in XML 1.0, escaped or not, so a converter that writes &#1; produces a file no parser will read." },

      { t: "h2", s: "Numbers, IDs and trailing zeros" },
      { t: "p", s: "This is the failure that costs real money. A converter written the quick way parses your JSON with JSON.parse, routing every number through a double-precision float. Anything past about 9 quadrillion is rounded, so a 20-digit order ID loses its last digits, and 1.50 becomes 1.5 because a float has no memory of a trailing zero. Version pins, invoice totals and database IDs are exactly what people paste into a converter, and all three are damaged." },
      { t: "p", s: "The fix is to keep each number as the text it was written as and emit that unchanged, which is what the [JSON to XML](/convert/json-to-xml) converter does - 1.50 stays 1.50 and every digit of a long ID survives. Evaluating another tool? Paste a 20-digit number in and check the output first." },

      { t: "h2", s: "Going back the other way" },
      { t: "p", s: "Because the attribute and text conventions are symmetrical, output from one direction feeds straight into the other: run a document through [XML to JSON](/convert/xml-to-json) and back through [JSON to XML](/convert/json-to-xml) and you land on the same tree. If the XML will not parse, the [XML Formatter](/dev/xml-formatter) gives the exact line and column, and the [JSON Formatter](/dev/json-formatter) validates the JSON before you convert it." },

      { t: "h2", s: "It runs entirely in your browser" },
      { t: "p", s: "API payloads and config files carry customer names, order values, hostnames and tokens, so where a conversion happens is not a detail. This converter runs inside your browser tab and nothing is uploaded, so you can paste a live response without it leaving your device." },
    ],
    faqs: [
      { q: "How do I make a JSON key become an XML attribute?", a: "Prefix it with @. The key \"@id\": \"A-4471\" becomes id=\"A-4471\" on the element's opening tag instead of a child element, and the key #text becomes the element's own text so it can carry attributes and a value together. You can switch the marker to _ or $, or turn attributes off entirely so that every key becomes an element. The convention matters because it is reversible - the same marker read backwards is how an XML to JSON converter decides what was an attribute, so a document can go both ways without losing anything." },
      { q: "Why does my JSON need a wrapper element?", a: "Because XML allows exactly one root element and JSON does not promise one. If your JSON is an object with a single top-level key, that key names the root and nothing is invented. If it has several top-level keys, or if it is an array, there is no single name to use and a wrapper - <root> by default, though you can rename it - has to be added around everything. A converter should tell you when it added one, since the wrapper is not part of your data." },
      { q: "Will a long ID or a price like 1.50 survive the conversion?", a: "It depends entirely on the tool. Any converter that parses your JSON with JSON.parse first pushes every number through a double-precision float, which rounds integers past roughly 9 quadrillion and drops trailing zeros, so a 20-digit order ID loses its last digits and 1.50 comes out as 1.5. A converter that keeps each number as the text you wrote and emits that text unchanged does not have this problem. It is worth testing with a 20-digit number before trusting any converter with invoice totals or database IDs." },
    ],
  },
  {
    slug: "how-to-verify-a-webhook-signature",
    title: "How to Verify a Webhook Signature",
    h1: "How to Verify a Webhook Signature (and Why Yours Does Not Match)",
    desc: "How webhook signatures work, how to check one with HMAC-SHA256, and the four reasons a signature does not match: key encoding, algorithm, message bytes and comparison.",
    category: "dev",
    tool: { slug: "hmac-generator", title: "HMAC Generator" },
    updated: "2026-09-08",
    body: [
      { t: "p", s: "A webhook endpoint is a URL on the public internet that accepts POST requests. Anything can send it one, so your endpoint has no way of knowing a payment really succeeded unless the sender proves it. The proof is the signature header, and verifying it is the only thing standing between your handler and anyone who guesses the URL." },
      { t: "p", s: "The mechanism is HMAC. You and the provider share a secret; the provider computes an HMAC over the exact request body and sends the result in a header, and you recompute it and check the two agree. Because only the two of you hold the secret, a matching tag means the body came from the provider and arrived unaltered. Here is how that works, a worked example you can reproduce, and the four reasons the numbers usually fail to line up." },

      { t: "h2", s: "What the signature actually is" },
      { t: "p", s: "HMAC takes two inputs, a secret key and a message, and returns a fixed-length tag. It is not the same as hashing the secret joined to the message. HMAC runs the hash twice, over two different paddings of the key, and that structure is what makes it safe: against a naive SHA256(secret + message), an attacker who never learns the secret can still append data to your message and compute a valid digest for the longer version. This is a length-extension attack, it works against SHA-256 and MD5 alike, and HMAC exists precisely to stop it. If you want the background on the hash functions underneath, see [MD5 vs SHA-256](/guides/md5-vs-sha256-hashing-explained)." },
      { t: "p", s: "Providers wrap the tag differently: GitHub sends sha256=<hex>, Stripe sends t=<timestamp>,v1=<hex> with the timestamp itself part of what gets signed, and Shopify sends bare Base64. Underneath they are the same operation in different spellings." },

      { t: "h2", s: "A worked example you can reproduce" },
      { t: "p", s: "Take the secret whsec_2f8a1d and this body, exactly 30 bytes with no trailing newline:" },
      { t: "p", s: "{\"id\":\"evt_001\",\"amount\":2000}" },
      { t: "p", s: "HMAC-SHA256 of that body under that key is:" },
      { t: "p", s: "04d7b16b35f8babd0563096264230506406bbb4a2eded5668cdb97eaed46bdd3" },
      { t: "p", s: "Paste all three into the [HMAC Generator](/dev/hmac-generator) - algorithm SHA-256, the secret as Text, the body in the message box - and you should get exactly that, along with the same value in uppercase hex, Base64 and Base64url. The command line agrees: printf '%s' '{\"id\":\"evt_001\",\"amount\":2000}' | openssl dgst -sha256 -hmac \"whsec_2f8a1d\" prints the same 64 characters, as does Python's hmac.new(b'whsec_2f8a1d', body, hashlib.sha256).hexdigest()." },
      { t: "p", s: "Now change one invisible thing. Add a single newline to the end of the body and the tag becomes 6428eb25b250a7482e3dfd7e7df195fe48fd46d0ae07c20d42f3496ea63e02b9 - not similar, completely different. That is what makes HMAC useful and also what makes it maddening to debug: no partial credit, and no clue which end was wrong." },

      { t: "h2", s: "The four reasons a signature does not match" },
      { t: "p", s: "In practice, essentially every mismatch is one of four things." },
      { t: "ol", items: [
        "The key is being read differently at the two ends. This is the big one. A secret written 2f8a1d4b is eight characters of text, but it is also four bytes of hex, and the two readings are different keys. Over the body above, the text reading gives d35b699f0a743cbf9d54b42a7a43d064dc6777c10dafa1cb253ee4223368b8fd and the hex reading gives 6bd29bb767a595979ec6da58f122ed248af0627b19d578402be272d5b701fc95. Nothing about the secret tells you which one the provider means, so check the documentation - and note that openssl's -hmac flag always takes the key as text, so a hex key needs -mac hmac -macopt hexkey:2f8a1d4b instead.",
        ...
        "The message bytes are not what you think. Sign the raw request body, before any JSON parsing. If your framework parses the body and you re-serialise it to sign, key order, spacing and unicode escaping can all shift and the tag moves with them. A trailing newline picked up from a file or a shell pipeline does the same, and so do CRLF line endings on a multi-line payload. Stripe and several others also sign a timestamp concatenated with the body rather than the body alone.",
        "The comparison itself is wrong. Hex is case-insensitive, so an uppercase digest and a lowercase one are the same value. Base64 and Base64url encode the same bytes with a different alphabet, and Base64 padding is often stripped. Decode both sides to bytes and compare those, rather than comparing strings.",
      ] },
      { t: "p", s: "Rather than working through that list by hand, paste the signature you were sent into the compare box on the [HMAC Generator](/dev/hmac-generator). It tries every algorithm, every reading of your secret and the usual transit changes, and names the combination that reproduces the signature - which tells you what to change in your code." },

      { t: "h2", s: "Compare in constant time" },
      { t: "p", s: "Once you have both tags, do not compare them with == or ===. An ordinary comparison returns as soon as it finds two bytes that differ, so a guess whose first byte is right takes measurably longer than one whose first byte is wrong. That timing difference is enough to recover a valid signature one byte at a time, without ever knowing the secret." },
      { t: "p", s: "Every runtime ships a fixed-time comparison: crypto.timingSafeEqual in Node, hmac.compare_digest in Python, hash_equals in PHP, subtle.ConstantTimeCompare in Go, Rack::Utils.secure_compare in Ruby. Use one. Note that Node's timingSafeEqual throws if the buffers differ in length, so check the length first and reject rather than letting it raise." },

      { t: "h2", s: "What a valid signature does not tell you" },
      { t: "p", s: "A correct signature proves the body came from someone holding the secret and was not modified. It does not prove the request is fresh. Anyone who captures a signed request can send the identical bytes again, and the signature will still verify - that is a replay. This is why providers sign a timestamp alongside the body and expect you to reject anything older than a few minutes, and why the timestamp has to be inside the signed data rather than sent beside it." },
      { t: "p", s: "Nor does it make your handler idempotent. Providers deliberately re-send events they did not get a 2xx for, so the same correctly signed event will arrive more than once. Record the event id and ignore ones you have already processed." },
      { t: "ul", items: [
      ...
      { t: "h2", s: "Checking it without sending anything anywhere" },
      { t: "p", s: "A signing secret is a credential, so where you test one matters. The [HMAC Generator](/dev/hmac-generator) computes everything inside your browser tab and uploads nothing. Even so, a live production secret is worth rotating rather than pasting into any tool, and a throwaway value is enough when you only need the shape of a signature. For a plain unkeyed digest there is the [Hash Generator](/dev/hash-generator), and the [Base64 Encoder](/dev/base64-encoder) will decode a Base64 signature into its bytes." },
    ],
    faqs: [
      { q: "How do I know whether my secret is text, hex or Base64?", a: "Usually only the provider's documentation says, because the secret itself gives no clue - 2f8a1d4b is a perfectly good eight-character string and a perfectly good four-byte hex value, and the two produce entirely unrelated tags. Two hints help. A secret that is all hex digits with an even length is often meant as hex, and one that ends in = is almost certainly Base64. If the documentation is silent, compute the tag under all three readings and see which reproduces a signature you have already received; the compare box on the HMAC Generator does exactly that sweep for you." },
      { q: "Why does my signature change when the JSON looks identical?", a: "Because HMAC signs bytes, not meaning. If you parse the request body into an object and then re-serialise it before signing, your serialiser may order keys differently, put spaces after colons, escape non-ASCII characters as \\u sequences, or drop a trailing newline that was in the original. Every one of those changes the bytes and therefore the tag, while leaving the JSON semantically the same. Capture the raw body as it arrives, verify against that, and only parse afterwards." },
      { q: "Is HMAC-SHA1 or HMAC-MD5 safe if my provider still uses one?", a: "It is not the emergency it sounds like. MD5 and SHA-1 are broken for collision resistance, which is what disqualifies them for certificates and digital signatures, but HMAC is a different setting and no practical forgery against HMAC-MD5 or HMAC-SHA1 is known. That is why they still appear in older payment, telecom and AWS Signature V2 integrations, and why the HMAC Generator supports them. For anything you control, choose HMAC-SHA256; for an integration you do not control, keep verifying what the provider sends and treat the algorithm as a reason to ask about their upgrade path." },
    ],
  },
  // -------------------------------------------------------- HTML entities
  {
    slug: "how-to-encode-and-decode-html-entities",
    title: "How to Encode and Decode HTML Entities",
    h1: "How to Encode and Decode HTML Entities (and Why &amp;lt; Keeps Showing Up)",
    desc: "How HTML entities work: which characters must be encoded, when to use named, decimal or hex references, why &copy without a semicolon still renders, and how to fix double-encoded text.",
    category: "dev",
    tool: { slug: "html-entity-encoder", title: "HTML Entity Encoder" },
    updated: "2026-09-09",
    body: [
      { t: "p", s: "An HTML entity - properly, a character reference - writes a character using nothing but plain ASCII. &copy; is a copyright sign, &#8212; an em dash, &lt; a less-than sign that will not be mistaken for the start of a tag. The whole system hangs off one character, the ampersand, and almost every entity problem is really a problem with that ampersand: one that should have been encoded and was not, or one that was encoded twice." },

      { t: "h2", s: "The five characters that actually need encoding" },
      { t: "p", s: "Modern advice is much shorter than the tables on older pages. If your file is served as UTF-8 - and it should be - accents, curly quotes, em dashes and emoji can all sit in the source as themselves. Only five characters have to be encoded, and only where a parser would read them as markup:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "The apostrophe is worth a note. &apos; is an XML entity that HTML did not define until HTML5, so it is undefined in HTML 4 and in XHTML served as text/html. The numeric &#39; is the one spelling every parser has always understood, which is why the [HTML Entity Encoder](/dev/html-entity-encoder) writes that." },

      { t: "h2", s: "Named, decimal or hex" },
      { t: "p", s: "The same character has three spellings. A copyright sign can be written &copy;, &#169; or &#xA9;, and a browser treats all three identically. Which to choose depends on what will read the file, not on taste." },
      { t: "p", s: "A named reference is readable, and the 253 names from HTML 4.01 and XHTML have been understood by every browser since 1999. HTML5 adds around 2,000 more, mostly MathML symbols and alternate spellings such as &excl; and &NewLine;, which are fine in a browser and nowhere else." },
      { t: "p", s: "A numeric reference needs no table at all, which makes it the safe default outside HTML. XML predefines exactly five names - &amp; &lt; &gt; &quot; and &apos; - and treats any other as a parse error unless the document declares it in a DTD. So a feed containing &copy; or &mdash; fails to load in a strict reader, while &#169; and &#8212; are always fine. For a feed, a SOAP body or an SVG file, use numeric." },

      { t: "h2", s: "A worked example: the link that breaks itself" },
      { t: "p", s: "This is the failure that sends most people looking for an entity tool in the first place. You write a link with a query string:" },
      { t: "p", s: "<a href=\"/search?q=tea&copy=full\">Full copy</a>" },
      { t: "p", s: "Inside the quoted attribute this survives: the href really is /search?q=tea&copy=full. Put the same characters in the page as text - a code sample, a support article, an error message - and the browser renders ?q=tea(c)=full, with a copyright sign where &copy was. No validator will complain; the text is simply wrong on screen." },
      { t: "p", s: "The fix is one character: write &amp;copy=full and it renders as &copy=full in both places. Paste either version into the [HTML Entity Encoder](/dev/html-entity-encoder) and switch between element text and attribute value to see the two readings side by side." },

      { t: "h2", s: "Why &copy without a semicolon still renders" },
      { t: "p", s: "Because 106 names are allowed to drop it: exactly the 96 Latin-1 names covering U+00A0 to U+00FF - &nbsp, &copy, &reg, &eacute, &pound - plus &amp, &lt, &gt, &quot and six all-caps aliases. Everything else needs its semicolon, so &hellip is literal text while &hellip; is an ellipsis." },
      { t: "p", s: "There is a second half to the rule, and it is the reason query strings usually survive. Inside an attribute value, a semicolon-less name is not consumed at all if the next character is = or a letter or digit. That single exception is what keeps href=\"?a=1&copy=2\" intact while the same characters as page text become ?a=1(c)=2. It is why a good decoder asks where your text came from before it answers." },

      { t: "h2", s: "Double encoding, and how to spot it" },
      { t: "p", s: "If your page is showing &lt;b&gt; on screen instead of bold text, or your database is full of &amp;lt;, something encoded the same string twice. The first pass turned < into &lt;. The second pass saw the ampersand in &lt; and encoded that too, giving &amp;lt;." },
      { t: "p", s: "What makes this confusing is that one decode does not fix it: decoding &amp;lt; gives &lt;, which still looks encoded. The [HTML Entity Encoder](/dev/html-entity-encoder) detects this, offers to keep decoding until nothing changes, and reports how many passes it took - a useful number, because three passes means the bug in your pipeline is running three times, not once." },

      { t: "h2", s: "The characters you cannot see" },
      { t: "p", s: "Non-breaking spaces, soft hyphens, zero-width spaces and joiners, directional marks and byte order marks all look like nothing, or like an ordinary space, in every editor there is. They arrive by way of a paste from Word, a CMS field or a PDF, and they decide where a line wraps and whether a find-and-replace matches." },
      { t: "p", s: "Encoding them is the cheapest way to make them visible: a &#xA0; in the source is obvious in a way an invisible byte never is. It is the one case where encoding above plain ASCII earns its keep on a UTF-8 page." },

      { t: "h2", s: "Why &#151; decodes to an em dash" },
      { t: "p", s: "Code points 128 to 159 are C1 control characters in Unicode, so &#151; should strictly be an invisible control code. Every browser decodes it as an em dash instead, and HTML5 now requires that: a huge number of pages were authored in Windows-1252 and served as something else, so the specification defines a replacement table mapping that range to the Windows-1252 characters. &#146; is a right single quote, &#128; a euro sign, &#151; an em dash. Seeing these in your data usually means a Windows-1252 file was read as UTF-8 upstream, and that same mismatch will be corrupting characters with no entity form at all." },

      { t: "h2", s: "Encoding is not the same as sanitising" },
      ...
      { t: "p", s: "For the neighbouring jobs there are separate tools: the [String Escaper](/dev/string-escaper) covers JSON, JavaScript, XML, CSV, SQL and shell quoting, and the [URL Encoder](/dev/url-encoder) handles percent-encoding, which is a different scheme entirely and the right one for a query string value." },

      { t: "h2", s: "It runs entirely in your browser" },
      { t: "p", s: "Page fragments carry customer names, internal hostnames and order numbers, so where a conversion happens is not a detail. Encoding and decoding both run in JavaScript inside your browser tab and nothing is uploaded, so you can paste a live template or a production error message without it leaving your device." },
    ],
    faqs: [
      { q: "Do I still need to encode accents and symbols on a UTF-8 page?", a: "No. If the document declares UTF-8 and is served as UTF-8, then e-acute, an em dash, a Greek letter and an emoji can all appear as themselves, and the file will be smaller and much easier to read for it. Encoding above ASCII is worth doing in three situations: the output has to pass through a pipeline you do not trust with UTF-8, such as an old CMS field or a database column with the wrong collation; you are producing something that will be read by a strict XML parser; or you want invisible characters like a no-break space to become visible in the source. Otherwise the five markup characters are all you need." },
      { q: "Why does my text show &amp;lt; instead of a tag?", a: "It was encoded twice. Something turned < into &lt;, and then a second pass encoded that ampersand as well, leaving &amp;lt;. Decoding once gives back &lt;, which still looks encoded, so it is easy to think the decoder failed - it just needs another pass. The usual cause is a value escaped on the way into storage and escaped again by a template engine on the way out, since Jinja, Twig, ERB and JSX all escape by default. Fix the pipeline so escaping happens exactly once, at output; to clean up the data you already have, decode repeatedly until the text stops changing." },
      { q: "Can I use HTML entity names like &copy; in an RSS feed or an SVG?", a: "Not safely. Those are XML, and XML predefines only five entity names - &amp; &lt; &gt; &quot; and &apos; - so any other name is a parse error unless the document declares it in a DTD. A feed containing &copy; or &nbsp; will be rejected by a strict reader, and the failure often looks like an empty feed rather than an error message. Use numeric references there instead: &#169; and &#160; need no table and are valid in every XML document, as is the character itself if the file is UTF-8." },
    ],
  },
  // -------------------------------------------------- ROMAN NUMERAL DATES
  {
    slug: "how-to-write-a-date-in-roman-numerals",
    title: "How to Write a Date in Roman Numerals",
    h1: "How to Write a Date in Roman Numerals (Tattoos, Weddings and Cornerstones)",
    desc: "How to write a date in Roman numerals: why a date is three numerals rather than one number, which order and separator to use, why there is no leading zero, and the mistakes that are hard to undo - with a worked example.",
    category: "convert",
    tool: { slug: "roman-numerals", title: "Roman Numeral Converter" },
    updated: "2026-09-11",
    body: [
      { t: "p", s: "A date in Roman numerals is three numerals standing side by side, not one long number. That single fact is behind nearly every mistake that ends up on a tattoo, a wedding invitation, a cornerstone or a film credit. Get it right and the rest is mechanical: convert the day, convert the month, convert the year, join them with a separator you like. This guide walks through each step, the choices worth making deliberately, and the errors that are expensive to discover late. Everything here can be checked against the [Roman Numeral Converter](/convert/roman-numerals), whose Date tab writes all three pieces at once." },

      { t: "h2", s: "Three numerals, not one number" },
      { t: "p", s: "The most common error is to treat the date as a single figure. 25 December 1990 is not 25,121,990, and converting that number gives a long, meaningless string of letters. Each field is converted on its own and the three results are placed in a row: 25 becomes XXV, 12 becomes XII, 1990 becomes MCMXC, and the date is XXV·XII·MCMXC." },
      { t: "p", s: "This is also why a numeral date needs a separator in a way an ordinary date does not. Roman numerals have no fixed width, so without something between them the letters run together into one numeral that reads as a completely different value. The separator is not decoration - it is what tells a reader where the day ends and the month begins." },

      { t: "h2", s: "The seven letters and the two rules" },
      { t: "p", s: "Everything is built from seven letters:" },
      { t: "ul", items: [
        "I is 1, V is 5, X is 10, L is 50.",
        "C is 100, D is 500, M is 1,000.",
      ]},
      { t: "p", s: "Letters are written largest first and added up, so XXV is 10 + 10 + 5 = 25. The exception is subtraction: a smaller letter placed in front of a larger one is taken away instead. There are exactly six of these, and no others are standard - IV (4), IX (9), XL (40), XC (90), CD (400) and CM (900). Two further limits keep spellings honest: I, X, C and M may repeat up to three times in a row, while V, L and D are never repeated, because two of them simply make the next letter up." },

      { t: "h2", s: "A worked example: 25 December 1990" },
      { t: "p", s: "Take the three fields one at a time." },
      { t: "ul", items: [
        "Day, 25: the largest letter that fits is X, twice, leaving 5, which is V. That gives XX + V = XXV.",
        "Month, 12: X leaves 2, written II. That gives XII.",
        "Year, 1990: M covers 1,000 and leaves 990. The next 900 is not written with nine Cs but as CM, one hundred taken from one thousand. That leaves 90, which is XC by the same logic. The year is M + CM + XC = MCMXC.",
      ]},
      { t: "p", s: "Read MCMXC back and the arithmetic is 1,000 + (1,000 - 100) + (100 - 10) = 1,990. Joined with middle dots the finished date is XXV·XII·MCMXC. Paste it into the [Roman Numeral Converter](/convert/roman-numerals) and it will take the date apart the same way, showing the value of each piece." },
      { t: "p", s: "A second example: 14 February 2026 is XIV·II·MMXXVI, where the year is 1,000 + 1,000 + 10 + 10 + 5 + 1. Years in the 2000s come out noticeably shorter than years in the 1900s, which is worth knowing if you are choosing between two dates for a design." },

      { t: "h2", s: "Choosing the order and the separator" },
      { t: "p", s: "Nothing in the numerals themselves fixes the field order, so it is your choice - but make it a deliberate one, because a reader cannot recover it from the letters. The three orders in ordinary use each give a different result for the same day:" },
      { t: "ul", items: [
        "Day · month · year - XXV·XII·MCMXC. The usual choice outside the United States, and the most common on tattoos.",
        "Month · day · year - XII·XXV·MCMXC. The American reading order.",
        "Year · month · day - MCMXC·XII·XXV. Used on buildings and in formal inscriptions, where the year is the point.",
      ]},
      { t: "p", s: "For the separator, a middle dot is the conventional choice for a tattoo or an engraving and keeps the fields visually distinct. A full stop, a slash, an en dash or a plain space all appear and are equally readable. The converter offers all five so you can see the same date in each before committing to one." },

      { t: "h2", s: "There is no leading zero, and no zero at all" },
      { t: "p", s: "Roman numerals have no symbol for zero, so nothing is ever padded to two digits. The fifth of the month is V, not 0V; January is I, not 0I. A date written 0V·0I·MMXX is not a stricter version of 5 January 2020 - it is not a Roman numeral at all, because the character 0 does not exist in the system. This trips people up because the digital habit of writing 05/01/2020 is so strong." },
      { t: "p", s: "Roman counting had no zero, no negative numbers and no fractions of a unit, which is why a numeral clock face starts at I and why medieval scribes writing in Latin used the word nulla when they needed to record nothing at all." },

      { t: "h2", s: "IV or IIII" },
      { t: "p", s: "Both are read as four, and neither is a misprint. IV is the standard subtractive spelling and the one to use for a date. IIII is the older additive form, and it survives in one very visible place: the great majority of clock and watch faces use IIII rather than IV, because it balances the VIII directly opposite it on the dial. If you want a numeral to look like a clock face, IIII is defensible. For a date, use IV - a converter, a proofreader or a relative will otherwise read it as an error." },
      { t: "p", s: "The [Roman Numeral Converter](/convert/roman-numerals) takes the middle position here: type IIII and it still returns 4, then tells you the standard spelling is IV and why. That matters when you are reading a numeral off an old inscription rather than writing a new one, since historical numerals break the modern rules constantly." },

      { t: "h2", s: "Reading a numeral date back, and why it can be ambiguous" },
      { t: "p", s: "Going the other way is mechanical, with one catch. When both small numbers are 12 or under, nothing in the numerals says which is the day and which is the month. VIII·III·MCMXCIX is 8 March 1999 read day-first and 3 August 1999 read month-first, and both are real dates. The converter reports every field order that yields a valid date rather than silently picking one, so you can see the ambiguity instead of inheriting it." },
      { t: "p", s: "XXV·XII·MCMXC has no such problem: 25 cannot be a month, so the order is settled by arithmetic. If you have a choice of dates and want one that reads unambiguously, a day above the twelfth does it." },

      { t: "h2", s: "Dates that never happened" },
      { t: "p", s: "A converter that only checks the day against 31 will happily write 29·II·MCM for you. February 1900 had 28 days: 1900 is divisible by 100 and not by 400, so it was not a leap year under the Gregorian rule, and that date does not exist. The Date tab checks the day against its actual month and year and refuses the impossible ones, which is a check worth having before a date is cut into stone or skin." },

      { t: "h2", s: "It all runs in your browser" },
      { t: "p", s: "Birthdays, wedding dates and anniversaries are personal, and there is no reason a converter should see them. Every conversion here is worked out in JavaScript inside your browser tab - nothing is uploaded and there is no signup. The same tool converts in both directions, and the [Number to Words](/convert/number-to-words) converter spells a figure out in English when that is what you need." },
    ],
    faqs: [
      { q: "How do I write my birthday in Roman numerals?", a: "Convert the three fields separately and join them. Take 9 August 2015: the day 9 is IX, the month 8 is VIII, and the year 2015 is MMXV, so day-first the date is IX·VIII·MMXV. Do not run the digits together and convert 9,082,015 as one number - that is the single most common mistake and it produces a long meaningless string. Nothing is padded either, so a single-digit day stays one letter: the ninth is IX, never 09 or 0IX. The Date tab on the converter does all three pieces at once and shows the value of each, which is the quickest way to check a result before you commit to it." },
      { q: "Should the day or the month come first?", a: "Either - the numerals do not encode an order, so it is a choice you make and one a reader cannot recover from the letters alone. Day · month · year is the most common on tattoos and the norm outside the United States; month · day · year is the American reading order; year · month · day is usual on buildings and formal inscriptions. The one thing worth checking is whether your date is ambiguous: if both small numbers are 12 or under, as in VIII·III·MCMXCIX, it reads as two different real dates depending on the order, and there is no way to tell them apart later. A day above the twelfth removes the ambiguity by itself." },
      { q: "Is four written IV or IIII, and is there a numeral for zero?", a: "Use IV for a date. IV is the standard subtractive spelling; IIII is the older additive form and is not exactly wrong, but it survives mainly on clock and watch faces, where it balances the VIII opposite it on the dial. There is no numeral for zero at all - the system has no symbol for it, which is why dates are never padded and the fifth of the month is V rather than 0V. Roman arithmetic also had no way to write a negative number or a fraction, so a converter that offers you a zero or a decimal is inventing notation rather than reporting it." },
    ],
  },
  ...
  // ---------------------------------------------------------------- FILE-CHECKSUM
  {
    ...
    h1: "How to Verify a File Checksum (SHA-256, MD5 and CRC-32)",
    desc: "How to check a downloaded file against its published SHA-256 or MD5 checksum - in the browser, on macOS, Linux and Windows - what to do when it does not match, and why a match is not proof the file is genuine.",
    category: "dev",
    tool: { slug: "file-checksum", title: "File Checksum Calculator" },
    updated: "2026-09-13",
    body: [
      { t: "p", s: "A download page shows a long string of hex next to the file - something like sha256: e3b0c442... - and most people scroll past it. That string is a checksum, and comparing it against the file you actually received is the cheapest way to know your download arrived complete and unmodified. It takes about ten seconds. This guide covers how to produce a file's checksum on any operating system or in the [File Checksum Calculator](/dev/file-checksum), how to read the formats publishers use, what to do when the numbers disagree, and the limit that matters: what a match does and does not prove." },

      { t: "h2", s: "What a checksum actually is" },
      { t: "p", s: "A checksum is the output of a hash function run over every byte of a file. Feed the same bytes in and you always get the same digest out; change a single bit and the digest changes completely. The publisher prints the digest of the file they meant to ship; you compute the digest of the file that arrived; if the two strings are identical, you have the same bytes they did." },
      { t: "p", s: "The length of the digest tells you which algorithm produced it, which is useful because publishers do not always say:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "A digest is occasionally handed to you in Base64 rather than hex - S3 and Azure both report MD5 that way in a Content-MD5 header. It is the same value written differently." },

      { t: "h2", s: "Reading the formats publishers use" },
      { t: "p", s: "There are three common ways to write a checksum down:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "A release often ships one SHA256SUMS file covering every image it publishes. Download that rather than copying a single value, so the desktop ISO is checked against the desktop line instead of whichever of six similar hex strings you picked out by eye." },

      { t: "h2", s: "Checking a file in the browser" },
      { t: "p", s: "Drop the file into the [File Checksum Calculator](/dev/file-checksum) and it computes SHA-256, SHA-1 and MD5, with SHA-512 and CRC-32 as tick boxes. Nothing is uploaded: the file is read off your disk in four-megabyte chunks, each folded into the running digest and discarded, so a 4 GB ISO costs time but not memory." },
      { t: "p", s: "Then paste what the publisher gave you into the compare box. It takes any of the three formats above, a Base64 digest, or the whole SHA256SUMS file - in which case each file you dropped in is matched to the line filed under its name. The algorithm is inferred from the digest's length, and the comparison is on bytes, so a digest in capitals matches one in lowercase rather than reading as a mismatch." },
      { t: "p", s: "You can also drop two files in with no published checksum at all: if their SHA-256 digests agree they are byte-for-byte identical, which is the quickest way to confirm a copy or a backup came through intact." },

      { t: "h2", s: "Checking a file from the command line" },
      { t: "p", s: "Every operating system ships a tool for this:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "The command line is faster on a large file - those tools use native code and often the CPU's own SHA instructions. The browser wins when you would rather not install anything, or want several algorithms in one pass." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Make a file holding exactly the five bytes of the word hello, no newline: printf 'hello' > hello.txt. Its checksums are:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Now make the same file the way almost everyone would: echo hello > hello.txt. That writes six bytes, because echo adds a trailing newline. The SHA-256 is now 5891b5b522d5df086d0ff0b110fbd9d21bb4fc7163af34d08286a2e846f6be03 - not similar to the previous value, not close to it, simply unrelated. One invisible byte in five has changed every character of the digest." },
      { t: "p", s: "That is the avalanche effect, and it is what makes checksums useful: there is no such thing as a near miss. Two digests either match exactly or tell you nothing at all about how alike the files are." },

      { t: "h2", s: "When the checksum does not match" },
      { t: "p", s: "Work through this in order, because the boring explanations are overwhelmingly the likely ones:" },
      { t: "ol", items: [
  ...
  },
  {
    ...
    body: [
      ...
      { t: "h2", s: "A worked example: one logo, two exports" },
      { t: "p", s: "Say the file is a wordmark starting <svg viewBox=\"0 0 160 40\"> with no width or height: detected size 160x40, ratio 4:1. Two jobs, two answers:" },
      { t: "ul", items: [
  ...
  // ---------------------------------------------------------------- AGE
  {
    ...
    h1: "How to Calculate Age from Date of Birth",
    desc: "How to calculate age from a date of birth by hand: count years, then months, then days. A worked example, why dividing by 365 is wrong, and the two dates that have no clean answer.",
    category: "convert",
    tool: { slug: "age-calculator", title: "Age Calculator" },
    updated: "2026-09-16",
    body: [
      { t: "p", s: "Age is the one calculation almost everybody does in their head and almost nobody does the same way twice. In conversation a year is close enough; on a passport application or a school admission form it has to be exact to the day, and at that point the arithmetic stops being obvious, because months are not the same length and years are not either. This guide sets out the rule and the two dates that have no clean answer." },

      { t: "h2", s: "The rule: years first, then months, then days" },
      { t: "p", s: "Age counts anniversaries that have already happened, largest unit first. That is the whole method, and it is worth stating because it is not what a subtraction does. In order:" },
      { t: "ol", items: [
        "Count the completed years: move the date of birth forward a year at a time for as long as the result is still on or before today. The number of steps is the age in years.",
        "Count the completed months on top of that, stepping forward a month at a time from the date you landed on.",
        "Count the days left over between that date and today.",
      ]},
      { t: "p", s: "The order is what keeps the answer stable: work the days out first and you end up borrowing from a month whose length you have to guess at, which is where most attempts go wrong." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take a date of birth of 20 May 1990, and work out the age on 16 September 2026." },
      { t: "ul", items: [
        "Years: 20 May 1990 plus 36 years is 20 May 2026, still on or before 16 September 2026; plus 37 would be 20 May 2027, which is not. So 36 completed years.",
        "Months: from 20 May 2026, one month is 20 June, two 20 July, three 20 August - still before 16 September. Four would be 20 September, past it. So 3 completed months.",
        "Days: from 20 August 2026, 11 days takes you to the 31st and 16 more to 16 September. That is 27 days.",
      ]},
      { t: "p", s: "The age is 36 years, 3 months and 27 days. The [Age Calculator](/convert/age-calculator) does the same three steps and also reports the same span counted in single units: 435 months, 1,895 weeks and 3 days, 13,268 days, or 318,432 hours. It will tell you that 20 May 1990 was a Sunday, and that the next birthday - the 37th - falls on Thursday 20 May 2027, 246 days after the date we measured from." },

      { t: "h2", s: "Why dividing by 365 gives the wrong answer" },
      { t: "p", s: "The shortcut everyone reaches for is to subtract the dates for a number of days and divide by 365. Try it on the example: the gap is 13,268 days, which is 36 years with 128 days left over, and dividing those by 30 gives 4 months and 8 days. The shortcut says 36 years, 4 months and 8 days; the calendar says 36 years, 3 months and 27 days. It is out by eleven days." },
      { t: "p", s: "Two errors combine to produce that. The first is leap days: there were 9 leap years between 1991 and 2026, so the span holds 9 days a 365-day year does not account for. The second is the 30-day month, which does not exist in the calendar at all - seven months have 31 days, four have 30, and February has 28 or 29. Neither error is constant, so no better divisor fixes them: 365.25 narrows the first, does nothing about the second, and adds a third, because 1900 and 2100 are not leap years. The fix is not a better average but to stop averaging - count real anniversaries on the real calendar." },

      { t: "h2", s: "The two dates with no clean answer" },
      { t: "h3", s: "Born on the 29th, 30th or 31st" },
      { t: "p", s: "What is one month after 31 January? There is no 31 February, so the question has no arithmetic answer - only a convention. Every mainstream date library, and this calculator, moves to the last day of the shorter month: 28 February, or 29 in a leap year. One consequence is worth knowing: adding a month twice is not the same as adding two months. 31 January plus a month is 28 February, and plus a month again is 28 March; plus two months in one step is 31 March - three days apart from the same start date, depending only on how you grouped the steps. A calculator should say when it has applied the rule rather than quietly shifting your anniversary." },
      { t: "h3", s: "Born on 29 February" },
      { t: "p", s: "A leap-day birthday exists only one year in four. In the other three it has to be assigned to a date that does exist, and different places decided differently. England and Wales, and India, treat it as 28 February, so somebody born on 29 February 2000 turns 23 on 28 February 2023; Hong Kong and several US states use 1 March. Nothing in the arithmetic settles it, so a calculator showing one date without naming the other is hiding a legal question behind a number." },

      { t: "h2", s: "Age in months, weeks and days" },
      { t: "p", s: "Plenty of forms do not want years at all. A paediatric chart wants a baby's age in completed months; some visa forms want days; a school cut-off wants the age on a fixed date - 31 March or 1 September - rather than today, which is why the date you are ageing to should be editable. These are not conversions of one another: the 435 months above is not 36.35 multiplied by 12 and rounded, it is the count of monthly anniversaries that have actually passed, and that is the figure a registrar or a doctor will arrive at." },

      { t: "h2", s: "Counting the days between two dates" },
      { t: "p", s: "The same machinery answers the other common question - how long from one date to another - but it carries a trap that has nothing to do with arithmetic. From 16 September to 15 December 2026 there are two correct answers: 90 is the gap between the dates, and 91 counts both the first day and the last. Leave, notice periods and hotel stays are usually written the second way; a deadline means the first. If a contract says 90 days, check which before you diary it." },
      { t: "p", s: "Working days are a third figure again: that span holds 65 weekdays and 26 weekend days counting both ends, so 60 working days is not the same deadline as 90 calendar days. The Days between tab in the [Age Calculator](/convert/age-calculator) reports all three at once, but deducts no public holidays - those differ by country and state." },

      { t: "h2", s: "Doing it in a spreadsheet" },
      { t: "p", s: "Excel and Google Sheets both carry DATEDIF, inherited from Lotus 1-2-3. The three-part age is DATEDIF(dob, today, \"Y\") for years, \"YM\" for the months on top and \"MD\" for the days. Microsoft's documentation warns against that last one: \"MD\" can return a negative number in exactly the month-end cases above." },
      { t: "p", s: "Plain subtraction - =B1-A1 - gives the day count, because a spreadsheet stores a date as a day number internally. That is the idea used here too: whole calendar dates on an integer day number, with no time of day and no time zone, so the answer does not move with your location or with daylight saving. Timestamps are a different job - for those, see the [Epoch Converter](/convert/epoch-converter)." },

    ],
    faqs: [
      { q: "How do I calculate my exact age in years, months and days?", a: "Count the completed years from your date of birth first, then the completed months on top of those, and finally the days left over - in that order, largest unit first. Born on 20 May 1990, on 16 September 2026 that gives 36 years (to 20 May 2026), then 3 months (to 20 August 2026), then 27 days to reach 16 September. The order matters: if you start with the days you end up borrowing from a month whose length you have to look up, which is where most by-hand attempts go wrong. The Age Calculator does the three steps and also shows the same age as 435 months, 1,895 weeks and 3 days, or 13,268 days." },
      { q: "Why do two age calculators give me different answers?", a: "Almost always for one of three reasons. The first is that one of them divides by 365 or by 365.25 instead of counting real anniversaries, which drifts by a day or more over a long span and by more still if the period covers several leap years. The second is the month-end convention: from 31 January to 28 February, a calculator that counts anniversaries says one month while one that borrows day-of-month figures says 28 days, and both are defensible. The third is time zones - a tool that parses your date of birth as UTC midnight and then reads it back in local time shows yesterday's date to anyone in the Americas, which moves the whole answer by a day." },
      { q: "When does someone born on 29 February have a birthday?", a: "It depends on where you are, and it is a legal question rather than a mathematical one. In England and Wales, and in India, the birthday is treated as 28 February in a year that has no 29th, so a person born on 29 February 2000 turns 23 on 28 February 2023 and 24 on 29 February 2024. Hong Kong and several US states take 1 March instead. There is no arithmetic that settles this, which is why the calculator here uses 28 February and names 1 March as well rather than quietly picking one." },
    ],
  },
  // ------------------------------------------- JSON to TypeScript
  {
    slug: "how-to-generate-typescript-interfaces-from-json",
    title: "How to Generate TypeScript Interfaces from JSON",
    h1: "How to Generate a TypeScript Interface from JSON",
    desc: "Turn a JSON response into TypeScript interfaces. How optional and nullable differ, why one sample is never enough, and the fields a generator gets wrong.",
    category: "dev",
    tool: { slug: "json-to-typescript", title: "JSON to TypeScript" },
    updated: "2026-09-16",
    body: [
      { t: "p", s: "You have a JSON response from an API and you want a TypeScript type for it. Writing one by hand is slow and, worse, quietly inaccurate: you type what the response happened to contain the day you looked, so fields that are sometimes absent come out required and fields that are sometimes null come out non-nullable - and the compiler then tells you your code is safe when it is not. Generating the type from real data is faster and more honest, but only if the generator reads the data properly. This guide covers what a good conversion does and how to read the result." },

      { t: "h2", s: "The three things a generated type has to get right" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Everything else - indentation, interface or type, exported or not - is preference. Those three are correctness, and they are what a five-line converter gets wrong." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take a response with three users in it, where the second has a null email and the third has no email key at all and carries an extra admin flag:" },
      { t: "p", s: "{ \"page\": 1, \"total\": 3, \"users\": [ { \"id\": 7, \"name\": \"Ada\", \"email\": \"ada@example.com\", \"team\": \"core\" }, { \"id\": 8, \"name\": \"Lin\", \"email\": null, \"team\": \"core\" }, { \"id\": 9, \"name\": \"Ravi\", \"team\": \"growth\", \"admin\": true } ] }" },
      { t: "p", s: "Paste that into the [JSON to TypeScript](/dev/json-to-typescript) generator, set the root name to ApiResponse, and you get two interfaces:" },
      { t: "p", s: "export interface ApiResponse { page: number; total: number; users: User[]; }" },
      { t: "p", s: "export interface User { id: number; name: string; email?: string | null; team: string; admin?: boolean; }" },
      { t: "p", s: "Three details are worth pulling out. The users array became User[] rather than an inline type, singularised from the key. The admin flag is optional, because only one of the three records had it. And email is both optional and nullable - missing from one record, null in another, which are two separate facts about one field." },

      { t: "h2", s: "Optional and nullable are not the same thing" },
      { t: "p", s: "This is the distinction most converters collapse, and the one that matters. A question mark means the key may not exist at all, so reading it gives undefined. A union with null means the key is always there and may hold null. They need different code - user.email?.trim() for the first, user.email !== null for the second - and guarding the wrong one leaves the crash you were trying to prevent." },
      { t: "p", s: "A generator that emits any for anything uncertain, or that marks nullable fields optional because it silences the compiler, throws the distinction away. So does one that ignores null and types email as string - the worst of the three, because the type now asserts something the data contradicts." },

      { t: "h2", s: "Why one record is never enough" },
      { t: "p", s: "Optionality is not visible in a single object. Feed a generator one user and every field comes out required - nothing in one sample could say otherwise. The type compiles, the code passes review, and the first record in production without an email fails at runtime instead of at build time, which is exactly the failure TypeScript was meant to move earlier." },
      { t: "p", s: "So paste an array of several records. The generator reads every element, merges the shapes and marks a field optional whenever some records lack it. This is also why reading only the first element - which is what a naive converter does, because it is far easier - is worse than it sounds: a field that first appears in the second record vanishes from the type altogether, and the property you needed becomes a compile error on data that is perfectly valid. The tool reports how many records the best-sampled shape was built from, and says so plainly when it only had one." },
      { t: "p", s: "Include the awkward rows on purpose - the record with the empty list, the one with the null, the one still carrying a legacy field. A type generated from the happy path only describes the happy path." },

      { t: "h2", s: "Keys that TypeScript cannot use as they are" },
      { t: "p", s: "A JSON key can be any string, while a TypeScript property name has to be an identifier - so \"first name\", \"content-type\", \"2fa\" and the empty string are quoted in the interface and read back as obj[\"first name\"]. A generator that renames them to look prettier produces a type that no longer matches the data, so quoting is the right answer even though it is the uglier one." },
      { t: "p", s: "Interface names are a separate problem, because they are derived from keys rather than being keys. They are PascalCased, singularised when they name an array, and kept away from the standard library's own type names - declaring an interface called Object merges with the built-in declaration rather than shadowing it, which breaks the file in a way the error message does not explain. Identical shapes also share one interface: an author and an editor that both hold a name and an email should not become two identical types." },

      { t: "h2", s: "The numbers that do not survive" },
      { t: "p", s: "TypeScript has one numeric type for JSON numbers, and it is a 64-bit float. Integers above 2^53 - roughly 9.007 quadrillion, or sixteen digits - cannot be held exactly, so a Discord or Twitter style snowflake ID typed as number comes back with its last digits changed. The generator flags those rather than typing them silently, and the fix is to declare the field as string or bigint." },
      { t: "p", s: "Check the other end too: JSON.parse does the same damage before any type is involved, so an annotation will not save an ID already rounded on the way in. The [JSON Formatter](/dev/json-formatter) shows you the raw text if you need to look." },

      { t: "h2", s: "When to trust a string literal union" },
      { t: "p", s: "A status field holding \"active\" or \"archived\" is more useful typed as \"active\" | \"archived\" than as string, because the compiler then catches a typo in a comparison. But inferring that from a sample is a guess, and a wrong guess rejects valid data. So it is an option rather than the default, and a union is only inferred on evidence of a closed set: at least three records carried the field, at most twelve distinct values, and at least one value repeating. Three rows with three different values are three strings, and stay string." },
      { t: "p", s: "Even then it is a starting point. A sample cannot show you the status your API returns once a month, so the honest version of the type is usually the generated union plus whatever the documentation lists." },

      { t: "h2", s: "A generated type is a description, not a guarantee" },
      { t: "p", s: "Worth ending on, because it is the mistake that outlives the others. An interface is erased at compile time, so const data: ApiResponse = await res.json() checks nothing at runtime - it asserts, and if the API changes next Tuesday your code carries on believing the old shape until something breaks somewhere unhelpful. Generated types are still excellent for editor completion, for catching typos and for documenting what an endpoint returns, and the [JSON to TypeScript](/dev/json-to-typescript) generator gives you one in seconds from a response you already have. If you need the data actually checked, validate it at the boundary with a schema library and derive the type from that schema instead. Generate the interface to learn the shape; keep the runtime check for the shape you cannot control." },
    ],
    faqs: [
      { q: "How do I create a TypeScript interface from a JSON response?", a: "Copy a real response, paste it into the generator, and give the root type a name. The key is what you paste: an array of several records rather than a single object, because a single object cannot show which fields are optional. Every element is read and the shapes merged, so a field missing from some records comes out with a question mark, a field holding null comes out as a union with null, and nested objects become their own named interfaces with the array keys singularised - a users key produces User[]. You can then choose interface or type declarations, 2 or 4 space indentation, whether the declarations are exported, and whether every property is marked readonly." },
      { q: "What is the difference between name?: string and name: string | null in a generated type?", a: "They describe two different things and need different code. The question mark means the key may be absent from the object, so reading it gives undefined and you need optional chaining or a check against undefined. The union with null means the key is always present but may hold null, so you need a check against null. A field that is sometimes missing and sometimes null is written name?: string | null. Most generators collapse the two, usually by marking everything optional or by typing anything uncertain as any, and that is where a type stops protecting you: code guarding for undefined still crashes on null." },
      { q: "Is a generated TypeScript interface enough to validate API data?", a: "No, and this is the most common misunderstanding about them. TypeScript types are erased when your code is compiled, so an interface on an API response is an assertion rather than a check - nothing at runtime confirms the data matches, and if the endpoint changes your code keeps believing the old shape until something breaks further downstream. A generated interface is genuinely useful for editor completion, for catching typos at compile time and for documenting what an endpoint returns. If you need the data actually verified, validate it at the boundary with a runtime schema and derive the TypeScript type from that schema instead of from a sample." },
    ],
  },
  // ---------------------------------------------------------------- WHITESPACE
  {
    slug: "how-to-remove-extra-spaces-from-text",
    title: "How to Remove Extra Spaces and Hidden Whitespace from Text",
    h1: "How to Remove Extra Spaces and Hidden Whitespace from Text",
    desc: "How to remove extra spaces from text, and why your double spaces survive find-and-replace. The no-break and zero-width characters pasted from Word, PDFs and spreadsheets, explained.",
    category: "text",
    tool: { slug: "whitespace-remover", title: "Whitespace Remover" },
    updated: "2026-09-19",
    body: [
      { t: "p", s: "You paste a paragraph out of a PDF, a Word document or a web table and the spacing is wrong: gaps too wide, ragged indentation, four blank lines where you wanted one. So you reach for find-and-replace, swap two spaces for one, and it reports two replacements - while the gap you were actually staring at is still there. This guide explains why, what the characters in your text really are, and the order the cleanup has to run in." },

      { t: "h2", s: "Why the gap will not close" },
      { t: "p", s: "Almost every piece of advice about extra spaces assumes the gap is made of spaces. Often it is not. The space you type, U+0020, is one of roughly twenty characters that render as a horizontal gap, and word processors, PDF generators and web pages use the others routinely. On the clipboard they are indistinguishable by eye." },
      { t: "p", s: "The most common by far is the no-break space, U+00A0. It is what a browser produces from the HTML entity &nbsp;, what Word inserts to keep a number with its unit, and what a PDF viewer hands over from justified text. It looks exactly like a space and is a different character, so a search for two spaces does not find a pair of them, and a tool built on that search reports your text is clean. Hence the conclusion that there is some invisible problem you cannot fix." },

      { t: "h2", s: "A worked example, character by character" },
      { t: "p", s: "Take an invoice reference copied from a web table. On screen it reads Invoice No. 2026-0417, with two wide gaps and stray indentation. On the clipboard it is 27 characters:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Find-and-replace on two spaces reports two matches: the pair at the start and the pair at the end. The two gaps in the middle - the ones you were trying to fix - are untouched, because neither is made of spaces. Paste the same text into the [Whitespace Remover](/text/whitespace-remover) and it names what it found: 2 no-break spaces, 2 tabs. The output is Invoice No. 2026-0417, 21 characters, 6 removed and 4 gaps fixed." },

      { t: "h2", s: "The characters that look like a space but are not" },
      { t: "p", s: "These all render as a gap, so each is safe to turn into an ordinary space:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "A tab, U+0009, belongs here for a different reason: its width is whatever the thing displaying it decides, so a tab in a form field or a CSV cell is a gap of unpredictable size. Collapsing runs of spaces and tabs together is what makes pasted text behave." },

      { t: "h2", s: "The invisible ones" },
      { t: "p", s: "A second group has no width at all, so nothing looks wrong - but they count toward character limits and break exact-match comparisons. It is why a code, a password or a search term that looks identical refuses to match." },
      { t: "ul", items: [
      ...
      { t: "p", s: "These should be deleted, not converted to a space. A zero-width space inside a word is a line-break hint, not a gap between words, so replacing it with a space splits the word in two - the difference between cleaning text and damaging it." },

      { t: "h2", s: "The two you must not delete" },
      { t: "p", s: "The zero-width joiner (U+200D) and non-joiner (U+200C) are invisible too, so tools advertising invisible-character removal sweep them up with the rest. They carry meaning. A family emoji is several emoji bound together by joiners, so stripping them turns one picture into three. In Devanagari and Perso-Arabic they decide whether a conjunct renders as a ligature or a half-form, so removing them corrupts Hindi, Arabic and Persian - every letter still there, every word spelled wrong. A cleaner should leave these alone by default and make removing them a deliberate, separate choice, which is how the [Whitespace Remover](/text/whitespace-remover) treats them." },

      { t: "h2", s: "Trailing spaces and blank lines" },
      { t: "p", s: "Trailing spaces at the ends of lines are invisible and survive every paste. They make a list look ragged in a form field, they break a spreadsheet lookup, and leading spaces do the same at the other end." },
      { t: "p", s: "Blank lines need a decision rather than a rule. One between paragraphs is a paragraph break and should stay; a run of four is an artefact of the layout the text came from. Collapsing a run to one keeps prose readable, removing them all closes up a list, and blank lines at the very start and end should just go - an empty first line is not a paragraph break." },

      { t: "h2", s: "Doing it in a spreadsheet or an editor" },
      { t: "p", s: "In Excel or Google Sheets, TRIM only half works: it strips leading and trailing spaces and collapses internal runs, but it does not touch U+00A0, so the cell that looked wrong still does. CLEAN does not help either - it only removes ASCII control characters below code 32. The working formula substitutes first: TRIM(SUBSTITUTE(A1, CHAR(160), CHAR(32))). Each zero-width character needs its own SUBSTITUTE on top." },
      { t: "p", s: "In an editor with regex search, replacing \\s+ with one space gets further than you might expect, because JavaScript's \\s does include the no-break space and the typographic spaces. But it also matches newlines, so it flattens paragraphs into one line, and it misses the zero-width space entirely. \\s is not the set you assume in either direction: it excludes U+200B, which people paste constantly, and includes U+FEFF, which nobody types." },

      { t: "h2", s: "The order the steps have to run in" },
      { t: "p", s: "The steps interfere with each other. Normalise line endings first: a file using carriage returns, or a U+2028 pasted out of a JavaScript string, is a single line to every per-line operation, so nothing else will fire. Delete the zero-width characters next, before collapsing gaps - a space, then a zero-width space, then a space is not a run of whitespace while that character sits in the middle, so collapsing first leaves two spaces behind. Convert the exotic spaces third, so the collapse sees one kind of character. Only then collapse runs, trim line ends and handle blank lines." },
      { t: "p", s: "Get the order wrong and the text looks cleaned while still holding what you were removing - the worst outcome, because you stop looking. Run it in that order and the result is stable: cleaning it again changes nothing." },
    ],
    faqs: [
      { q: "Why does find-and-replace not remove my double spaces?", a: "Because the gap is probably not made of space characters. Text from Word, PDFs, web pages and spreadsheets is full of no-break spaces (U+00A0) and typographic spaces like the em space and figure space, which render exactly like a space but are different characters. Searching for a space does not find them, so the tool truthfully reports no matches while the gap you can see stays put. The fix is to convert those characters to ordinary spaces first and collapse afterwards. Any cleaner worth using will also tell you which characters it found by name, because knowing your text contains four no-break spaces explains the CSV column that would not parse, the lookup that returned nothing, and the reference that would not match." },
      { q: "How do I remove all spaces from text rather than just the extra ones?", a: "Choose the option that removes all spaces rather than collapsing them. Every space and tab is deleted, including single spaces between words, so h e l l o becomes hello. Line breaks are kept, so each line of a list keeps its own line and just loses its internal spacing. Use the default collapse setting for ordinary prose, where you want runs of whitespace reduced to a single space and the text left readable. Removing every space is mainly useful for cleaning up a code, a serial number or a value that was pasted with spacing inside it." },
      { q: "Is it safe to remove invisible characters from text with emoji or Hindi?", a: "Only if the tool separates the two kinds. Characters like the zero-width space, byte-order mark and soft hyphen carry no meaning in plain text and can be deleted safely. The zero-width joiner (U+200D) and non-joiner (U+200C) are different: a family emoji is several emoji bound together by joiners, so deleting them turns one picture into three, and in Devanagari and Perso-Arabic they decide whether a conjunct renders as a ligature or a half-form, so removing them corrupts Hindi, Arabic and Persian text without any visible error. A cleaner should leave those two alone by default and make removing them a separate, clearly labelled choice." },
    ],
  },
  // ------------------------------------------------- Remove HTML tags
  {
    slug: "how-to-remove-html-tags-from-text",
    title: "How to Remove HTML Tags from Text",
    h1: "How to Remove HTML Tags and Get Clean Plain Text",
    desc: "How to strip HTML tags from text properly: why a regex leaves JavaScript and CSS behind, where line breaks belong, and the order entity decoding has to run in.",
    category: "text",
    tool: { slug: "remove-html-tags", title: "HTML Tag Remover" },
    updated: "2026-09-20",
    body: [
      { t: "p", s: "You have a chunk of HTML - a saved page, a CMS export, an email body - and you want the words without the markup. There is a one-line answer everybody reaches for, and it is wrong in three separate ways, each of which shows up on the first real page you feed it." },

      { t: "h2", s: "The one-liner, and what it leaves behind" },
      { t: "p", s: "Search for how to strip HTML and the answer is always some version of replace(/<[^>]*>/g, \"\"). Delete anything that looks like a tag. Take this fragment of a release-notes page:" },
      { t: "p", s: "<div class=\"post\"> <h2>Release notes</h2> <style>.post{margin:0}</style> <p>Version&nbsp;2.1 ships <b>fast</b>er builds &amp; a new <a href=\"/docs\">docs site</a>.</p> <ul><li>Cache warm-up</li><li>Smaller bundles</li></ul> <script>track(\"view\", {a: 1 > 0});</script> </div>" },
      { t: "p", s: "Run the regex over it and you get this:" },
      { t: "p", s: "Release notes .post{margin:0} Version&nbsp;2.1 ships faster builds &amp; a new docs site. Cache warm-upSmaller bundles track(\"view\", {a: 1 > 0});" },
      { t: "p", s: "Four things went wrong at once. The style sheet is in your text. The tracking call is in your text. The two list items have fused into Cache warm-upSmaller bundles. And the entities never decoded. Each is a category of mistake rather than a detail." },

      { t: "h2", s: "Script and style hold code, not markup" },
      { t: "p", s: "HTML has a handful of elements whose contents the parser does not treat as markup at all. Inside script, style, noscript and template, everything up to the matching closing tag is raw character data - so a regex that deletes things shaped like tags deletes a script's opening and closing tags and leaves the entire program between them." },
      { t: "p", s: "On a hand-written fragment that is an eyesore. On a real page it is the whole output: a modern page carries far more bytes of minified JavaScript and CSS than of prose, so the stripped result comes out longer than the page did. That wall of gibberish an online stripper hands back is your own page's code." },
      { t: "p", s: "The fix is a tokenizer, not a better regex: something that knows a script block ends at the first </script followed by whitespace, a slash or a greater-than sign, and discards what is inside. It also handles the other place the regex breaks down - a greater-than sign is legal inside an attribute value, so <a title=\"a > b\">x</a> stops [^>]* early and leaks b\">x into the output." },

      { t: "h2", s: "Deleting a tag deletes the boundary it stood for" },
      { t: "p", s: "This is the failure people notice last and care about most. Cache warm-upSmaller bundles is not a formatting problem; it is two facts that have merged into one. The li tags were the only thing telling you where the first item ended." },
      { t: "p", s: "So put a space at every tag? That breaks it the other way. <b>c</b>at renders as the single word cat, and a stripper that inserts whitespace at every tag gives you c at. Bold, italics, links and spans sit inside a line of text, and whatever you put where they were is a word break that was never on the page." },
      { t: "p", s: "The line is drawn by CSS, not by the tags. Whitespace between two inline boxes is rendered; whitespace at the edge of a block box is not. So a paragraph, heading, list item or table row is a real boundary and earns a line break, while b, i, span, a, em and strong earn nothing. An extractor carrying the browser's default display table gets that right for free, and can then put a blank line between paragraphs and a tab between table cells, so a row still pastes into a spreadsheet as a row." },
      { t: "p", s: "The [HTML Tag Remover](/text/remove-html-tags) follows exactly that model. On the fragment above it produces:" },
      { t: "p", s: "Release notes / (blank line) / Version 2.1 ships faster builds & a new docs site. / (blank line) / Cache warm-up / Smaller bundles" },
      { t: "p", s: "The heading is its own paragraph, faster is still spelled correctly, the two list items are two lines, and the CSS and the tracking call are gone with their tags." },

      { t: "h2", s: "Decode the entities - but only after the tags are gone" },
      { t: "p", s: "Character references have to be decoded or your text is full of &amp;, &nbsp; and &#8217;. That part is obvious. The order is not, and getting it backwards is a quiet, nasty bug." },
      { t: "p", s: "If you decode first, an escaped &lt;script&gt; - text a page displays literally, and which any article or changelog about HTML is full of - becomes a real script tag. The stripping step then eats it, and on a page discussing HTML it can eat everything between two of them. Tags first, entities second." },
      { t: "p", s: "Decoding also depends on where the text came from. HTML lets 106 legacy names drop their closing semicolon, which is why ?a=1&copy=2 reads as page text with a copyright sign in the middle - but inside an attribute value that is not a reference at all, so the same string survives intact in an href. Keep link URLs and each piece has to be decoded under its own rule; one pass over the finished document gets one of the two wrong." },
      { t: "p", s: "And a no-break space decodes to U+00A0, not to a space. It looks like one and is not, and pages use runs of them to fake indentation, so folding them into ordinary spaces saves a confusing afternoon when a search over the pasted text refuses to match." },

      { t: "h2", s: "Links, alt text, and what a copy-paste would give you" },
      { t: "p", s: "The honest default is what you would get by selecting the rendered page and pressing copy: no image placeholders, no URLs, just the words. Two exceptions are worth a checkbox each. Link URLs, because in a newsletter the links are half the content - in brackets after the link text they survive without the output becoming markdown. And image alt text, which is what a screen reader announces and what a browser shows when an image fails to load." },

      { t: "h2", s: "Stripping tags is not sanitising" },
      { t: "p", s: "This is a common reason people go looking for a tag stripper, and the one case where the answer is do not. Removing tags from user input as an XSS defence is a known-bad pattern: attacks get through on malformed markup your stripper and the browser disagree about, on attributes rather than elements, and on content re-encoded downstream. If you accept HTML from users, keep it as HTML and run it through a real sanitiser - DOMPurify is the standard answer - against an allowlist, on the server." },

      { t: "h2", s: "Doing it without writing any code" },
      { t: "ol", items: [
        "Open the [HTML Tag Remover](/text/remove-html-tags) and paste your HTML in. Everything runs in your browser and nothing is uploaded.",
        "Leave the layout on Readable. Switch to One single line for a word count, or to the third option to leave every byte outside a tag where it is.",
        "Tick Keep link URLs or Keep image alt text if you want them; off gives a clean copy-paste equivalent.",
        "Read the notes under the result: how many script and style blocks were dropped, how many no-break spaces were folded, how many images went.",
      ]},
      { t: "p", s: "If you want the markup tidied rather than removed, the [HTML Beautifier](/dev/html-beautifier) reindents a document without changing a byte a browser renders. If the text you get back still has ragged spacing, the [Whitespace Remover](/text/whitespace-remover) finishes the job." },
    ],
    faqs: [
      { q: "Why is the output of an online HTML stripper sometimes longer than the page?", a: "Because it kept the code. The contents of a script or style element are character data rather than markup, so a tool built on a find-and-replace over anything between angle brackets deletes the opening and closing tags and leaves the entire JavaScript bundle and style sheet behind as what it calls plain text. A modern page carries far more bytes of code than of prose, so the result grows. A stripper that tokenizes the document the way a browser does removes those blocks together with their contents." },
      { q: "How do I keep paragraph breaks when I remove HTML tags?", a: "By letting the element decide. Block-level boxes - paragraphs, headings, list items, table rows, divs - are real boundaries on screen, so they earn a line break; inline ones such as b, span and a are not, so they should leave nothing behind. That distinction is the whole job: insert breaks everywhere and a bold letter followed by text turns cat into c at, insert them nowhere and two list items fuse into one word. A blank line between paragraphs and a tab between table cells matches what you would get by selecting the rendered page and copying it." },
      { q: "Can I use a tag stripper to make user input safe?", a: "No. Stripping tags is a text-extraction tool, not a security control, and removing tags as an XSS defence is a well-known anti-pattern - attacks get through on malformed markup, on attributes rather than elements, and on text that is re-encoded later in the pipeline. If you accept HTML from users, keep it as HTML and run it through a dedicated sanitiser such as DOMPurify against an allowlist, server-side. Use a stripper for reading a saved page, an email body or a CMS export, which is what it is built for." },
    ],
  },
  // --------------------------------------------------------- MARKDOWN-TO-HTML
  {
    ...
    h1: "How to Convert Markdown to HTML (Without Losing Your Line Breaks)",
    desc: "How to convert Markdown to HTML: why single line breaks disappear, which features are CommonMark and which are GitHub's, what happens to raw HTML in the source, how heading anchors are generated, and when you need a whole page rather than a fragment.",
    category: "convert",
    tool: { slug: "markdown-to-html", title: "Markdown to HTML" },
    updated: "2026-09-22",
    body: [
      { t: "p", s: "Markdown is what you write; HTML is what a browser reads. Almost every complaint about the converter in between comes down to three surprises: the line breaks you typed are gone, a table came out as a row of pipe characters, and the HTML you pasted into the middle did something unexpected. None of those are bugs. They are the rules, and once you can see them the conversion stops being mysterious." },

      { t: "h2", s: "What the converter is actually doing" },
      { t: "p", s: "A Markdown parser works in two passes. It first splits the document into blocks by looking at the start of each line - a # makes a heading, a - or a digit a list, a fence a code block, anything left over a paragraph - then walks the text inside each block for inline markers. That shape explains most of Markdown's behaviour: a blank line ends a block, a code fence suspends the inline pass so asterisks inside it stay asterisks, and a paragraph reaches the inline pass as one run of text, which is where its newlines stop being structure." },

      { t: "h2", s: "The line break that disappears" },
      { t: "p", s: "This is the most common question, and the answer is that it is deliberate. Inside a paragraph, a newline is an ordinary space. Write this:" },
      { t: "ul", items: [
        "Install the CLI, then run it.",
        "It writes a config file.",
      ]},
      { t: "p", s: "and you get one paragraph. The newline survives into the HTML, but a browser renders it as a space, so the two lines flow together on the page. The rule exists so you can hard-wrap a paragraph at 80 columns in your editor without the wrapping leaking into the output." },
      { t: "p", s: "Markdown gives you two ways to force a real break: end the line with two spaces, or with a backslash. Both produce <br>. The two-space version is invisible in an editor and gets stripped by plenty of tools on the way, which is why people conclude Markdown has eaten their formatting." },
      { t: "p", s: "If the text came from somewhere else - a changelog, a chat export, an address block - editing every line is not realistic. Tick Keep single line breaks in the [Markdown to HTML converter](/convert/markdown-to-html) and every newline inside a paragraph becomes a <br>. The tool also counts the breaks the default rule is about to swallow, so you hear about them before you spot them in the output." },

      { t: "h2", s: "A worked example" },
      { t: "p", s: "Take a short piece of documentation with a heading, a wrapped paragraph, a numbered list and a quote:" },
      { t: "ul", items: [
        "## Setup steps",
        "",
        "Install the CLI, then run it.",
      ...
      { t: "p", s: "Three things to notice. The heading picked up an id you can link to. The list items hold no <p>, because the list is tight - add a blank line between them and each gains a paragraph wrapper, which changes the spacing. And the paragraph kept its newline, which a browser shows as a space." },

      { t: "h2", s: "CommonMark and the GitHub extras" },
      { t: "p", s: "Markdown has a specification, CommonMark, which nails down the awkward cases the original 2004 description left open. It covers headings, emphasis, lists, links, images, blockquotes, code blocks and raw HTML. It does not cover four things people assume are Markdown:" },
      { t: "ul", items: [
      ...
      { t: "p", s: "Those four are GitHub Flavored Markdown, an extension. A strict CommonMark converter leaves your table as a paragraph full of pipes - which is what it looks like when a README renders correctly on GitHub and wrongly somewhere else. The GitHub extras box is on by default; untick it only when you need plain CommonMark output." },

      { t: "h2", s: "Raw HTML inside the Markdown" },
      { t: "p", s: "Markdown allows HTML on purpose. A <details> block, a <kbd> key, an <img> with a width attribute - none of these have Markdown syntax, so the specification passes the tags through. That is the right default for a document you wrote and the wrong one for a document you did not." },
      { t: "p", s: "The failure worth knowing about is not an exotic attack. Mention a <script> tag in prose without escaping it and the browser opens a script element there and treats the rest of your document as JavaScript source - everything after it vanishes. The converter flags raw HTML holding a script tag, an iframe or an inline event handler, so you find out before you publish." },
      { t: "p", s: "When the Markdown came from a form or an API, switch Raw HTML in the Markdown to Show it as text or Remove it. Both also disable javascript: links written in ordinary link syntax - [click](javascript:alert(1)) is valid Markdown and never passes through the raw-HTML branch, so a converter that only strips tags leaves it live. Treat this as tidying, not security: on a real site, sanitise the HTML server-side against an allow-list as well." },

      { t: "h2", s: "Heading anchors, and why they should match GitHub's" },
      { t: "p", s: "A heading id is what makes #installation work in a link. GitHub generates one from the heading text: lower-case it, delete everything that is not a letter, a number, a space, a hyphen or an underscore, then turn each space into a hyphen. Repeats get -1, -2 and so on." },
      { t: "ul", items: [
        "## Setup steps  ->  id=\"setup-steps\"",
        "## Setup steps  (again)  ->  id=\"setup-steps-1\"",
        "## What's next?  ->  id=\"whats-next\"",
      ]},
      { t: "p", s: "Matching that rule matters because the anchors inside your own README already use it. Convert the file with a different slug rule and every internal cross-reference breaks the moment you publish the HTML elsewhere." },

      { t: "h2", s: "A fragment, or a whole page" },
      { t: "p", s: "A converter's natural output is a fragment: headings, paragraphs and lists with no <html>, <head> or <body> around them. That is what you want when the HTML is going into a template or a CMS field." },
      ...
      { t: "h2", s: "Doing it without writing any code" },
      { t: "ol", items: [
        "Open the [Markdown to HTML converter](/convert/markdown-to-html) and paste your Markdown in. Everything runs in your browser and nothing is uploaded.",
  ...
  },
  {
    ...
    body: [
      ...
      { t: "p", s: "So Hi! in binary is 01001000 01101001 00100001 - three characters, three bytes, 24 bits. Type the same text into the [Text to Binary converter](/convert/text-to-binary) and it gives those three groups, with a character-by-character table underneath showing which byte belongs to which letter." },

      { t: "h2", s: "Converting binary back to text" },
      { t: "p", s: "Decoding is the same two steps in reverse. Split the bits into groups of eight, turn each group into a number by adding up the columns that hold a 1, and look each number up." },
      { t: "h3", s: "Worked example: 01000011 01100001 01110100" },
      { t: "ul", items: [
  ...
  // --------------------------------------------------------- HTML-TO-MARKDOWN
  {
    ...
    body: [
      ...
      { t: "h2", s: "Tags Markdown has no syntax for" },
      { t: "p", s: "A superscript is the clearest case: drop the <sup> from x<sup>2</sup> and you have written x2, which means something else. So <sup>, <sub>, <u>, <mark>, <kbd>, embedded iframes and videos are kept as HTML by default. A <details> block is written the way GitHub expects, with the tags on their own lines and blank lines inside, so the Markdown between them still renders. Some things are dropped outright: scripts and style blocks, with their contents - code is not text - comments, hidden elements, and decorative icons marked aria-hidden." },

      { t: "h2", s: "Pasting from a web page or Google Docs" },
      { t: "p", s: "When you copy formatted text, your clipboard holds two versions: the plain text and the HTML behind it. An ordinary text box takes the plain text, which has already lost the headings, the links and the bold. The converter takes the HTML version, so you can select part of an article in your browser, or a whole Google Doc, and paste it straight in." },
      { t: "p", s: "Google Docs needs one more trick. It does not use <b> and <i> at all - it marks bold and italic with inline styles on <span> tags, and wraps the whole paste in a <b> whose style says it is not bold. A converter that reads only tag names turns an entire Google Doc bold, or loses all of its emphasis. Reading the font-weight and font-style gets both right." },
      { t: "p", s: "For a whole saved page, tick Main content only. If the page has a <main> element, or a single <article>, only that is converted, so the navigation, cookie banner and footer stay out of your Markdown." },

      { t: "h2", s: "Converting a page step by step" },
      { t: "ol", items: [
        "Open the [HTML to Markdown converter](/convert/html-to-markdown). It runs entirely in your browser, so nothing you paste is uploaded.",
      ...
      { t: "p", s: "To check a conversion, paste the Markdown into the [Markdown to HTML converter](/convert/markdown-to-html) and look at the preview. If you only want the words with no formatting at all, the [HTML Tag Remover](/text/remove-html-tags) is the simpler tool." },
    ],
    faqs: [
      { q: "How do I convert a Google Doc to Markdown?", a: "Select the text in the document, copy it, and paste it into an HTML to Markdown converter that reads the clipboard's HTML rather than its plain text. Google Docs puts both on the clipboard, and the plain text has already lost your headings, lists and links. The one thing to watch is emphasis: Google Docs marks bold and italic with inline font-weight and font-style styles on span tags, and wraps the whole paste in a b tag styled as normal weight, so a converter that only reads tag names either makes the whole document bold or drops every bit of emphasis. Headings, lists and links come across as real Markdown." },
      { q: "Why does my converted Markdown have backslashes in it?", a: "They stop ordinary characters being read as formatting. In Markdown a line that starts with a number and a full stop is a list, a leading # is a heading, asterisks and underscores around a word are emphasis, and square brackets can start a link, so when those characters are part of your text they are escaped with a backslash. The backslash disappears when the Markdown is rendered. A careful converter escapes only where a character would genuinely be misread, so ordinary text like snake_case or 2 * 3 stays clean." },
      { q: "Can every HTML page be converted to Markdown without losing anything?", a: "No, because HTML can express more than Markdown. Superscripts, underlining, merged table cells, colours, layout and embedded video have no Markdown syntax. Markdown does allow raw HTML, so the lossless option is to keep those parts as HTML inside the Markdown file, which GitHub and most static-site generators render correctly. If you need pure Markdown, they have to be reduced to plain text, and a good converter tells you how many elements it changed rather than dropping them silently." },
    ],
  },
  {
    slug: "how-to-rotate-an-image",
    title: "How to Rotate or Flip an Image",
    h1: "How to Rotate, Flip or Straighten an Image",
    desc: "How to rotate a photo 90 degrees, flip it into a mirror image, or straighten a tilted horizon - why phone photos show up sideways, which turns are lossless, and what happens to the corners.",
    category: "image",
    tool: { slug: "rotate-image", title: "Rotate Image" },
    updated: "2026-09-26",
    body: [
      { t: "p", s: "A photo that arrives sideways, a selfie that reads backwards, a horizon that runs downhill - three different problems with three different fixes. Sideways needs a quarter turn, backwards needs a flip, and a tilted horizon needs a small rotation by an exact angle. This guide explains each one, why the quarter turn is the only edit that costs nothing, and what happens to the corners when you straighten, with a worked example you can follow in the [Rotate Image](/image/rotate-image) tool." },

      { t: "h2", s: "Why photos show up sideways in the first place" },
      { t: "p", s: "A phone camera sensor has one fixed orientation. When you shoot in portrait, most phones do not turn the pixels upright; they save them the way the sensor saw them and add a note to the file's EXIF data - the Orientation tag - saying \"turn this 90 degrees before showing it\". Your gallery app reads the note, so the picture looks right. Software that ignores it - an older photo viewer, some upload forms, a few email clients - shows the raw pixels, and the photo appears on its side." },
      { t: "p", s: "Current browsers honour the tag, so a photo opened in the tool appears the way your phone shows it. When you download, the picture is redrawn with the rotation built into the pixels themselves and no Orientation tag at all, so it can only be shown one way. If a photo still looks sideways in the tool, the pixels really are sideways, and a quarter turn fixes it for good." },

      { t: "h2", s: "Rotating, flipping and straightening, step by step" },
      { t: "ol", items: [
        "Drop a PNG, JPG or WebP onto the [Rotate Image](/image/rotate-image) tool, or click to choose one. It is read on your device - nothing is uploaded.",
        "Use Rotate left or Rotate right for a quarter turn, or 180 degrees to turn it upside down. Clicks add up, so two rights make 180.",
        "Use Flip horizontal to mirror left and right, or Flip vertical to mirror top and bottom. Flips act on the picture as you currently see it.",
        "For a small correction, drag the angle slider or type an exact value, in steps as fine as 0.1 degrees. Positive is clockwise, negative anticlockwise.",
        "At any angle that is not a multiple of 90, choose Keep whole picture or Crop empty corners.",
        "Check the readout under the preview - it gives the original size, the new size and what was done - then pick PNG or JPG and download. Reset returns to the original.",
      ]},

      { t: "h2", s: "Quarter turns are lossless" },
      { t: "p", s: "Turning a picture by 90, 180 or 270 degrees does not create or blend a single pixel. Every pixel moves to a new position - the top-left corner of a landscape photo becomes the top-right of the portrait - and the grid simply changes shape, so a 4032x3024 photo becomes exactly 3024x4032. The tool switches image smoothing off for these turns, so the move really is a straight copy, and saving as PNG keeps it bit-for-bit. Flips are the same: a mirror only reorders pixels." },
      { t: "p", s: "Saving as JPG re-encodes the picture once, at a high quality setting, so the loss is hard to see but real. The tool defaults to JPG only when you started with a JPG, because a photo saved as PNG usually comes out several times larger. If the file is going to be edited again, choose PNG." },

      { t: "h2", s: "Flip versus rotate" },
      { t: "p", s: "Rotating turns the whole picture around its centre; anything written in it stays readable, just at an angle. Flipping mirrors it, and text comes out backwards. That is the tell for a front-camera selfie: many phones save them mirrored, so a sign behind you reads in reverse. A horizontal flip fixes it. Flip vertical is rarer, mostly for reflections." },
      { t: "p", s: "Flipping both ways at once gives exactly the same result as a 180-degree turn, and a 180-degree turn plus one flip is the same as the other flip on its own. The tool simplifies these combinations as you click, so the readout always describes the result in the fewest steps." },

      { t: "h2", s: "Straightening by a few degrees" },
      { t: "p", s: "A horizon a few degrees off is the most common reason to rotate by something other than 90. Drag the slider until the horizon, a door frame or a wall edge lines up; typical corrections are between 1 and 5 degrees. Unlike a quarter turn, an arbitrary angle does move pixels off the grid, so each output pixel is interpolated from its neighbours. That softens very fine detail slightly - true in every editor - which is a reason to straighten once rather than nudging repeatedly." },
      { t: "p", s: "A tilted picture no longer fills an upright rectangle, which leaves two options. Keep whole picture grows the canvas until the tilted photo fits inside it, and fills the four triangles it opens up - transparent in a PNG, or white or any colour you choose. Crop empty corners trims instead to the largest upright rectangle that fits entirely inside the tilted photo, so there is nothing to fill. For a photograph the crop is almost always the right call." },

      { t: "h2", s: "A worked example: fixing a 3-degree tilt" },
      { t: "p", s: "Take a 4032x3024 phone photo whose horizon runs 3 degrees downhill to the right. Load it into [Rotate Image](/image/rotate-image) and type -3 into the angle field, turning it 3 degrees anticlockwise. The horizon levels off." },
      { t: "p", s: "With Keep whole picture, the canvas grows to 4185x3231. The width is the photo's width projected onto the horizontal plus its height projected onto it: 4032 x cos 3 degrees + 3024 x sin 3 degrees = 4026.5 + 158.3, or 4185 rounded. The height works the same way the other round: 4032 x sin 3 degrees + 3024 x cos 3 degrees = 211.0 + 3019.9, or 3231. The extra pixels are the four slim triangles at the corners." },
      { t: "p", s: "Switch to Crop empty corners and the output becomes 3889x2824 - still landscape, with no filled corners, keeping about 90% of the original area. At 5 degrees the same photo keeps about 84%, and at 45 degrees only 37.5%, which is why straightening works best on small corrections. Download as JPG, and the photo is straight." },

      { t: "h2", s: "If something looks wrong" },
      { t: "ul", items: [
        "It turned the wrong way. Rotate right is clockwise and positive angles are clockwise; for the other direction use Rotate left or a negative angle.",
        "The corners are black or white instead of see-through. JPG cannot store transparency, so they are filled with the background colour. Choose PNG to keep them transparent.",
        "The photo came out a different size. At a tilt, Keep whole picture adds canvas and Crop empty corners removes edge. Only quarter turns keep the pixel count the same.",
        "The file is much bigger than the original. You probably saved a photo as PNG. Choose JPG, or run it through the [Image Compressor](/image/image-compressor).",
        "It needs a tighter frame after rotating. Rotate first, then use [Crop Image](/image/crop-image) to choose exactly what stays.",
      ]},

      { t: "h2", s: "Rotate your image now" },
      { t: "p", s: "Load the picture, turn or flip it until it looks right, and download. Open the [Rotate Image](/image/rotate-image) tool - free, no sign-up, no watermark, and processed entirely in your own browser." },
    ],
    faqs: [
      { q: "How do I rotate a picture 90 degrees without losing quality?", a: "Use a quarter turn and save as PNG. Rotating by 90, 180 or 270 degrees only moves pixels to new positions without blending any, so the picture itself is unchanged; the tool turns image smoothing off for these turns so nothing is resampled. Saving as JPG adds one high-quality re-encode, which is usually invisible but not bit-for-bit identical." },
      { q: "Why is my photo sideways on my computer but upright on my phone?", a: "Your phone probably saved the pixels in the sensor's orientation and recorded the correct rotation in the EXIF Orientation tag. The phone reads that tag; the software showing it sideways does not. Rotating the photo and downloading it builds the rotation into the pixels and drops the tag, so it displays the same way everywhere." },
      { q: "How do I mirror an image?", a: "Use Flip horizontal. It swaps left and right, the same as seeing the picture in a mirror - the usual fix for a front-camera selfie with backwards text. Flip vertical mirrors top and bottom instead, and doing both is the same as rotating 180 degrees." },
    ],
  },
];
