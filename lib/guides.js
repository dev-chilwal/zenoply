...
export const GUIDES = [
  ...
  // ---------------------------------------------------------------- PDF-TO-EXCEL
  {
    ...
    body: [
      ...
      { t: "p", s: "There are no gridlines to follow inside most PDFs - just text placed at coordinates. The tool reconstructs each table from those positions. It groups pieces of text that sit at the same height into rows, and works out the columns from the vertical lanes of white space that run down between them. When a table has drawn borders, it goes one better: it reads the actual ruled lines from the page and uses them as the exact cell edges, which is what makes right-aligned numbers, tightly-packed columns and cells that wrap onto two lines come out correctly. Each piece of text is then dropped into the right cell, rebuilding the grid - no table markup required." },

      { t: "h2", s: "Every table found separately" },
      { t: "p", s: "A real document is rarely one clean table. It might have two tables stacked on a page, different tables of different shapes on different pages, a running header and page numbers repeated throughout, and a single long table that flows across a page break. The tool is built for this:" },
      { t: "ul", items: [
        "It detects each table on a page on its own, so a three-column table and a five-column table are never mashed into one grid.",
        "It drops repeated page headers, footers and page numbers, so that furniture doesn't leak into your data.",
        "It stitches a table that continues across a page break back into one, dropping the header row that repeats at the top of the next page.",
        "It gives every detected table its own preview and its own sheet in the Excel file.",
      ]},

      { t: "h2", s: "Which PDFs work best" },
      { t: "p", s: "Extraction relies on real, selectable text and on the columns being visually separated - drawn borders help most of all. That makes some documents ideal and others unsuitable." },
      { t: "ul", items: [
        "Works well: digital PDFs with clearly separated columns or ruled borders - bank and credit-card statements, invoices, price lists, financial reports, exported data tables and schedules.",
        "Struggles: borderless tables whose columns are jammed together with almost no gap, where the column boundaries become genuinely ambiguous.",
      ...
      { t: "p", s: "A fast check: open the PDF and try to select a value in the table with your cursor. If it highlights, the [PDF to Excel](/pdf/pdf-to-excel) tool can read it. If nothing selects, the page is an image and extraction will report that no tables were found." },

      { t: "h2", s: "Preview before you download" },
      { t: "p", s: "Automatic detection is not magic, so the tool shows a live preview of every reconstructed table before you export anything - each one labelled with its row and column count and the page it came from. This is your chance to confirm the rows and columns landed where you expect. If a column looks split in two or two columns look merged into one, the source layout was ambiguous on that table - usually a borderless table with uneven spacing. Checking the previews first saves you from importing a messy spreadsheet and having to clean it up afterward." },

      { t: "h2", s: "Excel (XLSX) or CSV?" },
      { t: "p", s: "The tool exports the same data as either an Excel workbook or CSV files. They differ in format and how they open." },
      { t: "ul", items: [
        "Excel (XLSX) opens straight in Excel, Google Sheets or Apple Numbers, puts each detected table on its own sheet, and stores cells that look like numbers as real numbers - so you can sum, sort and chart them immediately. Choose this if the spreadsheet is your destination.",
        "CSV is a plain-text file that almost any program can import - databases, analytics tools, other apps. A CSV holds one table, so you download each table separately. Choose it for maximum portability or when another system expects a CSV upload.",
      ]},
      { t: "p", s: "With CSV you can also pick the delimiter - comma, semicolon or tab. Semicolon is handy in regions where a comma is the decimal separator, and tab-separated values paste cleanly into a spreadsheet. If you want every table in one file, XLSX with one sheet per table is the simplest choice." },

      { t: "h2", s: "How to extract tables from a PDF" },
      { t: "p", s: "The full process is quick:" },
      { t: "ol", items: [
        "Open the [PDF to Excel](/pdf/pdf-to-excel) tool in your browser.",
        "Load the PDF that contains the tables.",
        "Click Extract tables and let the tool read every page and rebuild each grid.",
        "Check the preview of each detected table to confirm the rows and columns look right.",
        "Download all tables as one Excel file (one sheet each), or download any single table as CSV - choosing the delimiter first.",
      ...
      { t: "h2", s: "Extract your tables now" },
      { t: "p", s: "Skip the retyping. Load your PDF, let the tool detect every table, check the previews, and download a ready-to-use spreadsheet. Open the [PDF to Excel](/pdf/pdf-to-excel) tool to pull tables out of a PDF into Excel or CSV in seconds - free, private, and entirely in your browser. If you need the whole document as editable text instead of just its tables, the [PDF to Word](/pdf/pdf-to-word) tool converts a PDF into a Word document." },
    ],
    faqs: [
      { q: "How does the tool find the tables and columns?", a: "It reads the position of every piece of text and groups it into rows and columns; where a table has drawn borders it uses those ruled lines as exact cell edges, otherwise it finds columns from the vertical white space between them. It detects each table separately, so different shapes aren't merged, and it drops repeated page headers and footers." },
      { q: "What if there are several tables or the table spans pages?", a: "Each detected table gets its own preview and its own sheet in the Excel file, so tables of different shapes stay separate. A single table that continues across a page break is stitched back into one. You can also download any individual table as a CSV." },
