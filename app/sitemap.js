...
export default function sitemap() {
  ...
  const guidesHub = [{ url: `${SITE.domain}/guides/`, lastModified: now, priority: 0.7 }];
  const guides = allGuides().map((g) => ({ url: `${SITE.domain}/guides/${g.slug}/`, lastModified: now, priority: 0.6 }));
  return [...home, ...cats, ...tools, ...guidesHub, ...guides];
}
